...
>
	<!--Версия: 0.12-->
	<!--27.10.2016-->
	<!--09.11.2016 xslt поддерживает работу по 2м схемам т.к. РВЗ по схеме statementFormMortgage, а ПИ и ОП по схеме statementFormOwnerless -->
	<!--Версия: 0.13-->
	<!--12.12.2016-->
	...
	<!--Версия схем: 0.18-->
	<!--04.04.2017-->
	<!--Добавился код урд 659411111122 Государственная регистрация передачи права по закладной-->
	<!--15.06.2017-->
	...
	<xsl:variable
		name="tempRepresent"
		select="(//node()/tns:subjects/descendant::subj:representative/subj:subject)"
	/>
	<xsl:variable
		name="tempRepFiltred"
	...
	<xsl:variable
		name="nameOrganInner"
		select="//node()/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:regRightAuthority"
	/>
	...
	<!-- Владелец закладной statementFormMortgage -->
	<xsl:variable name="repRvz" select="'659411111123'" />
	<xsl:variable name="repRvz2" select="'659411111122'" />
	<!-- Принятие имущества statementFormOwnerless -->
	<xsl:variable name="repPi" select="'659411111125'" />
	<!-- Собственник не совершал statementFormOwnerless -->
	<xsl:variable name="repOp" select="'659411111126'" />

	<!--Стартовый шаблон для всех видов-->
	<!-- 2 вида, т.к. РВЗ по схеме statementFormMortgage, а ПИ и ОП по схеме statementFormOwnerless -->
	<xsl:template match="tns:statementFormOwnerless | tns:statementFormMortgage">
		<html lang="ru">
			<head>
...
input[type="reset"],input[type="submit"] {
						...
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">
									<xsl:choose>
										<xsl:when test="$urdActionCode = $repRvz">
											<xsl:text
												>Прошу зарегистрировать законного владельца закладной в
												отношении объекта недвижимости:</xsl:text
											>
										</xsl:when>
										<xsl:when test="$urdActionCode = $repRvz2">
											<xsl:text
												>Прошу зарегистрировать законного владельца закладной в
												отношении объекта недвижимости:</xsl:text
											>
										</xsl:when>
										<xsl:when test="$urdActionCode = $repPi">
											<xsl:text
												>Прошу снять с учета в качестве бесхозяйного объект
												недвижимости в связи с принятием его вновь во владение,
												пользование и распоряжение:</xsl:text
											>
										</xsl:when>
										<xsl:when test="$urdActionCode = $repOp">
											<xsl:text
												>Заявляю о том, что мной не совершались действия,
												направленные на отказ от права собственности, в
												отношении объекта недвижимости:</xsl:text
											>
										</xsl:when>
										<xsl:otherwise>
											<xsl:text
												>Код УРД не соответствует данному XSLT:
											</xsl:text>
											<xsl:value-of select="$urdActionCode" />
										</xsl:otherwise>
										<!--xsl:when test="$rvzpiopActionCode = $repRvz">
										<xsl:text>Прошу зарегистрировать законного владельца закладной в отношении объекта недвижимости:</xsl:text>
									</xsl:when>
									<xsl:when test="$rvzpiopActionCode = $repPi">
										<xsl:text>Прошу снять с учета в качестве бесхозяйного объект недвижимости в связи с принятием его вновь во владение, пользование и распоряжение:</xsl:text>
									</xsl:when>
									<xsl:when test="$rvzpiopActionCode = $repOp">
										<xsl:text>Заявляю о том, что мной не совершались действия, направленные на отказ от права собственности, в отношении объекта недвижимости:</xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:text>Код заявления не опознан: </xsl:text>
										<xsl:value-of select="$rvzpiopActionCode"/>
									</xsl:otherwise-->
									</xsl:choose>
								</div>
							</div>
				...
				<div class="cell clear">
					<div class="table tablemargin">
						<div class="row">
							<div class="table tablemargin">
								<div class="cell leftpad">
									<xsl:choose>
										<xsl:when test="$urdActionCode = $repRvz">
											<xsl:text> Законный владелец закладной:</xsl:text>
										</xsl:when>
										<xsl:when test="$urdActionCode = $repRvz2">
											<xsl:text> Законный владелец закладной:</xsl:text>
										</xsl:when>
										<xsl:when test="$urdActionCode = $repPi">
											<xsl:text> Собственник объекта недвижимости:</xsl:text>
										</xsl:when>
										<xsl:when test="$urdActionCode = $repOp">
											<xsl:text> Собственник объекта недвижимости:</xsl:text>
										</xsl:when>
						...
						<div class="row">
							<div class="table tablemargin">
								<xsl:if
									test="$oorganization or $ocountry or $orfSubject or $oother"
								>
	...
	<xsl:template name="point5">
		<xsl:variable
			name="receiveMethod"
			select="//node()/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:receivingMethodCode"
		/>
				...
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">
									Способ представления заявления и иных документов:
								</div>
							</div>
							<xsl:if
	...
	<xsl:template name="point6">
		<xsl:variable
			name="deliveryMethod"
			select="//node()/tns:deliveryDetails/stCom:resultDeliveryMethod"
		/>
									...
									<div class="table tablemargin">
										...
										<div class="cell leftpad w251 wrap">
											<xsl:text
												...
												приостановлении, сообщения об отказе)</xsl:text
											>
										</div>
										<div class="cell leftpad wrap bold">
											<xsl:value-of
								...
								<div class="row">
									...
									<div class="cell clear">
										<div class="table tablemargin">
													...
													<xsl:when test="normalize-space($tempRepFiltred)">
														<div class="cell leftpad wrap">
															<xsl:choose>
																<xsl:when test="$urdActionCode = $repRvz">
																	<xsl:text
																		>Представитель законного владелеца
																		закладной:</xsl:text
																	>
																</xsl:when>
																<xsl:when test="$urdActionCode = $repRvz2">
																...
																<xsl:when test="$urdActionCode = $repPi">
																	<xsl:text
																		>Представитель собственника объекта
													...
													<xsl:otherwise>
														<div class="cell leftpad wrap">
															<xsl:choose>
																<xsl:when test="$urdActionCode = $repRvz">
																	<xsl:text
																		>Законный владелец закладной лично</xsl:text
																	>
																</xsl:when>
																<xsl:when test="$urdActionCode = $repRvz2">
																	<xsl:text
																...
																<xsl:when test="$urdActionCode = $repPi">
																	<xsl:text
																		>Собственник объекта недвижимости</xsl:text
																	>
																</xsl:when>
																<xsl:when test="$urdActionCode = $repOp">
																	<xsl:text
											...
											<xsl:if test="$ororganization or $orcountry or $orrfSubject or $orother">
											...
											<!-- Не могут быть представителями xsl:if test="normalize-space($repContractorList)">
												<xsl:call-template name="point8_3">
	...
	<xsl:template name="point811">
		...
		<!--xsl:if test="$curPerson/parent::node()/following-sibling::node()">
				<xsl:for-each select="//node()/tns:header/stCom:appliedDocument/node()[@_id=$curPerson/parent::node()/following-sibling::node()/@documentID]">
					<xsl:call-template name="documentTemplate">
						<xsl:with-param name="thisDocument" select="."/>
					</xsl:call-template>
				</xsl:for-each>
	...
	<xsl:template name="point812">
		...
		<xsl:if test="$tmpDocNode/parent::node()/following-sibling::node()">
			<xsl:for-each select="//node()/tns:header/stCom:appliedDocument/node()[@_id=$tmpDocNode/parent::node()/following-sibling::node()/@documentID]">
				<xsl:call-template name="documentTemplate">
					<xsl:with-param name="thisDocument" select="."/>
				</xsl:call-template>
			</xsl:for-each>
	...
	<xsl:template name="point15">
		<xsl:variable
			name="note15"
			select="//node()/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:specialistNote"
		/>
		<xsl:if test="$note15">
			<div class="table">
				<div class="row">
