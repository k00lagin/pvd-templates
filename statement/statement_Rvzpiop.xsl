...
xmlns:adr="http://rosreestr.ru/services/v0.26/commons/Address">
	...
	<xsl:variable name="tempRepresent" select="(//node()/tns:subjects/descendant::subj:representative/subj:subject)"/>
...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
						...
						<div class="table tablemargin">
							<div class="row">
								...
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:text>Код УРД не соответствует данному XSLT: </xsl:text>
	...
	<xsl:template name="point5">
		<xsl:variable name="receiveMethod" select="//node()/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:receivingMethodCode"/>
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">Способ представления заявления и иных документов:</div>
							</div>
	...
	<xsl:template name="point6">
		<xsl:variable name="deliveryMethod" select="//node()/tns:deliveryDetails/stCom:resultDeliveryMethod"/>
							...
							<xsl:if test="$deliveryMethod/stCom:failSuspendEMail">
								<div class="row">
									<div class="table tablemargin">
										<div class="cell center w40 bold">V</div>
										<div class="cell leftpad  w251 wrap">
											<xsl:text>Также по адресу электронной почты: (для уведомления о приостановлении, сообщения об отказе)</xsl:text>
										</div>
										<div class="cell leftpad wrap bold">
												...
												<xsl:choose>
													<xsl:when test="normalize-space($tempRepFiltred)">
														<div class="cell leftpad wrap">
															<xsl:choose>
																<xsl:when test="$urdActionCode = $repRvz">
																	<xsl:text>Представитель законного владелеца закладной:</xsl:text>
																</xsl:when>
																<xsl:when test="$urdActionCode = $repRvz2">
																	<xsl:text>Представитель законного владелеца закладной:</xsl:text>
																</xsl:when>
																<xsl:when test="$urdActionCode =  $repPi">
																	<xsl:text>Представитель собственника объекта недвижимости:</xsl:text>
																</xsl:when>
																<xsl:when test="$urdActionCode = $repOp">
																	<xsl:text>Представитель собственника объекта недвижимости:</xsl:text>
																</xsl:when>
													...
													<xsl:otherwise>
														<div class="cell leftpad wrap">
															<xsl:choose>
																<xsl:when test="$urdActionCode = $repRvz">
																	<xsl:text>Законный владелец закладной лично</xsl:text>
																</xsl:when>
																<xsl:when test="$urdActionCode = $repRvz2">
																	<xsl:text>Законный владелец закладной лично</xsl:text>
																</xsl:when>
	...
	<xsl:template name="point10">
		<xsl:variable name="note10" select="//node()/tns:statementDetails/tns:note"/>
		<xsl:if test="$note10">
			<div class="table">
				<div class="row">
	...
	<xsl:template name="point15">
		<xsl:variable name="note15" select="//node()/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:specialistNote"/>
		<xsl:if test="$note15">
			<div class="table">
				<div class="row">
