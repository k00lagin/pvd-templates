...
>
	...
	<!--24.01.2017-->
	<!--Версия схем: 0.15-->
	<!--09.02.2017  Переименован узел actionDetail в actionDetailMortgage-->
	<!--Версия схем: 0.16-->
	<!--20.02.2017-->
	<!--Версия схем: 0.17-->
	<!--21.03.2017-->
...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
	...
	<xsl:template name="point3_2">
		<xsl:if test="$registryChangeAction">
			<div class="row">
				<div class="table tablemargin">
					...
					<div class="table tablemargin">
						<div class="row">
							...
							<div class="cell clear">
								<div class="table tablemargin">
									<div class="row">
										<div class="cell leftpad wrap">внести изменения в запись реестра прав, ограничений прав и обременений недвижимого имущества Единого государственного реестра недвижимости:</div>
									</div>
									
									<xsl:for-each select="$registryChangeAction/tns:actionDetailMortgage"> 
										<!--xsl:value-of select="."/-->
										<xsl:apply-templates select="."/>
									</xsl:for-each>
								</div>
					...
					<div class="table tablemargin">
						<div class="row">
							...
							<div class="cell clear">
								<div class="table tablemargin">
									<div class="row">
										<div class="cell leftpad wrap">погасить регистрационную запись об ипотеке:</div>
									</div>
									
									<div class="table tablemargin">
										<div class="row">
											<div class="cell leftpad wrap w331">
												<xsl:text>дата государственной регистрации ипотеки:</xsl:text>
											</div>
											<div class="cell leftpad wrap">
												<xsl:text>номер государственной регистрации ипотеки:</xsl:text>
											</div>
										</div>
										<div class="row">
											<div class="cell leftpad w331 bold">
												<xsl:call-template name="DateStr">
	...
	<!--xsl:template match="tns:actionDetail"-->
	<xsl:template match="tns:actionDetailMortgage">
		<xsl:variable name="details" select="tns:details"/>
		<xsl:variable name="regRecordInfo" select="tns:regRecordInfo"/>
		<xsl:if test="$regRecordInfo">
			<div class="row">
				<div class="table tablemargin">
					<div class="cell leftpad w280">
						...
						<span class="bold">
							<xsl:call-template name="DateStr">
								<xsl:with-param name="dateStr" select="$regRecordInfo/tns:regDate"/>
							</xsl:call-template>
						</span>	
					</div>
					<div class="cell leftpad wrap">
						<xsl:text>№ </xsl:text>
						<span class="bold">
							<xsl:value-of select="$regRecordInfo/tns:regNumber"/>
						</span>	
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="table tablemargin">
				<div class="cell leftpad w280 wrap">
					<xsl:text>в связи с изменением сведений:</xsl:text>
				</div>
				<div class="cell leftpad wrap bottomborder bold">
					<xsl:for-each select="$details">
						<xsl:value-of select="com:text"/>
						<xsl:if test="position() != count($details)">
							<xsl:text>, </xsl:text>
						</xsl:if>
					</xsl:for-each>
				</div>
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">Способ получения документов (в том числе уведомления о приостановлении, сообщения об отказе):</div>
							</div>
	...
	<!-- сначала заявители физ лица, потом юр лица, потом без повторений всех представители -->
			...
			<div class="row">
				...
				<div class="cell clear">
					<div class="table tablemargin">
                        ...
                        <!-- отображение заявителей  физлиц -->
                        <xsl:if test="$operson | $opersonPrevileg">
                            <xsl:call-template name="point711n">
                                <xsl:with-param name="personList" select="$operson | $opersonPrevileg"/>
                            </xsl:call-template>
                        </xsl:if>

                        <!-- отображение представителей  физлиц -->
                        <xsl:if test="$orperson | $orpersonPrevileg | $ororganizationPer | $ororganizationPerPrevileg">
                            <xsl:call-template name="point711n">
                                <xsl:with-param name="personList" select="$orperson | $orpersonPrevileg | $ororganizationPer | $ororganizationPerPrevileg"/>
                            </xsl:call-template>
                        </xsl:if>

                        <!-- отображение представителей  ЮР лиц -->
                        <xsl:if test="$ororganization | $orcountry | $orrfSubject | $orother">
                            <xsl:call-template name="point712">
                                <xsl:with-param name="orgList" select="$ororganization | $orcountry | $orrfSubject | $orother"/>
                            </xsl:call-template>
                        </xsl:if>
						...
						<!-- отображение заявителей ЮР лиц --> 
							<xsl:if test="$oorganization | $ocountry | $orfSubject | $oother">
								<xsl:call-template name="point712">
                                    <xsl:with-param name="orgList" select="$oorganization | $ocountry | $orfSubject | $oother"/>
                                </xsl:call-template>
							</xsl:if>
					</div>
				</div>
            ...
            <div class="row">
                ...
                <div class="cell clear">
                    <div class="table tablemargin">
                        ...
                        <xsl:for-each select="$personList">
	...
	<xsl:template name="point9">
		<xsl:variable name="note9" select="//tns:statementFormMortgage/tns:statementDetails/tns:note"/>
		<xsl:if test="$note9">
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="cell leftpad wrap">
								...
								<span class="bold">
									<xsl:for-each select="$note9">
										<xsl:value-of select="com:text"/>
										<xsl:if test="position() != count($note9)">
											<xsl:text>; </xsl:text>
										</xsl:if>
									</xsl:for-each>
								</span>
	...
	<xsl:template name="point12">
		...
		<xsl:variable name="declarantIdList"> 
			  <xsl:for-each select="//tns:statementFormMortgage/tns:subjects/tns:declarant"> 
	...
	<xsl:template name="point14">
		<xsl:variable name="note14" select="//tns:statementFormMortgage/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:specialistNote"/>
		<xsl:if test="$note14">
			<div class="table">
				<div class="row">
