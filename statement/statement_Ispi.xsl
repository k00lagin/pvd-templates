...
input[type="reset"],input[type="submit"] {
								...
								<div class="table tablemargin">
									<div class="row">
										<div class="cell leftpad wrap">
											внести изменения в запись реестра прав, ограничений прав и
											обременений недвижимого имущества Единого государственного
											реестра недвижимости:
										</div>
									</div>

									<xsl:for-each
										select="$registryChangeAction/tns:actionDetailMortgage"
									>
										<!--xsl:value-of select="."/-->
										<xsl:apply-templates select="." />
									</xsl:for-each>
									...
									<div class="table tablemargin">
										<div class="row">
											<div class="cell leftpad wrap w331">
												<xsl:text
													>дата государственной регистрации ипотеки:</xsl:text
												>
											</div>
											<div class="cell leftpad wrap">
												<xsl:text
													>номер государственной регистрации ипотеки:</xsl:text
												>
											</div>
										</div>
										<div class="row">
					...
					<div class="cell leftpad w280">
						...
						<span class="bold">
							<xsl:call-template name="DateStr">
								<xsl:with-param
									name="dateStr"
									select="$regRecordInfo/tns:regDate"
								/>
							</xsl:call-template>
						</span>
					</div>
				...
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">
									Способ получения документов (в том числе уведомления о
									приостановлении, сообщения об отказе):
								</div>
							</div>
							<xsl:if
			...
			<div class="row">
				...
				<div class="cell clear">
					<div class="table tablemargin">
						...
						<xsl:if
							test="$orperson | $orpersonPrevileg | $ororganizationPer | $ororganizationPerPrevileg"
						>
							<xsl:call-template name="point711n">
								<xsl:with-param
									name="personList"
									select="$orperson | $orpersonPrevileg | $ororganizationPer | $ororganizationPerPrevileg"
								/>
							</xsl:call-template>
						</xsl:if>
						...
						<xsl:if
							test="$ororganization | $orcountry | $orrfSubject | $orother"
						>
							<xsl:call-template name="point712">
								<xsl:with-param
									name="orgList"
									select="$ororganization | $orcountry | $orrfSubject | $orother"
								/>
							</xsl:call-template>
						</xsl:if>
	...
	<xsl:template name="point12">
		...
		<xsl:variable name="declarantIdList">
			<xsl:for-each
				select="//tns:statementFormMortgage/tns:subjects/tns:declarant"
			>
