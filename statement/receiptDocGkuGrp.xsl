...
xmlns:adr="http://rosreestr.ru/services/v0.26/commons/Address">
	...
	<xsl:variable name="actions" select="//node()/tns:statementDetails/tns:registryAction"/>
				...
				<div class="cell clear center wrap bottomborder bold">
					...
					<xsl:for-each select="$actions">
						<xsl:if test="string(tns:registryErrorAction/tns:cadastralNumber)">
							<xsl:value-of select="tns:registryErrorAction/tns:cadastralNumber"/>
							<xsl:if test="string(tns:registryErrorAction/tns:regRecordInfo/tns:regNumber)">
								<xsl:text>, </xsl:text>
							</xsl:if>
						</xsl:if>

						<xsl:if test="string(tns:registryErrorAction/tns:regRecordInfo/tns:regNumber)">
							<xsl:value-of select="tns:registryErrorAction/tns:regRecordInfo/tns:regNumber"/>
						</xsl:if>

						<xsl:if test="string(tns:borderErrorAction/tns:borderRecordNumber)">
							<xsl:value-of select="tns:borderErrorAction/tns:borderRecordNumber"/>
						</xsl:if>
