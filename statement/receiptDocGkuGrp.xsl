...
>
	...
	<xsl:variable
		name="owner"
		select="//node()/tns:subjects/tns:owners/tns:owner"
	/>
	<xsl:variable name="declarant" select="//node()/tns:subjects/tns:declarant" />
	<xsl:variable
		name="actions"
		select="//node()/tns:statementDetails/tns:registryAction"
	/>
	<xsl:variable name="signCount" select="count($owner)+count($declarant)" />
	<xsl:variable
		name="receiptCodes"
		select="'-558501030100-558502020100-558503000100-558601000000'"
	/>
	<!-- 558501030100 гку
	558502020100 грп
	...
	<xsl:variable
		name="specialistNote"
		select="//tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:specialistNote"
	/>
...
input[type="reset"],input[type="submit"] {
				...
				<div id="wrapperClear">
					<div class="table">
						...
						<xsl:if test="$typeOrgan = 'MFC'">
							<xsl:if
								test="normalize-space($when) or normalize-space($numPPOZ)"
							>
								<div class="row">
									<div class="cell w45 clear"></div>
									<div class="cell right clear lh-small">
										<div style="font-size: 80%">
	...
	<xsl:template name="point0">
		<xsl:if test="$typeOrgan = 'PVD'">
			...
			<div class="table">
				<div class="row">
					<div class="cell clear center w50 wrap"></div>
			...
			<div class="table">
				<div class="row">
					<div class="cell noborder bold center">
						<div>РАСПИСКА</div>
						<div>
							в получении документов на государственный кадастровый учет и (или)
						</div>
				...
				<div class="row">
					<div class="cell clear w165 center wrap bottomborder bold">
						<xsl:call-template name="getActionCode">
							<xsl:with-param
								name="actionCode"
								select="//node()/tns:header/stCom:actionCode"
							/>
						</xsl:call-template>
					</div>
			...
			<div class="table">
				<div class="row">
					<div class="cell clear w165 center wrap">
						<div style="font-size: 80%">
							<xsl:text
								>&nbsp;&nbsp;&nbsp;&nbsp;(вид учетного и (или) регистрационного
								действия, о совершении которого подано
								заявление)&nbsp;&nbsp;&nbsp;&nbsp;</xsl:text
							>
						</div>
					</div>
					<div class="cell clear w30 center wrap"></div>
					<div class="cell clear w165 center wrap">
						<div style="font-size: 80%">
			...
			<div class="row">
				<div class="cell clear center wrap bottomborder bold">
					<xsl:for-each select="$actions">
						<xsl:if test="string(tns:registryErrorAction/tns:cadastralNumber)">
							<xsl:value-of
								select="tns:registryErrorAction/tns:cadastralNumber"
							/>
							<xsl:if
								test="string(tns:registryErrorAction/tns:regRecordInfo/tns:regNumber)"
							>
								<xsl:text>, </xsl:text>
							</xsl:if>
						</xsl:if>
						...
						>
							<xsl:value-of
								select="tns:registryErrorAction/tns:regRecordInfo/tns:regNumber"
							/>
						</xsl:if>

						<xsl:if test="string(tns:borderErrorAction/tns:borderRecordNumber)">
							<xsl:value-of
								select="tns:borderErrorAction/tns:borderRecordNumber"
							/>
						</xsl:if>
					</xsl:for-each>
				</div>
		...
		<div class="table">
			<div class="row">
				<div class="cell clear center wrap">
					<div style="font-size: 80%">
						<xsl:text
							>(кадастровый номер, адрес (местоположение) объекта
							недвижимости)</xsl:text
						>
					</div>
				</div>
				...
				<div class="cell clear center wrap bottomborder bold">
					<xsl:for-each select="//node()/tns:subjects/tns:declarant">
						...
						<xsl:if test="$curRepresent">
							...
							<xsl:variable
								name="curRepresent2"
								select="subj:representative/subj:subject/subj:representative/subj:subject/node()"
							/>
	...
	<xsl:template name="showDeclarant">
		...
		<xsl:if
			test="contains(name($declarantParam), ':person') or contains(name($declarantParam), ':previligedPerson')"
		>
			<xsl:value-of
				select="concat($declarantParam/subj:surname, ' ', $declarantParam/subj:firstname)"
			/>
		...
		<xsl:if
			test="not(contains(name($declarantParam), ':person') or contains(name($declarantParam), ':previligedPerson'))"
		>
	...
	<xsl:template name="point1">
		...
		<xsl:variable
			name="appDocs"
			select="//node()/tns:header/stCom:appliedDocument/node()[not((contains(name(), ':idDocument')) and (//subj:idDocumentRef/@documentID = @_id)) and (not(normalize-space(doc:number) = normalize-space(doc:supplierBillId) and normalize-space(doc:number) != '' and normalize-space(doc:supplierBillId) != '')) and (not(contains($receiptCodes, doc:documentTypes/node())) ) ]"
		/>

		<!--
and (doc:documentTypes/node()[not(contains($receiptCodes, doc:documentTypes/node()))] )
-->

		<!--xsl:variable name="appDocs" select="//node()/tns:header/stCom:appliedDocument/node()[not(contains(name(), ':idDocument'))]"/-->
		<!-- Шапка -->
		...
		<div id="wrapperClear">
			<span class="lh-middle notoppad">
				<div class="table">
					<div class="row">
						<div
							class="cell w30 wrap noborder topborder leftborder rightborder"
						>
							&nbsp;&nbsp;&nbsp;№
						</div>
						<div
							class="cell center leftpad w360 wrap noborder topborder leftborder rightborder"
						>
							Наименование и реквизиты документов
						</div>
						<div class="cell leftpad w45 noborder topborder leftborder">
							&nbsp;&nbsp;&nbsp;&nbsp;Количество
						</div>
						<div class="cell leftpad w45 noborder topborder rightborder">
							&nbsp;
						</div>
						<div class="cell leftpad w45 noborder topborder leftborder">
							&nbsp;&nbsp;&nbsp;&nbsp;Количество
					...
					<div class="row">
						<div class="cell w30 wrap noborder leftborder rightborder">
							&nbsp;п/п
						</div>
						<div
							class="cell center leftpad w360 wrap noborder leftborder rightborder"
						>
							&nbsp;
						</div>
						<div class="cell leftpad w45 noborder leftborder">
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;экземпляров
						</div>
						<div class="cell center leftpad w45 noborder rightborder">
							&nbsp;
						</div>
						<div class="cell leftpad w45 noborder leftborder">
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;листов
					...
					<div class="row">
						<div class="cell leftpad w30 wrap noborder leftborder rightborder">
							&nbsp;
						</div>
						<div
							class="cell center leftpad w360 wrap noborder leftborder rightborder"
						...
						<div class="cell center leftpad w45 noborder leftborder">
							&nbsp;
						</div>
						<div class="cell center leftpad w45 noborder rightborder">
							&nbsp;
			...
			<div class="table"-->
					<div class="row">
						<div
							class="cell leftpad w30 wrap noborder bottomborder leftborder rightborder"
						>
							&nbsp;
						</div>
						<div
							class="cell center leftpad w360 wrap noborder bottomborder leftborder rightborder"
						>
							<xsl:text>&nbsp;</xsl:text>
							<br />
							<br />
					...
					<xsl:if test="$appDocs">
						<!--div class="table"-->
						<!--div class="row"-->
						<xsl:for-each select="$appDocs">
							<!--Перенесли выше! Не отображаем документы с типом "Документ удостоверяющий личность" -->
							<!--xsl:if test="not(contains(name(), ':idDocument'))"-->
							<div class="row">
								...
								<div class="cell center wrap w45">
									...
									<xsl:if
										test="doc:attachment/doc:receivedInPaper/doc:original/doc:docCount"
									>
										<xsl:value-of
											select="doc:attachment/doc:receivedInPaper/doc:original/doc:docCount"
										/>
									</xsl:if>
								</div>
								<div class="cell center wrap w45">
									...
									<xsl:if
										test="doc:attachment/doc:receivedInPaper/doc:copy/doc:docCount"
									>
										<xsl:value-of
											select="doc:attachment/doc:receivedInPaper/doc:copy/doc:docCount"
										/>
									</xsl:if>
								</div>
								<div class="cell center wrap w45">
									<xsl:if
										test="doc:attachment/doc:receivedInPaper/doc:original/doc:pageCount"
									>
										<xsl:value-of
											select="doc:attachment/doc:receivedInPaper/doc:original/doc:pageCount"
										/>
									</xsl:if>
								</div>
								<div class="cell center wrap w45">
									<xsl:if
										test="doc:attachment/doc:receivedInPaper/doc:copy/doc:pageCount"
									>
										<xsl:value-of
											select="doc:attachment/doc:receivedInPaper/doc:copy/doc:pageCount"
										/>
									</xsl:if>
								</div>
								<div class="cell center wrap w45">
								...
								<!--
										<div class="cell left wrap">
											<xsl:attribute name="style">
												 width:<xsl:value-of select="53"/>px;
												</xsl:attribute>
											<xsl:text>&nbsp;</xsl:text>
										</div>
										<div class="cell left wrap">
	...
	<xsl:template name="point2">
			...
			<div class="row">
				...
				<div class="cell clear w165 center wrap bottomborder">
					...
					<xsl:if test="$deadlineDays">
						...
						<xsl:text
							>&nbsp;р.д. с даты приема заявления на осуществление ГКУ и (или)
							ГРП и прилагаемых к нему документов</xsl:text
						>
						<xsl:if test="$deadlineDate">
							<br />плановая дата выдачи
							<span class="bold"><xsl:value-of select="$deadlineDate" /></span>
						</xsl:if>
					</xsl:if>
					<br />
					<!--/xsl:if-->
		...
		<xsl:if test="$typeOrgan = 'PVD'">
			<xsl:text
				>После проведения государственной регистрации документы
				выданы.</xsl:text
			>
		</xsl:if>
		<xsl:if test="$typeOrgan = 'MFC'">
			<xsl:text
				>После оказания государственной услуги документы выданы.</xsl:text
			>
		</xsl:if>
		<br />
		<br />
		<xsl:if test="$signCount != 0">
			<xsl:for-each
				select="//node()/tns:subjects/tns:owners/tns:owner | //node()/tns:subjects/tns:declarant"
			>
				<div class="table">
					<div class="row">
						<div class="cell clear w165 center wrap bottomborder bold">
				...
				<div style="font-size: 80%">
					<xsl:text
						>&nbsp;(должность, Ф.И.О., подпись сотрудника, выдавшего
						документы)</xsl:text
					>
					<xsl:text
						>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</xsl:text
					...
					<xsl:text
						>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(Ф.И.О.,
						подпись лица, получившего документы)</xsl:text
					>
				</div>
			</xsl:for-each>
		</xsl:if>
	...
	<xsl:template name="showDocumentData">
		...
		<xsl:if test="$thisDocument/doc:amount">
			...
			<xsl:if test="$thisDocument/doc:notaryInfo">
				<xsl:if test="$thisDocument/doc:notaryInfo/doc:registryNumber">
					<xsl:text
						>, сведения о нотариальном удостоверении: реестровый номер
					</xsl:text>
					<xsl:value-of
				...
				<xsl:text>, </xsl:text>
				<xsl:value-of
					select="$thisDocument/doc:notaryInfo/doc:register/subj:surname"
				/>
				<xsl:text> </xsl:text>
				<xsl:value-of
					select="$thisDocument/doc:notaryInfo/doc:register/subj:firstname"
				/>
				<xsl:if
					test="$thisDocument/doc:notaryInfo/doc:register/subj:patronymic"
				>
					<xsl:text> </xsl:text>
					<xsl:value-of
						select="$thisDocument/doc:notaryInfo/doc:register/subj:patronymic"
					/>
				</xsl:if>
			</xsl:if>
		</xsl:if>
		...
		<xsl:if test="not($thisDocument/doc:amount)">
			<xsl:if test="$thisDocument/doc:name">
				...
				>
				<xsl:if
					test="($thisDocument/doc:series or $thisDocument/doc:notes/doc:text or $thisDocument/doc:issuer/doc:name) and not($thisDocument/doc:issueDate or $thisDocument/doc:number) "
					>,
				</xsl:if>
			</xsl:if>
			...
			<xsl:if test="$thisDocument/doc:issueDate">
				...
				<xsl:if
					test="($thisDocument/doc:series or $thisDocument/doc:notes/doc:text or $thisDocument/doc:issuer/doc:name) and not($thisDocument/doc:number) "
					>,
				</xsl:if>
			</xsl:if>
			<xsl:if test="normalize-space($thisDocument/doc:number)">
				...
				<xsl:value-of select="$thisDocument/doc:number" />
				<xsl:if
					test="$thisDocument/doc:series or $thisDocument/doc:notes/doc:text or $thisDocument/doc:issuer/doc:name"
					>,
				</xsl:if>
			</xsl:if>
			<xsl:if test="$thisDocument/doc:series">
				...
				<xsl:if
					test="$thisDocument/doc:notes/doc:text or $thisDocument/doc:issuer/doc:name"
					>,
				</xsl:if>
			</xsl:if>
	...
	<xsl:template name="getActionCode">
		...
		<xsl:value-of
			select="$documentTypeEnum//xs:enumeration[@value=$actionCode]/xs:annotation/xs:documentation"
		/>
	</xsl:template>
	...
	<xsl:template name="ShowJpgImage">
