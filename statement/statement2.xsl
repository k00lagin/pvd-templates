...
xmlns:doc="http://rosreestr.ru/services/v0.26/commons/Documents"
 ...
 xmlns:adr="http://rosreestr.ru/services/v0.26/commons/Address">
	...
	<xsl:variable name="registryChangeAction311" select="//tns:statementForm2/tns:statementDetails/tns:registryAction/tns:registryChangeObjectAction"/>
	<xsl:variable name="registryChangeAction313" select="//tns:statementForm2/tns:statementDetails/tns:registryAction/tns:registryChangeAction"/>
	<xsl:variable name="registryErrorAction" select="//tns:statementForm2/tns:statementDetails/tns:registryAction/tns:registryErrorAction | //tns:statementForm2/tns:statementDetails/tns:registryAction/tns:borderErrorAction"/>
	...
	<!-- Заявители -->
	<xsl:variable name="operson" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:person"/>
	<xsl:variable name="opersonPrevileg" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:previligedPerson"/>
	<xsl:variable name="oorganization" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:organization"/>
	<xsl:variable name="ocountry" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:country"/>
	<xsl:variable name="orfSubject" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:rfSubject"/>
	<xsl:variable name="oother" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:other"/>
	<!-- Представители 1го уровня -->
	<xsl:variable name="orperson" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:representative/subj:subject/subj:person"/>
	<xsl:variable name="orpersonPrevileg" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:representative/subj:subject/subj:previligedPerson"/>
	<xsl:variable name="ororganization" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:representative/subj:subject/subj:organization"/>
	<xsl:variable name="orcountry" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:representative/subj:subject/subj:country"/>
	<xsl:variable name="orrfSubject" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:representative/subj:subject/subj:rfSubject"/>
	<xsl:variable name="orother" select="//tns:statementForm2/tns:subjects/tns:declarant/subj:representative/subj:subject/subj:other"/>
	...
	<xsl:variable name="tempRepresent" select="(//tns:statementForm2/tns:subjects/descendant::subj:representative/subj:subject)"/>
	<xsl:variable name="tempRepFiltred" select="$tempRepresent/self::node()[not(@_id=preceding::subj:subject/@_id)]"/>
	<!-- Разное -->
	<xsl:variable name="appDocs" select="//tns:statementForm2/tns:header/stCom:appliedDocument"/>
	...
	<!-- Блок для подсчета страниц -->
		...
		<xsl:variable name="nameOrganInner" select="tns:statementForm2/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:regRightAuthority"/>
...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
					...
					<div class="row">
						...
						<div class="cell clear">
							<div class="table tablemargin">
								...
								<xsl:for-each select="//tns:statementForm2/tns:objects/stCom:object">
									<!--xsl:value-of select="."/-->
				...
				<div class="table">
					<div class="row">
						...
						<div class="cell clear">
							<div class="table tablemargin">
								<div class="row">
									<div class="cell leftpad wrap">Прошу исправить техническую ошибку, допущенную в сведениях Единого государственного реестра недвижимости:</div>
								</div>
				...
				<div class="table">
					<div class="row">
						...
						<div class="cell clear">
							<div class="table tablemargin">
								<div class="row">
									<div class="cell leftpad">Прошу внести в Единый государственный реестр недвижимости следующие сведения:</div>
								</div>
	...
	<xsl:template match="tns:registryChangeAction | tns:registryChangeObjectAction">
			...
			<div class="table tablemargin">
				...
				<div class="cell leftpad wrap">
					<xsl:if test="tns:regRecordInfo">
						...
						<span class="bold">
							<xsl:call-template name="DateStr">
								<xsl:with-param name="dateStr" select="tns:regRecordInfo/tns:regDate"/>
							</xsl:call-template>
						</span>
	...
	<xsl:template name="point4">
		<xsl:variable name="receiveMethod" select="//tns:statementForm2/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:receivingMethodCode"/>
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">Способ представления заявления и прилагаемых к нему документов:</div>
							</div>
	...
	<xsl:template name="point5">
		<xsl:variable name="deliveryMethod" select="//tns:statementForm2/tns:deliveryDetails/stCom:resultDeliveryMethod"/>
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">Способ получения документов (в том числе выписки из Единого государственного реестра недвижимости, оригиналов ранее представленных документов)</div>
							</div>
										...
										<div class="cell leftpad wrap">
											<xsl:choose>
												<xsl:when test="$deliveryMethod/stCom:receiveDischarge = 'true'">
													<xsl:text>с выдачей выписки из Единого государственного реестра недвижимости</xsl:text>
												</xsl:when>
												<xsl:otherwise>
													<xsl:text>без выдачи выписки из Единого государственного реестра недвижимости</xsl:text>
												</xsl:otherwise>
											</xsl:choose>
										</div>
									</div>
					...
					<div class="table tablemargin">
						...
						<div class="row">
							<div class="table tablemargin wrap">
								...
								<xsl:if test="normalize-space($declarantIdList) or normalize-space($repSignList)">
									<xsl:for-each select="$tempRepFiltred">
										<!-- Проверка отсутствия представителя среди уже выведенных заявителей  --> 
										<xsl:if test="not(contains($declarantIdList, @_id))">
											<xsl:if test="subj:person | subj:previligedPerson">
												<xsl:variable name="headerParam"> 
													<xsl:if test="normalize-space($operson) or normalize-space($opersonPrevileg)">
														<xsl:value-of select="'false'"/>
													</xsl:if>	
													<xsl:if test="not(normalize-space($operson) or normalize-space($opersonPrevileg))">
														<xsl:value-of select="'true'"/>
													</xsl:if>	
												</xsl:variable> 
												<xsl:call-template name="point711">
													<xsl:with-param name="curPerson" select="subj:person | subj:previligedPerson"/>
					...
					<div class="table tablemargin">
						<xsl:if test="$showHeader='true'">
							<div class="row">
								<div class="table tablemargin">
									<div class="cell leftpad wrap">
										<xsl:text>физические лицо (в том числе нотариус, судебный пристав-исполнитель, кадастровый инженер):</xsl:text>
									</div>
								</div>
							</div>
						</xsl:if>
						...
						<xsl:for-each select="$curPerson">
							...
							<xsl:variable name="tmpID" select="parent::node()/child::node()/@documentID | parent::node()/following-sibling::node()/@documentID"/>
							<xsl:if test="$tmpID">
	...
	<xsl:template name="point71person">
		...
		<xsl:variable name="subjDoc" select="//tns:statementForm2/tns:header/stCom:appliedDocument/node()[@_id=$thisPerson/subj:idDocumentRef/@documentID]"/>
		<div class="row">
			<div class="table tablemargin">
	...
	<xsl:template name="point9">
		<xsl:variable name="note9" select="//tns:statementForm2/tns:statementDetails/tns:note"/>
		<xsl:if test="$note9">
			<div class="table">
				<div class="row">
	...
	<xsl:template name="point12">
		<xsl:variable name="phoneNumber" select="//tns:statementForm2/tns:statementAgreements/stCom:qualityOfServiceTelephoneNumber"/>
		<div class="table">
			<div class="row">
	...
	<xsl:template name="point15">
		<xsl:variable name="note15" select="//tns:statementForm2/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:specialistNote"/>
		<xsl:if test="$note15">
			<div class="table">
				<div class="row">
		...
		<xsl:if test="$changeNode/tns:actionDetail/tns:regRecordInfo">
			<xsl:choose>
				<xsl:when test="$actionType = '559010220000' ">
					<xsl:text>внести запись о наличии возражения, заявленном в судебном порядке праве требования в отношении зарегистрированного </xsl:text>
					<xsl:call-template name="DateStrFull">
						<xsl:with-param name="dateStr" select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regDate"/>
					</xsl:call-template>
					<xsl:text> за №  </xsl:text>
					<xsl:value-of select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regNumber"/>
					<xsl:text> права на объект недвижимости</xsl:text>
				</xsl:when>
				<xsl:when test="$actionType = '559010221000' ">
					<xsl:text>погасить запись о наличии возражения в отношении зарегистрированного </xsl:text>
					<xsl:call-template name="DateStrFull">
						<xsl:with-param name="dateStr" select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regDate"/>
					</xsl:call-template>
					<xsl:text> за №  </xsl:text>
				...
				<xsl:when test="$actionType = '559010222000' ">
					<xsl:text>внести запись о невозможности государственной регистрации перехода, прекращения, ограничения права и обременения объекта недвижимости зарегистрированного </xsl:text>
					<xsl:call-template name="DateStrFull">
						<xsl:with-param name="dateStr" select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regDate"/>
					</xsl:call-template>
					<xsl:text> за №  </xsl:text>
					<xsl:value-of select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regNumber"/>
					<xsl:text> без личного участия собственника (его законного представителя)</xsl:text>
				</xsl:when>
				<xsl:when test="$actionType = '559010223000' ">
					<xsl:text>погасить запись о невозможности государственной регистрации перехода, прекращения, ограничения права и обременения объекта недвижимости зарегистрированного </xsl:text>
					<xsl:call-template name="DateStrFull">
						<xsl:with-param name="dateStr" select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regDate"/>
					</xsl:call-template>
					<xsl:text> за №  </xsl:text>
				...
				<xsl:when test="$actionType = '559010226000' ">
					<xsl:text>внести запись о невозможности государственной регистрации перехода, прекращения, ограничения права на земельный участок из земель сельскохозяйственного назначения или обременения такого земельного участка зарегистрированного </xsl:text>
					<xsl:call-template name="DateStrFull">
						<xsl:with-param name="dateStr" select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regDate"/>
					</xsl:call-template>
					<xsl:text> за №  </xsl:text>
					<xsl:value-of select="$changeNode/tns:actionDetail/tns:regRecordInfo/tns:regNumber"/>
					<xsl:text> до завершения рассмотрения судом дела о его изъятии в связи с неиспользованием по целевому назначению или использованием с нарушением законодательства Российской Федерации</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					...
					<xsl:value-of select="$registryActionEnum//xs:enumeration[@value=$actionType]/xs:annotation/xs:documentation"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
