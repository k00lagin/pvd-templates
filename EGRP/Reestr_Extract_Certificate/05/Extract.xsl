...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template name="Extract">
			...
			<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">
				...
				<tr>
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text>Состав объекта недвижимости:</xsl:text>
							</xsl:with-param>
						</xsl:call-template>
					</td>
					<td colspan="2">
		...
		<p>
			<xsl:text
				>Настоящая выписка подтверждает проведение государственной регистрации
	...
	<xsl:template name="Rights">
		...
		<xsl:for-each select="$Rights">
			...
			<tr>
				<td rowspan="{$Encumbrances*6+$Mdf_Encumb+1}">5.</td>
				<td colspan="2">
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
