			...
			<tr>
				...
				<td width="15%" class="ul">
					<xsl:call-template name="Value">
						<xsl:with-param
							name="Node"
							select="parent::ReestrExtract/DeclarAttribute/@ExtractDate"
						/>
						<xsl:with-param
							name="IsDuplicate"
							select="$ExtractExists > 0 or $NoticeExists > 0"
						/>
					</xsl:call-template>
				</td>
				<td align="right">№&#160;</td>
				<td width="35%" class="ul">
					<xsl:call-template name="Value">
						<xsl:with-param
							name="Node"
							select="parent::ReestrExtract/DeclarAttribute/@ExtractNumber"
						/>
						<xsl:with-param
							name="IsDuplicate"
							select="$ExtractExists > 0 or $NoticeExists > 0"
