...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="Reestr_Extract_Subject[eDocument[@Version='02']]">
		<html>
			<head>
				<title>Выписка из ЕГРП о правах лица (версия 02)</title>
			</head>
			<body lang="RU">
				<div style="width: 17cm; text-align: justify">
					<xsl:choose>
						<xsl:when test="//ReestrExtract/ExtractSubjectRights">
							<xsl:call-template name="Header" />
							<p align="center">
								<b
									>ВЫПИСКА<br />ИЗ ЕДИНОГО ГОСУДАРСТВЕННОГО РЕЕСТРА ПРАВ НА
									НЕДВИЖИМОЕ<br />ИМУЩЕСТВО И СДЕЛОК С НИМ О ПРАВАХ ОТДЕЛЬНОГО
									ЛИЦА НА<br />ИМЕЮЩИЕСЯ У НЕГО ОБЪЕКТЫ НЕДВИЖИМОГО ИМУЩЕСТВА</b
								>
							</p>
							...
							<p>
								...
								<xsl:for-each select="//ExtractSubjectRights/Subject">
									<xsl:choose>
								...
								<xsl:text>
									принадлежат следующие объекты недвижимого имущества:</xsl:text
								>
							</p>
							<table border="1" cellpadding="2" cellspacing="0" width="100%">
								...
								<xsl:for-each select="//ExtractSubjectRight/ObjectRight">
									...
									<xsl:variable
										name="Rights"
										select="count(RegistryRecords/Right)"
									/>
									<xsl:variable
										name="Restrictions"
										select="count(RegistryRecords/Right/RightRestriction)"
									/>
									<xsl:variable
										name="Encumbrances"
										select="count(RegistryRecords/Encumbrance)"
									/>
									<tr valign="top">
										<td
											rowspan="{($Restrictions+$Encumbrances)*2+$Rights*4+1+5}"
										>
										...
										<tr valign="top">
											...
											<td>
												...
												<xsl:choose>
													<xsl:when test="Share">
														<xsl:value-of
															select="concat(', ',//@Numerator,'/',//@Denominator,' доли в праве')"
														/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:if test="ShareText">
															<xsl:value-of
																select="concat(', ',//ShareText,' доли в праве')"
															/>
														</xsl:if>
													</xsl:otherwise>
												</xsl:choose>
									...
									<xsl:choose>
										<xsl:when test="($Restrictions + $Encumbrances) > 0">
											...
											<xsl:for-each
												select="RegistryRecords/Right/RightRestriction | RegistryRecords/Encumbrance"
											>
												<tr valign="top">
													<td rowspan="2">
														<xsl:value-of
															select="concat($Object,'.3.',position(),'.')"
														/>
													</td>
													<td>вид:</td>
													<td>
												...
												<tr valign="top">
													...
													<td>
														<xsl:choose>
															<xsl:when
																test="Registration/RegNumber and not(Registration/RegNumber='')"
															>
																<xsl:value-of select="Registration/RegNumber" />
															</xsl:when>
															<xsl:otherwise>не указано</xsl:otherwise>
										...
										<xsl:otherwise>
											<tr valign="top">
												<td><xsl:value-of select="position()" />.3.</td>
												<td colspan="2">Ограничение (обременение) права:</td>
						...
						<xsl:when test="//ReestrExtract/ExtractSubjectRightRefusal">
							<xsl:choose>
								<xsl:when
									test="//ReestrExtract/ExtractSubjectRightRefusal/@RefusalType='002'"
								>
											...
											<td>
												<xsl:choose>
													<xsl:when test="//ExtractSubjectRightRefusal/Subject">
														<xsl:for-each
															select="//ExtractSubjectRightRefusal/Subject"
														>
															<xsl:choose>
																<xsl:when test="Person">
																	<xsl:value-of
													...
													<xsl:otherwise>
														<xsl:choose>
															<xsl:when
																test="//ExtractSubjectRightRefusal/SubjectInfo"
															>
																<xsl:value-of
																	select="//ExtractSubjectRightRefusal/SubjectInfo"
																/>
															</xsl:when>
															<xsl:otherwise>не указано</xsl:otherwise>
														</xsl:choose>
								...
								<xsl:otherwise>
									...
									>
										...
										<tr valign="top">
											...
											<td>
												<xsl:value-of
													select="//ExtractSubjectRightRefusal/@RefusalTypeText"
												/>
											</td>
										</tr>
									</table>
