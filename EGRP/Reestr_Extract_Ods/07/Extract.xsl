...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:variable
		name="FootContent"
		select="Extract/ReestrExtract/ExtractObjectRight/FootContent"
	/>
	<xsl:template name="Extract">
		...
		<xsl:for-each select="$Extract">
			<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">
				<xsl:variable
					name="Mdf_Obj"
					select="count(ObjectRight/ObjectDesc/MdfDate)"
				/>
				<tr>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>Сведения о характеристиках земельного участка:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
					</td>
						...
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:call-template name="Value">
									<xsl:with-param
										name="Node"
										select="ObjectRight/ObjectDesc/CadastralNumber"
									/>
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
						...
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:call-template name="Value">
									<xsl:with-param
										name="Node"
										select="ObjectRight/ObjectDesc/Address/Content"
									/>
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
						...
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:call-template name="Value">
									<xsl:with-param
										name="Node"
										select="ObjectRight/ObjectDesc/GroundCategoryText"
									/>
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
						...
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:call-template name="Value">
									<xsl:with-param
										name="Node"
										select="ObjectRight/ObjectDesc/Assignation_Code_Text"
									/>
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
								...
								<xsl:choose>
									<xsl:when test="ObjectRight/ObjectDesc/Area/AreaText">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="ObjectRight/ObjectDesc/Area/AreaText"
											/>
										</xsl:call-template>
									</xsl:when>
									...
									<xsl:call-template name="Panel">
										<xsl:with-param name="Text">
											<xsl:call-template name="Value">
												<xsl:with-param
													name="Node"
													select="ObjectRight/ObjectDesc/MdfDate"
												/>
											</xsl:call-template>
										</xsl:with-param>
									</xsl:call-template>
				...
				<tr>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>Заявленные в судебном порядке права требования:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
					</td>
						...
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									...
									для государственных и муниципальных нужд:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
					</td>
				...
				<tr>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>Правопритязания и сведения и наличии поступивших, но не
									рассмотренных заявлений о проведении государственной
									регистрации права (перехода, прекращения права), ограничения
				...
				<tr>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>Сведения об отсутствии у застройщика права привлекать
									денежные средства граждан:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
					</td>
			...
			<tr>
				...
				<td colspan="2">
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:text
								>Вид, номер и дата государственной регистрации права:</xsl:text
							>
						</xsl:with-param>
					</xsl:call-template>
				</td>
			...
			<tr>
				...
				<td colspan="2">
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:text
								>Ограничение права и обременение объекта недвижимости:</xsl:text
							>
						</xsl:with-param>
					</xsl:call-template>
				</td>
				...
				<tr>
					<td>
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>срок, на который установлено ограничение права и обременение
									объекта недвижимости:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
				...
				<tr>
					<td>
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>лицо, в пользу которого установлено ограничение права и
									обременение объекта недвижимости:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
					</td>
					...
					<tr>
						<td>
							<xsl:call-template name="Panel">
								<xsl:with-param name="Text">
									<xsl:text
										>сведения о залоге прав требования участника долевого
										строительства, ином ограничении его прав:</xsl:text
									>
								</xsl:with-param>
							</xsl:call-template>
						</td>
									...
									<xsl:choose>
										<xsl:when test="Encumbrance">
											<xsl:for-each select="Encumbrance">
												...
												<xsl:text>;</xsl:text>
												<xsl:if
													test="Duration/Started|Duration/Stopped|Duration/Term"
												>
													<br />
													<xsl:text
														>срок, на который установлено ограничение
														(обременение) права:
													</xsl:text>
													<xsl:choose>
														<xsl:when test="Duration/Term">
					...
					<tr>
						<td>
							<xsl:call-template name="Panel">
								<xsl:with-param name="Text">
									<xsl:text
										>способ обеспечения застройщиком исполнения обязательств по
										договору:</xsl:text
									>
								</xsl:with-param>
							</xsl:call-template>
					...
					<tr>
						<td>
							<xsl:call-template name="Panel">
								<xsl:with-param name="Text">
									<xsl:text
										>сведения о банке, в котором в соответствии с договором
										участия в долевом строительстве должен быть открыт
										специальный счет эскроу для специального депонирования
										денежных средств в счет уплаты цены такого
										договора:</xsl:text
									>
								</xsl:with-param>
							</xsl:call-template>
