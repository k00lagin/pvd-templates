	...
	<xsl:template name="GetAssignationName">
		...
		<xsl:choose>
			...
			<xsl:when test="$AssignationCode='005002001004'">
				<xsl:value-of
					select="'Здравоохранения, физической культуры и социального обеспечения'"
				/>
			</xsl:when>
			...
			<xsl:when test="$AssignationCode='005002002011'">
				<xsl:value-of
					select="'Электротехнической, приборостроительной и радиопромышленности'"
				/>
			</xsl:when>
			...
			<xsl:when test="$AssignationCode='005002002017'">
				<xsl:value-of
					select="'Целлюлозно-бумажной и лесохимической промышленности'"
				/>
			</xsl:when>
			...
			<xsl:when test="$AssignationCode='005002002022'">
				<xsl:value-of
					select="'Швейной, кожевенной, меховой, обувной промышленности'"
				/>
			</xsl:when>
			...
			<xsl:when test="$AssignationCode='005002002025'">
				<xsl:value-of
					select="'Рыбоперерабатывающей и плодоовощной промышленности'"
				/>
			</xsl:when>
