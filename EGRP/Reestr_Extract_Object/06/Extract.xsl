								...
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="ObjectRight/ObjectDesc/ConditionalNumber"
											/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
						...
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:call-template name="Value">
									<xsl:with-param
										name="Node"
										select="ObjectRight/ObjectDesc/Name"
									/>
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
							...
							<xsl:with-param name="Text">
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:choose>
											<xsl:when
												test="ObjectRight/ObjectDesc/GroundCategoryText"
											>
												<xsl:call-template name="Value">
													<xsl:with-param
														name="Node"
								...
								<xsl:choose>
									<xsl:when test="ObjectRight/ObjectDesc/Inv_No">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="ObjectRight/ObjectDesc/Inv_No"
											/>
										</xsl:call-template>
									</xsl:when>
									<xsl:otherwise>&#160;</xsl:otherwise>
								...
								<xsl:choose>
									<xsl:when test="ObjectRight/ObjectDesc/Floor">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="ObjectRight/ObjectDesc/Floor"
											/>
										</xsl:call-template>
									</xsl:when>
									<xsl:otherwise>&#160;</xsl:otherwise>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:choose>
									<xsl:when
										test="ObjectRight/ObjectDesc/FloorPlan_No/Explication"
									>
										<xsl:for-each
											select="ObjectRight/ObjectDesc/FloorPlan_No/Explication"
										>
							...
							<xsl:with-param name="Text">
								<xsl:choose>
									<xsl:when test="ObjectRight/ObjectDesc/Complex/Explication">
										<xsl:for-each
											select="ObjectRight/ObjectDesc/Complex/Explication"
										>
											<xsl:if test="not(position()=1)">
												<xsl:text>; </xsl:text>
											</xsl:if>
				...
				<tr>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>Отметка о возражении в отношении зарегистрированного
									права:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
				...
				<tr>
					...
					<td colspan="2">
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>Отметка о наличии решения об изъятии объекта недвижимости для
				...
				<tr>
					<td>
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>срок, на который установлено ограничение (обременение)
									права:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
				...
				<tr>
					<td>
						<xsl:call-template name="Panel">
							<xsl:with-param name="Text">
								<xsl:text
									>лицо, в пользу которого установлено ограничение (обременение)
									права:</xsl:text
								>
							</xsl:with-param>
						</xsl:call-template>
	...
	<xsl:template name="Shares">
		...
		<xsl:choose>
			<xsl:when test="count($Shares/ShareHolding) > 0">
				<tr>
					<td
						rowspan="{count($Shares/ShareHolding)+count($Shares/ShareHolding/Owner)+count($Shares/ShareHolding/Encumbrance)+1}"
					>
						5.
					</td>
					<td colspan="2">
