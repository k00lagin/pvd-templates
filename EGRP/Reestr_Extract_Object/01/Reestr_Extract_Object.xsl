...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template
		match="Reestr_Extract_Object[eDocument[@Version='01']] | Reestr_Extract[eDocument[@Version='01'] and (count(//ExtractObjectRight/RightAssert) > 0 or count(//ExtractObjectRightRefusal/RightAssert) > 0)]"
	>
		<html>
			<head>
				...
				<div style="width: 17cm; text-align: justify">
					<xsl:choose>
						<xsl:when test="//ReestrExtract/ExtractObjectRight">
							...
							<p align="center">
								<b
									>ВЫПИСКА<br />ИЗ ЕДИНОГО ГОСУДАРСТВЕННОГО РЕЕСТРА ПРАВ<br />НА
									НЕДВИЖИМОЕ ИМУЩЕСТВО И СДЕЛОК С НИМ</b
								>
							</p>
							...
							<p>
								...
								<xsl:text
									>, сообщаем, что в Едином государственном реестре прав на
									недвижимое имущество и сделок с ним
									зарегистрировано:</xsl:text
								>
							</p>
							<table border="1" cellpadding="2" cellspacing="0" width="100%">
								<xsl:for-each select="//ExtractObjectRight/ObjectRight">
									...
									<xsl:variable
										name="Parts"
										select="count(RegistryRecords/ShareHolding)"
									/>
									<col width="1%" />
									<col width="2%" />
									<col width="45%" />
									...
									<xsl:for-each select="RegistryRecords/Encumbrance">
										<xsl:variable
											name="EncumbranceIndex"
											select="position()+$RightRows"
										/>
										<tr valign="top">
											<td>2.</td>
											<td colspan="2">Правообладатель (правообладатели):</td>
									...
									<td colspan="2">
										<xsl:choose>
											<xsl:when test="//ExtractObjectRight/RightAssert[text()]">
												<xsl:value-of
													select="//ExtractObjectRight/RightAssert"
												/>
											</xsl:when>
									...
									<td colspan="2">
										<xsl:choose>
											<xsl:when test="//ExtractObjectRight/RightClaim[text()]">
												<xsl:value-of
													select="//ExtractObjectRight/RightClaim"
												/>
											</xsl:when>
						...
						<xsl:when test="//ReestrExtract/ExtractObjectRightRefusal">
							<xsl:choose>
								<xsl:when
									test="//ReestrExtract/ExtractObjectRightRefusal/@RefusalType='002'"
								>
									...
									>
										...
										<tr valign="top">
											...
											<td>
												о зарегистрированных правах на объект недвижимого
												имущества
											</td>
										</tr>
										<tr valign="top">
								...
								<xsl:otherwise>
									...
									>
										...
										<tr valign="top">
											...
											<td>
												<xsl:value-of
													select="//ExtractObjectRightRefusal/@RefusalTypeText"
												/>
											</td>
										</tr>
									</table>
