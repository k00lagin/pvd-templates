					...
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:call-template name="Value">
								<xsl:with-param
									name="Node"
									select="RealReg/ObjectDesc/CadastralNumber"
								/>
							</xsl:call-template>
						</xsl:with-param>
					</xsl:call-template>
					...
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:call-template name="Value">
								<xsl:with-param
									name="Node"
									select="RealReg/ObjectDesc/ObjectTypeText"
								/>
							</xsl:call-template>
						</xsl:with-param>
					</xsl:call-template>
							...
							<xsl:choose>
								<xsl:when test="RealReg/ObjectDesc/Assignation_Code_Text">
									<xsl:call-template name="Value">
										<xsl:with-param
											name="Node"
											select="RealReg/ObjectDesc/Assignation_Code_Text"
										/>
									</xsl:call-template>
								</xsl:when>
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="RealReg/ObjectDesc/GroundCategoryText">
											<xsl:call-template name="Value">
												<xsl:with-param
													name="Node"
													select="RealReg/ObjectDesc/GroundCategoryText"
												/>
											</xsl:call-template>
										</xsl:when>
			...
			<tr>
				<td colspan="2">
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:text
								>Основная характеристика объекта м ее значение:</xsl:text
							>
						</xsl:with-param>
					</xsl:call-template>
				</td>
							...
							<xsl:choose>
								<xsl:when test="RealReg/ObjectDesc/Area/AreaText">
									<xsl:call-template name="Value">
										<xsl:with-param
											name="Node"
											select="RealReg/ObjectDesc/Area/AreaText"
										/>
									</xsl:call-template>
								</xsl:when>
								<xsl:otherwise>&#160;</xsl:otherwise>
					...
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:call-template name="Value">
								<xsl:with-param
									name="Node"
									select="RealReg/ObjectDesc/Address/Content"
								/>
							</xsl:call-template>
						</xsl:with-param>
					</xsl:call-template>
