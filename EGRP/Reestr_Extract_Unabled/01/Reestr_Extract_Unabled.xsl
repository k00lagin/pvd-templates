...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="Reestr_Extract_Unabled[eDocument[@Version='01']] | Reestr_Extract[eDocument[@Version='01'] and (//ExtractSbjIncompetent or //ExtractRefusal/Subject or //ExtractRefusal//SubjectInfo)]">
		<html>
			<head>
				<title>Выписка о признании правообладателя недееспособным или ограниченно дееспособным (версия 01)</title>
			</head>
			<body lang="RU">
				<div style="width:17cm;text-align:justify">
					<xsl:choose>
						<xsl:when test="//ReestrExtract/ExtractSbjIncompetent">
							...
							<p align="center">
								<b>ВЫПИСКА<br/>ИЗ ЕДИНОГО ГОСУДАРСТВЕННОГО РЕЕСТРА ПРАВ НА НЕДВИЖИМОЕ<br/>ИМУЩЕСТВО И СДЕЛОК С НИМ<br/>О ПРИЗНАНИИ ПРАВООБЛАДАТЕЛЯ НЕДЕЕСПОСОБНЫМ ИЛИ<br/>ОГРАНИЧЕННО ДЕЕСПОСОБНЫМ</b>
							</p>
							<xsl:call-template name="DateNumber"/>
							<p>
								<xsl:call-template name="Request"/>
								<xsl:text>, сообщаем, что согласно записям Единого государственного реестра прав на недвижимое имущество и сделок с ним:</xsl:text>
							</p>
								...
								<xsl:when test="//ReestrExtract/ExtractRefusal/@RefusalType='002'">
									...
									<table border="1" cellpadding="2" cellspacing="0" width="100%">
										...
										<tr valign="top">
											...
											<td>о признании правообладателя недееспособным или ограниченно дееспособным</td>
										</tr>
										<tr valign="top">
											<td>2.</td>
													...
													<xsl:otherwise>
														<xsl:choose>
															<xsl:when test="//ExtractRefusal/SubjectInfo">
																<xsl:value-of select="//ExtractRefusal/SubjectInfo"/>
															</xsl:when>
