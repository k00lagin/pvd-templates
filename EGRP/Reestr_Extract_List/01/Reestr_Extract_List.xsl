...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template
		match="Reestr_Extract_List[eDocument[@Version='01']] | Reestr_Extract[eDocument[@Version='01'] and count(//ExtractObjectRight/RightAssert) = 0 and count(//ExtractObjectRightRefusal/RightAssert) = 0 and (//ExtractObjectRight or //ExtractObjectRightRefusal)]"
	>
		<html>
			<head>
				<title>
					Выписка о переходе прав на объект недвижимого имущества (версия 01)
				</title>
			</head>
			<body lang="RU">
						...
						<xsl:when test="//ReestrExtract/ExtractObjectRight">
							...
							<p align="center">
								<b
									...
									НЕДВИЖИМОЕ<br />ИМУЩЕСТВО И СДЕЛОК С НИМ О ПЕРЕХОДЕ ПРАВ НА
									ОБЪЕКТ<br />НЕДВИЖИМОГО ИМУЩЕСТВА</b
								>
							</p>
							...
							<p>
								...
								<xsl:text
									>, сообщаем, что в Единый государственный реестр прав на
									недвижимое имущество и сделок с ним внесены записи о
									государственной регистрации перехода прав на:</xsl:text
								>
							</p>
							<table border="1" cellpadding="2" cellspacing="0" width="100%">
								<xsl:for-each select="//ExtractObjectRight/ObjectRight">
									...
									<xsl:variable
										name="rows"
										select="count(RegistryRecords/Right)*5+1"
									/>
									<tr valign="top">
									...
									<xsl:for-each select="RegistryRecords/Right">
										...
										<tr valign="top">
											...
											<td>
												<xsl:value-of
													select="Registration/OpenRegistration/RegDate"
												/>
											</td>
										</tr>
										<tr valign="top">
											...
											<td>
												<xsl:value-of
													select="Registration/OpenRegistration/RegNumber"
												/>
											</td>
										</tr>
										<tr valign="top">
											...
											<td>
												<xsl:choose>
													<xsl:when test="Registration/CloseRegistration">
														<xsl:value-of
															select="Registration/CloseRegistration/RegDate"
														/>
													</xsl:when>
