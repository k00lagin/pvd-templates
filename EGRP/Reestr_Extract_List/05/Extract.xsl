		...
		<xsl:for-each select="$Extract">
			<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">
								...
								<xsl:choose>
									<xsl:when test="ObjectDesc/Assignation_Code_Text">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="ObjectDesc/Assignation_Code_Text"
											/>
										</xsl:call-template>
									</xsl:when>
									<xsl:otherwise>
										<xsl:choose>
											<xsl:when test="ObjectDesc/GroundCategoryText">
												<xsl:call-template name="Value">
													<xsl:with-param
														name="Node"
														select="ObjectDesc/GroundCategoryText"
													/>
												</xsl:call-template>
											</xsl:when>
								...
								<xsl:choose>
									<xsl:when test="ObjectDesc/Area/AreaText">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="ObjectDesc/Area/AreaText"
											/>
										</xsl:call-template>
									</xsl:when>
									<xsl:otherwise>&#160;</xsl:otherwise>
				...
				<xsl:for-each select="Registration">
					...
					<xsl:variable
						name="RightOwners"
						select="../Owner[@OwnerNumber=$RightIndex]"
					/>
					<tr>
					...
					<tr>
						<td rowspan="{count(Registration)}">
							<xsl:call-template name="Panel">
								<xsl:with-param name="Text">
									<xsl:text
										>вид зарегистрированного права; доля в праве:</xsl:text
									>
								</xsl:with-param>
							</xsl:call-template>
						</td>
					...
					<tr>
						<td rowspan="{count(Registration)}">
							<xsl:call-template name="Panel">
								<xsl:with-param name="Text">
									<xsl:text
										>дата, номер и основание государственной регистрации
										перехода (прекращения) права:</xsl:text
									>
								</xsl:with-param>
							</xsl:call-template>
						</td>
