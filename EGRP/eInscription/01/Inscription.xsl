				...
				<div class="block">
								...
								<xsl:call-template name="Panel">
									<xsl:with-param name="Text">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="//eInscription/Department"
											/>
										</xsl:call-template>
									</xsl:with-param>
								</xsl:call-template>
									...
									<tr>
										<xsl:call-template name="LabelWithText">
											...
											<xsl:with-param name="Label"
												>Номер регистрации</xsl:with-param
											>
											<xsl:with-param
												name="Text"
												select="substring-before($place, '##')"
											/>
										</xsl:call-template>
									</tr>
								</xsl:when>
					...
					<table border="0" cellspacing="1" cellpadding="5" width="100%">
								...
								<xsl:call-template name="Panel">
									<xsl:with-param name="Text">
										<xsl:call-template name="Value">
											<xsl:with-param
												name="Node"
												select="//eInscription/RegistrarName"
											/>
										</xsl:call-template>
									</xsl:with-param>
								</xsl:call-template>
						...
						<tr>
							<td width="220"></td>
							<td width="2"></td>
							<td width="50" valign="top" align="center" style="font-size: 8pt">
								(подпись)
							</td>
							<td width="2">&#160;</td>
							<td
