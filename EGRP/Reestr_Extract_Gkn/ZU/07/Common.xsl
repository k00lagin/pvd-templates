...
<xsl:stylesheet
	...
	xmlns:kp="urn://x-artefacts-rosreestr-ru/outgoing/kpzu/6.0.1"
...
>
	...
	<xsl:variable
		name="SExtract"
		select="kp:KPZU/kp:ReestrExtract/kp:ExtractObjectRight"
	/>
	<xsl:variable
		name="DeclarAttribute"
		select="kp:KPZU/kp:ReestrExtract/kp:DeclarAttribute"
	/>
	<xsl:variable
		name="HeadContent"
		select="kp:KPZU/kp:ReestrExtract/kp:ExtractObjectRight/kp:HeadContent"
	/>
	<xsl:variable
		name="FootContent"
		select="kp:KPZU/kp:ReestrExtract/kp:ExtractObjectRight/kp:FootContent"
	/>
		...
		<html>
			<head>
				...
				<style type="text/css">
					...
					.tbl_section_sign {
						font: bold x-small Arial, Verdana, Geneva, Helvetica, sans-serif;
					}
					.tbl_section_sign TD {
				...
				<xsl:if test="descendant::kp:EntitySpatial">
					<xsl:comment
						><![CDATA[[if (lt IE 9)|!(IE)]><script
						type="text/javascript">document.createElement("canvas").getContext||function(){function
						H(){return this.context_||(this.context_=new w(this))}function
						I(a,b,c){var d=A.call(arguments,2);return function(){return
						a.apply(b,d.concat(A.call(arguments)))}}function J(a){var
						b=a.srcElement;switch(a.propertyName){case
						"width":b.style.width=b.attributes.width.nodeValue+"px";b.getContext().clearRect();break;case
						"height":b.style.height=b.attributes.height.nodeValue+"px",b.getContext().clearRect()}}function
						K(a){a=a.srcElement;if(a.firstChild)a.firstChild.style.width=
						a.clientWidth+"px",a.firstChild.style.height=a.clientHeight+"px"}function
						x(){return[[1,0,0],[0,1,0],[0,0,1]]}function p(a,b){for(var
						c=x(),d=0;d<3;d++)for(var f=0;f<3;f++){for(var
						e=0,j=0;j<3;j++)e+=a[d][j]*b[j][f];c[d][f]=e}return c}function
						B(a,b){b.fillStyle=a.fillStyle;b.lineCap=a.lineCap;b.lineJoin=a.lineJoin;b.lineWidth=a.lineWidth;b.miterLimit=a.miterLimit;b.shadowBlur=a.shadowBlur;b.shadowColor=a.shadowColor;b.shadowOffsetX=a.shadowOffsetX;b.shadowOffsetY=a.shadowOffsetY;b.strokeStyle=a.strokeStyle;
						b.globalAlpha=a.globalAlpha;b.arcScaleX_=a.arcScaleX_;b.arcScaleY_=a.arcScaleY_;b.lineScale_=a.lineScale_}function
						C(a){var
						b,c=1,a=String(a);if(a.substring(0,3)=="rgb"){b=a.indexOf("(",3);var
						d=a.indexOf(")",b+1),d=a.substring(b+1,d).split(",");b="#";for(var
						f=0;f<3;f++)b+=D[Number(d[f])];d.length==4&&a.substr(3,1)=="a"&&(c=d[3])}else
						b=a;return{color:b,alpha:c}}function L(a){switch(a){case
						"butt":return"flat";case
						"round":return"round";default:return"square"}}function
						w(a){this.m_=x();this.mStack_=
						[];this.aStack_=[];this.currentPath_=[];this.fillStyle=this.strokeStyle="#000";this.lineWidth=1;this.lineJoin="miter";this.lineCap="butt";this.miterLimit=k*1;this.globalAlpha=1;this.canvas=a;var
						b=a.ownerDocument.createElement("div");b.style.width=a.clientWidth+"px";b.style.height=a.clientHeight+"px";b.style.overflow="hidden";b.style.position="absolute";a.appendChild(b);this.element_=b;this.lineScale_=this.arcScaleY_=this.arcScaleX_=1}function
						E(a,b,c,d){a.currentPath_.push({type:"bezierCurveTo",
						cp1x:b.x,cp1y:b.y,cp2x:c.x,cp2y:c.y,x:d.x,y:d.y});a.currentX_=d.x;a.currentY_=d.y}function
						q(a,b,c){var d;a:{for(d=0;d<3;d++)for(var
						f=0;f<2;f++)if(!isFinite(b[d][f])||isNaN(b[d][f])){d=!1;break
						a}d=!0}if(d&&(a.m_=b,c))a.lineScale_=M(N(b[0][0]*b[1][1]-b[0][1]*b[1][0]))}function
						u(a){this.type_=a;this.r1_=this.y1_=this.x1_=this.r0_=this.y0_=this.x0_=0;this.colors_=[]}function
						F(){}var
						s=Math,h=s.round,y=s.sin,z=s.cos,N=s.abs,M=s.sqrt,k=10,r=k/2,A=Array.prototype.slice,G={init:function(a){/MSIE/.test(navigator.userAgent)&&
						!window.opera&&(a=a||document,a.createElement("canvas"),a.attachEvent("onreadystatechange",I(this.init_,this,a)))},init_:function(a){a.namespaces.g_vml_||a.namespaces.add("g_vml_","urn:schemas-microsoft-com:vml","#default#VML");a.namespaces.g_o_||a.namespaces.add("g_o_","urn:schemas-microsoft-com:office:office","#default#VML");if(!a.styleSheets.ex_canvas_){var
						b=a.createStyleSheet();b.owningElement.id="ex_canvas_";b.cssText="canvas{display:inline-block;overflow:hidden;text-align:left;width:300px;height:150px}g_vml_\\:*{behavior:url(#default#VML)}g_o_\\:*{behavior:url(#default#VML)}"}a=
						a.getElementsByTagName("canvas");for(b=0;b<a.length;b++)this.initElement(a[b])},initElement:function(a){if(!a.getContext){a.getContext=H;a.innerHTML="";a.attachEvent("onpropertychange",J);a.attachEvent("onresize",K);var
						b=a.attributes;b.width&&b.width.specified?a.style.width=b.width.nodeValue+"px":a.width=a.clientWidth;b.height&&b.height.specified?a.style.height=b.height.nodeValue+"px":a.height=a.clientHeight}return
						a}};G.init();for(var D=[],e=0;e<16;e++)for(var
						v=0;v<16;v++)D[e*16+v]=e.toString(16)+
						v.toString(16);e=w.prototype;e.clearRect=function(){this.element_.innerHTML=""};e.beginPath=function(){this.currentPath_=[]};e.moveTo=function(a,b){var
						c=this.getCoords_(a,b);this.currentPath_.push({type:"moveTo",x:c.x,y:c.y});this.currentX_=c.x;this.currentY_=c.y};e.lineTo=function(a,b){var
						c=this.getCoords_(a,b);this.currentPath_.push({type:"lineTo",x:c.x,y:c.y});this.currentX_=c.x;this.currentY_=c.y};e.bezierCurveTo=function(a,b,c,d,f,e){f=this.getCoords_(f,e);a=this.getCoords_(a,b);c=this.getCoords_(c,
						d);E(this,a,c,f)};e.quadraticCurveTo=function(a,b,c,d){a=this.getCoords_(a,b);c=this.getCoords_(c,d);d={x:this.currentX_+2/3*(a.x-this.currentX_),y:this.currentY_+2/3*(a.y-this.currentY_)};E(this,d,{x:d.x+(c.x-this.currentX_)/3,y:d.y+(c.y-this.currentY_)/3},c)};e.arc=function(a,b,c,d,f,e){c*=k;var
						j=e?"at":"wa",h=a+z(d)*c-r,i=b+y(d)*c-r,d=a+z(f)*c-r,f=b+y(f)*c-r;h==d&&!e&&(h+=0.125);a=this.getCoords_(a,b);h=this.getCoords_(h,i);d=this.getCoords_(d,f);this.currentPath_.push({type:j,x:a.x,y:a.y,radius:c,
						xStart:h.x,yStart:h.y,xEnd:d.x,yEnd:d.y})};e.rect=function(a,b,c,d){this.moveTo(a,b);this.lineTo(a+c,b);this.lineTo(a+c,b+d);this.lineTo(a,b+d);this.closePath()};e.strokeRect=function(a,b,c,d){var
						f=this.currentPath_;this.beginPath();this.moveTo(a,b);this.lineTo(a+c,b);this.lineTo(a+c,b+d);this.lineTo(a,b+d);this.closePath();this.stroke();this.currentPath_=f};e.fillRect=function(a,b,c,d){var
						f=this.currentPath_;this.beginPath();this.moveTo(a,b);this.lineTo(a+c,b);this.lineTo(a+c,b+d);this.lineTo(a,
						b+d);this.closePath();this.fill();this.currentPath_=f};e.createLinearGradient=function(a,b,c,d){var
						f=new u("gradient");f.x0_=a;f.y0_=b;f.x1_=c;f.y1_=d;return
						f};e.createRadialGradient=function(a,b,c,d,f,e){var h=new
						u("gradientradial");h.x0_=a;h.y0_=b;h.r0_=c;h.x1_=d;h.y1_=f;h.r1_=e;return
						h};e.drawImage=function(a,b){var
						c,d,f,e,j,m,i,g;f=a.runtimeStyle.width;e=a.runtimeStyle.height;a.runtimeStyle.width="auto";a.runtimeStyle.height="auto";var
						l=a.width,n=a.height;a.runtimeStyle.width=f;a.runtimeStyle.height=
						e;if(arguments.length==3)c=arguments[1],d=arguments[2],j=m=0,i=f=l,g=e=n;else
						if(arguments.length==5)c=arguments[1],d=arguments[2],f=arguments[3],e=arguments[4],j=m=0,i=l,g=n;else
						if(arguments.length==9)j=arguments[1],m=arguments[2],i=arguments[3],g=arguments[4],c=arguments[5],d=arguments[6],f=arguments[7],e=arguments[8];else
						throw Error("Invalid number of arguments");var
						o=this.getCoords_(c,d),t=[];t.push(" <g_vml_:group",'
						coordsize="',k*10,",",k*10,'"',' coordorigin="0,0"','
						style="width:',10,
						"px;height:",10,"px;position:absolute;");if(this.m_[0][0]!=1||this.m_[0][1]){var
						r=[];r.push("M11=",this.m_[0][0],",","M12=",this.m_[1][0],",","M21=",this.m_[0][1],",","M22=",this.m_[1][1],",","Dx=",h(o.x/k),",","Dy=",h(o.y/k),"");var
						p=this.getCoords_(c+f,d),q=this.getCoords_(c,d+e);c=this.getCoords_(c+f,d+e);o.x=s.max(o.x,p.x,q.x,c.x);o.y=s.max(o.y,p.y,q.y,c.y);t.push("padding:0
						",h(o.x/k),"px ",h(o.y/k),"px
						0;filter:progid:DXImageTransform.Microsoft.Matrix(",r.join(""),",
						sizingmethod='clip');")}else t.push("top:",
						h(o.y/k),"px;left:",h(o.x/k),"px;");t.push(' ">','<g_vml_:image
						src="',a.src,'"',' style="width:',k*f,"px;"," height:",k*e,'px;"','
						cropleft="',j/l,'"',' croptop="',m/n,'"','
						cropright="',(l-j-i)/l,'"',' cropbottom="',(n-m-g)/n,'"',"
						/>","</g_vml_:group>");this.element_.insertAdjacentHTML("BeforeEnd",t.join(""))};e.stroke=function(a){var
						b=[],c=C(a?this.fillStyle:this.strokeStyle),d=c.color,c=c.alpha*this.globalAlpha;b.push("<g_vml_:shape",'
						filled="',!!a,'"',' style="position:absolute;width:',10,
						"px;height:",10,'px;"',' coordorigin="0 0" coordsize="',k*10,"
						",k*10,'"',' stroked="',!a,'"',' path="');for(var
						f=null,e=null,j=null,m=null,i=0;i<this.currentPath_.length;i++){var
						g=this.currentPath_[i];switch(g.type){case "moveTo":b.push(" m
						",h(g.x),",",h(g.y));break;case "lineTo":b.push(" l
						",h(g.x),",",h(g.y));break;case "close":b.push(" x
						");g=null;break;case "bezierCurveTo":b.push(" c
						",h(g.cp1x),",",h(g.cp1y),",",h(g.cp2x),",",h(g.cp2y),",",h(g.x),",",h(g.y));break;case
						"at":case "wa":b.push(" ", g.type,"
						",h(g.x-this.arcScaleX_*g.radius),",",h(g.y-this.arcScaleY_*g.radius),"
						",h(g.x+this.arcScaleX_*g.radius),",",h(g.y+this.arcScaleY_*g.radius),"
						",h(g.xStart),",",h(g.yStart),"
						",h(g.xEnd),",",h(g.yEnd))}if(g){if(f==null||g.x<f)f=g.x;if(j==null||g.x>j)j=g.x;if(e==null||g.y<e)e=g.y;if(m==null||g.y>m)m=g.y}}b.push('
						">');if(a)if(typeof this.fillStyle=="object"){var
						d=this.fillStyle,l=0,g=c=a=0,n=1;d.type_=="gradient"?(l=d.x1_/this.arcScaleX_,f=d.y1_/this.arcScaleY_,i=this.getCoords_(d.x0_/this.arcScaleX_,
						d.y0_/this.arcScaleY_),l=this.getCoords_(l,f),l=Math.atan2(l.x-i.x,l.y-i.y)*180/Math.PI,l<0&&(l+=360),l<1.0E-6&&(l=0)):(i=this.getCoords_(d.x0_,d.y0_),g=j-f,n=m-e,a=(i.x-f)/g,c=(i.y-e)/n,g/=this.arcScaleX_*k,n/=this.arcScaleY_*k,i=s.max(g,n),g=2*d.r0_/i,n=2*d.r1_/i-g);f=d.colors_;f.sort(function(a,b){return
						a.offset-b.offset});for(var
						e=f.length,m=f[0].color,j=f[e-1].color,o=f[0].alpha*this.globalAlpha,r=f[e-1].alpha*this.globalAlpha,p=[],i=0;i<e;i++){var
						q=f[i];p.push(q.offset*n+g+" "+q.color)}b.push('<g_vml_:fill
						type="', d.type_,'"',' method="none" focus="100%"','
						color="',m,'"',' color2="',j,'"',' colors="',p.join(","),'"','
						opacity="',r,'"',' g_o_:opacity2="',o,'"',' angle="',l,'"','
						focusposition="',a,",",c,'" />')}else b.push('<g_vml_:fill
						color="',d,'" opacity="',c,'" />');else
						a=this.lineScale_*this.lineWidth,a<1&&(c*=a),b.push("<g_vml_:stroke",'
						opacity="',c,'"',' joinstyle="',this.lineJoin,'"','
						miterlimit="',this.miterLimit,'"',' endcap="',L(this.lineCap),'"','
						weight="',a,'px"',' color="',d,'" />');b.push("</g_vml_:shape>");
						this.element_.insertAdjacentHTML("beforeEnd",b.join(""))};e.fill=function(){this.stroke(!0)};e.closePath=function(){this.currentPath_.push({type:"close"})};e.getCoords_=function(a,b){var
						c=this.m_;return{x:k*(a*c[0][0]+b*c[1][0]+c[2][0])-r,y:k*(a*c[0][1]+b*c[1][1]+c[2][1])-r}};e.save=function(){var
						a={};B(this,a);this.aStack_.push(a);this.mStack_.push(this.m_);this.m_=p(x(),this.m_)};e.restore=function(){B(this.aStack_.pop(),this);this.m_=this.mStack_.pop()};e.translate=function(a,b){q(this,p([[1,
						0,0],[0,1,0],[a,b,1]],this.m_),!1)};e.rotate=function(a){var
						b=z(a),a=y(a);q(this,p([[b,a,0],[-a,b,0],[0,0,1]],this.m_),!1)};e.scale=function(a,b){this.arcScaleX_*=a;this.arcScaleY_*=b;q(this,p([[a,0,0],[0,b,0],[0,0,1]],this.m_),!0)};e.transform=function(a,b,c,d,f,e){q(this,p([[a,b,0],[c,d,0],[f,e,1]],this.m_),!0)};e.setTransform=function(a,b,c,d,e,h){q(this,[[a,b,0],[c,d,0],[e,h,1]],!0)};e.clip=function(){};e.arcTo=function(){};e.createPattern=function(){return
						new F};u.prototype.addColorStop=function(a,
			...
			<xsl:element name="body">
				<xsl:attribute name="onload">
					...
					<xsl:call-template name="BodyFunctionCycle">
						...
						<xsl:with-param
							name="total"
							select="count(descendant::kp:EntitySpatial[parent::kp:SubParcel]|descendant::kp:EntitySpatial[parent::kp:Contour]|descendant::kp:EntitySpatial[parent::kp:EntryParcel])"
						/>
						<!--<xsl:with-param name="cur_index" select="0"/>-->
						<xsl:with-param
							name="cur_index"
							select="count(descendant::kp:EntitySpatial[parent::kp:SubParcel])"
						/>
					</xsl:call-template>
				</xsl:attribute>
				<table
	...
	<xsl:template match="kp:Parcel">
		<xsl:variable name="countV1Line4_np">
			<xsl:if
				test="(count(kp:PrevCadastralNumbers/kp:CadastralNumber)+count(kp:OldNumbers/num:OldNumber/@Number)) &gt; 3"
			>
				<xsl:value-of select="1" />
			</xsl:if>
			<xsl:if
				test="not((count(kp:PrevCadastralNumbers/kp:CadastralNumber)+count(kp:OldNumbers/num:OldNumber/@Number)) &gt; 3)"
			>
		...
		<xsl:variable name="countV1Line9">
			<xsl:if
				test="string-length(normalize-space(kp:Utilization/@ByDoc)) &gt; 200"
			>
				<xsl:value-of select="1" />
			</xsl:if>
			<xsl:if
				test="not(string-length(normalize-space(kp:Utilization/@ByDoc)) &gt; 200)"
			>
				<xsl:value-of select="0" />
			</xsl:if>
		</xsl:variable>
		<xsl:variable name="countV1Line7">
			<xsl:if
				test="string-length(normalize-space(kp:Location/kp:Address/adr:Note)) &gt; 300"
			>
				<xsl:value-of select="1" />
			</xsl:if>
			<xsl:if
				test="not(string-length(normalize-space(kp:Location/kp:Address/adr:Note)) &gt; 300)"
			>
				<xsl:value-of select="0" />
			</xsl:if>
		</xsl:variable>
		<xsl:variable name="countV1Line16">
			...
			<xsl:if test="kp:Name='02'">
				<xsl:value-of
					select="ceiling(count(kp:CompositionEZ/kp:EntryParcel) div $max_page_records)"
				/>
			</xsl:if>
			<xsl:if test="kp:Name='05'">
				<xsl:value-of
					select="ceiling(count(kp:Contours/kp:Contour) div $max_page_records)"
				/>
			</xsl:if>
		</xsl:variable>
		<xsl:variable name="list1">
			<xsl:value-of
				select="1+$countV1Line16+$countV1Line9+$countV1Line7+$countV1Line4"
			/>
		</xsl:variable>
		...
		<xsl:variable name="list2c">
			...
			<xsl:value-of
				select="count(descendant::kp:EntitySpatial[parent::kp:Contour]|descendant::kp:EntitySpatial[parent::kp:EntryParcel])"
			/>
			<!--<xsl:value-of select="1"/>-->
		</xsl:variable>
		<xsl:variable name="list3">
			<xsl:value-of
				select="ceiling((count(kp:SubParcels/kp:SubParcel)+count(kp:Encumbrances/kp:Encumbrance)) div ($countV3))"
			/>
		</xsl:variable>
		...
		<xsl:if test="$typeExtract != '010000'">
			<xsl:if test="$countV1Line16 &gt; 0">
				<xsl:call-template name="Line16_Cycle">
					<xsl:with-param
						name="prev_pages_total"
						select="1+$countV1Line9+$countV1Line7+$countV1Line4"
					/>
		...
		<xsl:if test="$typeExtract != '010000'">
			<xsl:if test="$list4 &gt; 0">
				...
				<xsl:call-template name="V4_Form_Cycle">
					<xsl:with-param
						name="prev_pages_total"
						select="$list1+$list2+$list2c+$list3"
					/>
		...
		<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">
			...
			<tr>
				...
				<th style="text-align: left">
					...
					<xsl:if
						test="not(string(@DateCreatedDoc)) and not(string(@DateCreated))"
					>
			...
			<tr>
				...
				<th style="text-align: left">
					<xsl:for-each select="kp:OldNumbers/num:OldNumber/@Number">
						<xsl:variable
							name="old_numbers"
							select="document('schema/KPZU_v06/SchemaCommon/dOldNumber_v01.xsd')"
						/>
						<xsl:variable name="type" select="../@Type" />
						<xsl:value-of
							select="$old_numbers//xs:enumeration[@value=$type]/xs:annotation/xs:documentation"
						/>
						<xsl:if
							test="$old_numbers//xs:enumeration[@value=$type]/xs:annotation/xs:documentation"
							>:
						</xsl:if>
						<xsl:value-of select="." />
			...
			<tr>
				...
				<th style="text-align: left">
					<xsl:variable
						name="categories"
						select="document('schema/KPZU_v06/SchemaCommon/dCategories_v01.xsd')"
					/>
					<xsl:variable name="kod" select="kp:Category" />
					<xsl:value-of
						select="$categories//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"
					/>
				</th>
			</tr>
			<tr valign="top">
				...
				<th style="text-align: left">
					<xsl:if test="@State='05'">
						...
						<xsl:if test="@DateExpiry">
							<xsl:text
								>. Дата истечения срока действия временного характера -
							</xsl:text>
					...
					<xsl:if test="@State='01'">
						<xsl:text
							>Сведения об объекте недвижимости имеют статус "актуальные, ранее
							учтенные"</xsl:text
						>
					</xsl:if>
	...
	<xsl:template name="OBJ_FROM_EGRP">
		...
		<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">
			...
			<tr valign="top">
				...
				<td style="text-align: left">
					<xsl:text
						>Сведения об объекте недвижимости сформированы по данным ранее
						внесенным в Единый Государственный реестр прав.</xsl:text
					>
					<br />
					<xsl:text
						>Сведения необходимые для заполнения раздела 3
						отсутствуют.</xsl:text
					>
				</td>
			</tr>
			<tr valign="top">
		...
		<tr>
			...
			<th style="text-align: left">
				<xsl:if test="string(kp:Utilization/@LandUse)">
					<xsl:variable
						name="var0"
						select="document('schema/KPZU_v06/SchemaCommon/dAllowedUse_v02.xsd')"
					/>
					<xsl:variable name="kod" select="kp:Utilization/@LandUse" />
					<xsl:value-of
						select="$var0//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"
					/>
				</xsl:if>
				<xsl:if
					test="string(kp:Utilization/@ByDoc) and not(string(kp:Utilization/@LandUse))"
				>
					<xsl:value-of select="normalize-space(kp:Utilization/@ByDoc)" />
				</xsl:if>
				<xsl:if
					test="not(string(kp:Utilization/@ByDoc)) and not(string(kp:Utilization/@ByDoc)) and not(string(kp:Utilization/@LandUse))"
				>
					<xsl:variable
						name="var0"
						select="document('schema/KPZU_v06/SchemaCommon/dUtilizations_v01.xsd')"
					/>
					...
					/>
					<xsl:if
						test="not(string($var0//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation))"
					>
						<xsl:text>данные отсутствуют</xsl:text>
					</xsl:if>
				</xsl:if>
	...
	<xsl:template name="line12Area">
		...
		<th style="text-align: left" colspan="">
			<xsl:if test="string(kp:Area/kp:Area)">
				...
				<xsl:text> </xsl:text>
				<xsl:if
					test="string(kp:Area/kp:Inaccuracy) and ceiling(kp:Area/kp:Inaccuracy)!=0"
				>
					+/-
	...
	<xsl:template name="Extract">
		...
		<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">
			<xsl:call-template name="Rights">
				<xsl:with-param
					name="Rights"
					select="$SExtract/kp:ExtractObject/kp:ObjectRight"
				/>
			</xsl:call-template>
			<tr>
				<td>5.</td>
					...
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:text
								...
								государственных и муниципальных нужд:</xsl:text
							>
						</xsl:with-param>
					</xsl:call-template>
				</td>
			...
			<tr>
				...
				<td colspan="2">
					<xsl:call-template name="Panel">
						<xsl:with-param name="Text">
							<xsl:text
								>Сведения об осуществлении государственной регистрации прав без
								необходимого в силу закона согласия третьего лица,
								органа:</xsl:text
							>
						</xsl:with-param>
					</xsl:call-template>
	...
	<xsl:template name="Rights">
		...
		<xsl:for-each select="$Rights/kp:Right">
			...
			<xsl:variable
				name="Mdf_Encumb"
				select="count(kp:Encumbrance/kp:MdfDate)"
			/>
			<xsl:if
				test="position()>1 and
				(
			...
			>
				...
				<xsl:value-of
					disable-output-escaping="yes"
					select="concat('&lt;', 'table class=t border=1 cellpadding=2 cellspacing=0 width=100%', '&gt;')"
				/>
			</xsl:if>
			<tr>
				<td width="1%">1.</td>
										...
										<xsl:choose>
											...
											<xsl:when test="kp:Organization">
												<xsl:call-template name="Value">
													<xsl:with-param
														name="Node"
														select="kp:Organization/kp:Content"
													/>
												</xsl:call-template>
												<xsl:choose>
											...
											<xsl:when test="kp:Governance">
												<xsl:call-template name="Value">
													<xsl:with-param
														name="Node"
														select="kp:Governance/kp:Content"
													/>
												</xsl:call-template>
											</xsl:when>
										</xsl:choose>
					...
					<tr>
						...
						<td colspan="2">
							<xsl:call-template name="Panel">
								<xsl:with-param name="Text">
									<xsl:text
										>Вид, номер и дата государственной регистрации
										права:</xsl:text
									>
								</xsl:with-param>
							</xsl:call-template>
									...
									<xsl:choose>
										<xsl:when test="kp:Registration">
											<xsl:call-template name="Value">
												<xsl:with-param
													name="Node"
													select="kp:Registration/kp:Name"
												/>
											</xsl:call-template>
											<xsl:call-template name="Value">
												<xsl:with-param
													name="Node"
													select="kp:Registration/kp:RestorCourt"
												/>
											</xsl:call-template>
											<xsl:choose>
							...
							<xsl:if test="(position() mod 3) = 0">
								<xsl:element name="td">
									...
									<xsl:attribute name="rowspan">
										<xsl:choose>
											<xsl:when test="count(../kp:Encumbrance) - position() > 3"
												><xsl:value-of select="(6+$Mdf_D)*3"
											/></xsl:when>
											<xsl:otherwise
												><xsl:value-of
													select="(6+$Mdf_D)*(count(../kp:Encumbrance) - position() + 1)"
											/></xsl:otherwise>
										</xsl:choose>
									</xsl:attribute>
									<xsl:text>&nbsp;</xsl:text>
						...
						<tr>
							<td>
								<xsl:call-template name="Panel">
									<xsl:with-param name="Text">
										<xsl:text
											>срок, на который установлено ограничение прав и
											обременение объекта:</xsl:text
										>
									</xsl:with-param>
								</xsl:call-template>
							</td>
						...
						<tr>
							<td>
								<xsl:call-template name="Panel">
									<xsl:with-param name="Text">
										<xsl:text
											>лицо, в пользу которого установлено ограничение прав и
											обременение объекта:</xsl:text
										>
									</xsl:with-param>
								</xsl:call-template>
	...
	<xsl:template name="line15">
		...
		<tr>
			...
			<th style="text-align: left">
				<xsl:variable
					name="count3"
					select="count($parcel/kp:SubParcels | $parcel/kp:EntitySpatial | $parcel/kp:Contours | $parcel/kp:CompositionEZ//kp:EntitySpatial)"
				/>
				...
				<xsl:if test="count(kp:CompositionEZ/kp:EntryParcel) &gt; 0">
					<xsl:if
						test="$parcel/kp:SpecialNote or count(kp:Contours/kp:Contour) &gt; 0"
						><br
					/></xsl:if>
					<xsl:call-template name="EZList"> </xsl:call-template>
				</xsl:if>
				<xsl:if
					test="not($parcel/descendant::kp:EntitySpatial) and not(contains(string($parcel/kp:SpecialNote), 'Граница земельного участка не установлена в соответствии с требованиями земельного законодательства'))"
				>
					<xsl:if
						test="$parcel/kp:SpecialNote or count(kp:Contours/kp:Contour) &gt; 0 or count(kp:CompositionEZ/kp:EntryParcel) &gt; 0"
						><br
					/></xsl:if>
					<xsl:text
						>Граница земельного участка не установлена в соответствии с
						требованиями земельного законодательства</xsl:text
					>
				</xsl:if>
				<xsl:if test="not($SExtract/kp:ExtractObject)">
					<xsl:if
						test="$parcel/kp:SpecialNote or
					...
					<xsl:text
						>Сведения необходимые для заполнения раздела 2
						отсутствуют.</xsl:text
					>
				</xsl:if>
				...
				<xsl:if
					test="$SExtract/kp:ExtractObject
	...
	<xsl:template name="CountoursLine15">
		...
		<xsl:param name="countCurrent" />
		<xsl:if
			test="not(contains($parcel/kp:SpecialNote, 'Граница земельного участка состоит из'))"
		>
			<xsl:value-of
				select="concat('Граница земельного участка состоит из ',count(kp:Contours/kp:Contour),' контуров. ')"
			/>
		</xsl:if>
	...
	<xsl:template name="CompositionLine15">
		...
		<xsl:param name="countCurrent" />
		<xsl:text
			>Список кадастровых номеров обособленных (условных) участков, входящих в
			единое землепользование, приведен на
		</xsl:text>
		<xsl:if test="$countCurrent &gt; 1">
			<xsl:value-of
				select="concat('листах № ',1+$countPrev,' - ',$countPrev+$countCurrent,'. ')"
			/>
		</xsl:if>
		<xsl:if test="not($countCurrent &gt; 1)">
			<xsl:value-of select="concat('листе № ',1+$countPrev,'. ')" />
		...
		<tr>
			...
			<th style="text-align: left; padding-left: 5px" colspan="3">
				...
				<xsl:if test="kp:NaturalObjects">
					<xsl:variable
						name="natural"
						select="document('schema/KPZU_v06/SchemaCommon/dNaturalObjects_v01.xsd')"
					/>
					<xsl:variable
						name="forest"
						select="document('schema/KPZU_v06/SchemaCommon/dForestUse_v01.xsd')"
					/>
					<xsl:variable
						name="forestProt"
						select="document('schema/KPZU_v06/SchemaCommon/dForestCategoryProtective_v01.xsd')"
					/>
					<xsl:variable
						name="forestEnc"
						select="document('schema/KPZU_v06/SchemaCommon/dForestEncumbrances_v01.xsd')"
					/>
					<xsl:for-each select="kp:NaturalObjects/nat:NaturalObject">
						<xsl:variable name="kod0" select="nat:Kind" />
						<xsl:value-of
							select="$natural//xs:enumeration[@value=$kod0]/xs:annotation/xs:documentation"
						/>
						<xsl:if
							test="nat:Forestry or nat:ForestUse or nat:ProtectiveForest or nat:QuarterNumbers or nat:TaxationSeparations or nat:ForestEncumbrances or nat:WaterObject or nat:NameOther or nat:CharOther"
						>
							(</xsl:if
						>
						<xsl:if test="nat:Forestry">
							...
							<xsl:if
								test="nat:ForestUse or nat:ProtectiveForest or nat:QuarterNumbers or nat:TaxationSeparations or nat:ForestEncumbrances or nat:WaterObject or nat:NameOther or nat:CharOther"
								>;
							</xsl:if>
						</xsl:if>
						<xsl:if test="nat:ForestUse">
							...
							<xsl:value-of
								select="$forest//xs:enumeration[@value=$kod1]/xs:annotation/xs:documentation"
							/>
							<xsl:if
								test="nat:ProtectiveForest or nat:QuarterNumbers or nat:TaxationSeparations or nat:ForestEncumbrances or nat:WaterObject or nat:NameOther or nat:CharOther"
								>;
							</xsl:if>
						</xsl:if>
						<xsl:if test="nat:ProtectiveForest">
							...
							<xsl:value-of
								select="$forestProt//xs:enumeration[@value=$kod3]/xs:annotation/xs:documentation"
							/>
							<xsl:if
								test="nat:QuarterNumbers or nat:TaxationSeparations or nat:ForestEncumbrances or nat:WaterObject or nat:NameOther or nat:CharOther"
								>;
							</xsl:if>
						</xsl:if>
						<xsl:if test="nat:QuarterNumbers">
							...
							<xsl:if
								test="nat:TaxationSeparations or nat:ForestEncumbrances or nat:WaterObject or nat:NameOther or nat:CharOther"
								>;
							</xsl:if>
						</xsl:if>
						<xsl:if test="nat:TaxationSeparations">
							...
							<xsl:if
								test="nat:ForestEncumbrances or nat:WaterObject or nat:NameOther or nat:CharOther"
								>;
							</xsl:if>
						</xsl:if>
						<xsl:if test="nat:ForestEncumbrances">
							...
							<xsl:for-each
								select="nat:ForestEncumbrances/nat:ForestEncumbrance"
							>
								<xsl:variable name="kod2" select="text()" />
								<xsl:value-of
									select="$forestEnc//xs:enumeration[@value=$kod2]/xs:annotation/xs:documentation"
								/>
								<xsl:if test="position()!=last()">; </xsl:if>
							</xsl:for-each>
							<xsl:if test="nat:WaterObject or nat:NameOther or nat:CharOther"
								>;
							</xsl:if>
						</xsl:if>
	...
	<xsl:template name="line17KP">
		...
		<tr>
			...
			>
				<xsl:text
					>Кадастровые номера участков, образованных с земельным участком:
				</xsl:text>
		...
		<tr>
			...
			>
				<xsl:text
					>Кадастровые номера участков, подлежащих снятию или снятых с
					кадастрового учета:
				</xsl:text>
		...
		<tr>
			...
			>
				<xsl:text
					>Кадастровые номера участков, образованных из земельного участка:
				</xsl:text>
		...
		<tr>
			...
			<th style="text-align: left; padding-left: 5px" colspan="2">
				...
				<xsl:for-each select="$contractors/kp:Contractor">
					<xsl:value-of
						select="normalize-space(concat(kp:FamilyName,' ',kp:FirstName,' ',kp:Patronymic,' №',kp:NCertificate))"
					/>
	...
	<xsl:template name="Line16_Cycle">
		...
		<xsl:if test="$cur_index &lt; $border_pages_total">
			...
			<xsl:call-template name="Line16_Cycle">
				...
				<xsl:with-param
					name="border_pages_total"
					select="$border_pages_total"
				/>
				<xsl:with-param name="listAll" select="$listAll" />
			</xsl:call-template>
		</xsl:if>
							...
							<tr>
								...
								<th style="text-align: left">
									<xsl:call-template name="topSheets">
										...
										<xsl:with-param
											name="curSheet"
											select="$prev_total_pages+$index_cur+1"
										/>
										<xsl:with-param name="allSheets" select="$listAll" />
									</xsl:call-template>
								</th>
							...
							<tr>
								...
								<th style="text-align: left; border-style: none">
									...
									>
										...
										<xsl:for-each
											select="kp:Contours/kp:Contour|kp:CompositionEZ/kp:EntryParcel"
										>
											<xsl:if
												test="(position() &lt; $max_page_records+1+$position_cur) and (position() &gt; $position_cur)"
											>
												<tr>
													...
													>
														<xsl:value-of
															select="@NumberRecord|@CadastralNumber"
														/>
														<br />
													</th>
													<th
													...
													>
														<xsl:if test="string(@DateRemoved)">
															<xsl:text
																>Земельный участок снят с государственного
																кадастрового учета
															</xsl:text>
															<xsl:apply-templates select="@DateRemoved" />
														</xsl:if>
														<xsl:if
															test="not(string(@DateRemoved)) or self::kp:Contour"
														>
															<xsl:call-template name="procherk" />
														</xsl:if>
													</th>
				...
				<td colspan="4">
					...
					<center>
						...
						<xsl:text
							...
							основных характеристиках и зарегистрированных правах на объект
							недвижимости</xsl:text
						>
						<!--<br/>
						<xsl:text>и зарегистрированных правах на объект недвижимости</xsl:text>
	...
	<xsl:template name="V2_FormC_Cycle">
		...
		<xsl:if test="$cur_index &lt; $contour_pages_total">
			...
			<xsl:call-template name="V2_FormC_Cycle">
				<xsl:with-param name="prev_pages_total" select="$prev_pages_total" />
				<xsl:with-param
					name="contour_pages_total"
					select="$contour_pages_total"
				/>
				<xsl:with-param name="cur_index" select="$cur_index+1" />
				<xsl:with-param name="listAll" select="$listAll" />
				<xsl:with-param name="formKind" select="'3'" />
	...
	<xsl:template name="V2_FormC">
		...
		<xsl:variable
			name="canvas"
			select="concat('canvas', $index_cur+count(descendant::kp:EntitySpatial[parent::kp:SubParcel]))"
		/>
		<!--<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<th>
	...
	<xsl:template name="V3_Form">
		...
		<xsl:variable
			name="dif"
			select="$countV3+0-count(kp:SubParcels/kp:SubParcel) mod $countV3"
		/>
						...
						<tr>
							<th style="border-style: none">
								...
								>
									...
									<xsl:for-each
										select="kp:SubParcels/kp:SubParcel[position() &gt; $position_cur][position() &lt; $countV3+1]"
									>
										<tr>
											...
											<td style="text-align: center">
												<xsl:if test="@Full='1'"> весь </xsl:if>
											...
											<td style="text-align: left; padding-left: 5px">
												...
												<xsl:if test="(@State='05')">
													...
													<xsl:if test="@DateExpiry">
														<xsl:text
															>. Дата истечения срока действия временного
															характера -
														</xsl:text>
														<xsl:apply-templates select="@DateExpiry" />
													</xsl:if>
												...
												<xsl:if
													test="not(@State) and not(string(kp:Encumbrance))"
												>
													<xsl:call-template name="procherk" />
												</xsl:if>
												<br />
									...
									<xsl:if
										test="$position_cur+$countV3 &gt; $list3_1 and $position_cur &lt; $list3_1+1"
									>
										<xsl:for-each
											select="kp:Encumbrances/kp:Encumbrance[position() &lt; $dif+1]"
										>
											<tr>
												<td style="text-align: center">
													<xsl:value-of
														select="position()+$position_cur+$countV3+0-$dif"
													/>
												</td>
									...
									<xsl:if test="$position_cur &gt; $list3_1">
										<xsl:for-each
											select="kp:Encumbrances/kp:Encumbrance[position() &gt; $position_cur+0-$list3_1][position() &lt; $countV3+1]"
										>
											<tr>
												<td style="text-align: center">
						...
						<tr>
							...
							<th style="text-align: left">
								<xsl:call-template name="topSheets">
									...
									<xsl:with-param
										name="curSheet"
										select="$prev_pages_total+$index_cur+1"
									/>
									<xsl:with-param name="allSheets" select="$listAll" />
								</xsl:call-template>
							</th>
						...
						<tr>
							...
							>
								...
								<u>
									<b>
										<xsl:value-of
											select="concat(@CadastralNumber, '/', descendant::kp:EntitySpatial[parent::kp:SubParcel][$index_cur+1]/parent::node()/@NumberRecord)"
										/>
									</b>
								</u>
							</th>
	...
	<xsl:template
		match="kp:PrevCadastralNumbers|kp:OldNumbers|kp:InnerCadastralNumbers|kp:AllNewParcels|kp:ChangeParcel|kp:RemoveParcels|kp:AllOffspringParcel"
	>
	...
	<xsl:template match="kp:Documents">
		<xsl:for-each select="kp:Document">
			...
			<xsl:if test="not(string(doc:Name))">
				<xsl:variable
					name="var"
					select="document('schema/KPZU_v06/SchemaCommon/dAllDocumentsOut_v03.xsd')"
				/>
				<xsl:variable name="kod" select="doc:CodeDocument" />
				<xsl:value-of
					select="$var//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"
				/>
			</xsl:if>
	...
	<xsl:template match="kp:Address">
		<xsl:if test="not(string(adr:Note))">
			...
			<xsl:if test="string(adr:Region)">
				<xsl:variable
					name="region"
					select="document('schema/KPZU_v06/SchemaCommon/dRegionsRF_v01.xsd')"
				/>
				<xsl:variable name="kod" select="adr:Region" />
				<xsl:value-of
					select="$region//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"
				/>
				<xsl:if
					test="adr:District or adr:City or adr:UrbanDistrict or adr:SovietVillage or adr:Locality or adr:Street or adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:District/@Name)">
				<xsl:value-of select="adr:District/@Name" />&nbsp;<xsl:value-of
					select="adr:District/@Type"
				/>
				<xsl:if
					test="adr:City or adr:UrbanDistrict or adr:SovietVillage or adr:Locality or adr:Street or adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:City/@Name)">
				<xsl:value-of select="adr:City/@Type" />&nbsp;<xsl:value-of
					select="adr:City/@Name"
				/>
				<xsl:if
					test="adr:UrbanDistrict or adr:SovietVillage or adr:Locality or adr:Street or adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:UrbanDistrict/@Name)">
				<xsl:value-of select="adr:UrbanDistrict/@Type" />&nbsp;<xsl:value-of
					select="adr:UrbanDistrict/@Name"
				/>
				<xsl:if
					test="adr:SovietVillage or adr:Locality or adr:Street or adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:SovietVillage/@Name)">
				<xsl:value-of select="adr:SovietVillage/@Type" />&nbsp;<xsl:value-of
					select="adr:SovietVillage/@Name"
				/>
				<xsl:if
					test="adr:Locality or adr:Street or adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:Locality/@Name)">
				<xsl:value-of select="adr:Locality/@Type" />&nbsp;<xsl:value-of
					select="adr:Locality/@Name"
				/>
				<xsl:if
					test="adr:Street or adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:Street/@Name)">
				<xsl:value-of select="adr:Street/@Type" />&nbsp;<xsl:value-of
					select="adr:Street/@Name"
				/>
				<xsl:if
					test="adr:Level1 or adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:Level1/@Value)">
				<xsl:value-of select="adr:Level1/@Type" />&nbsp;<xsl:value-of
					select="adr:Level1/@Value"
				/>
				<xsl:if test="adr:Level2 or adr:Level3 or adr:Apartment or adr:Other"
					>,
				</xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:Level2/@Value)">
				<xsl:value-of select="adr:Level2/@Type" />&nbsp;<xsl:value-of
					select="adr:Level2/@Value"
				/>
				<xsl:if test="adr:Level3 or adr:Apartment or adr:Other">, </xsl:if>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:if test="string(adr:Level3/@Value)">
				<xsl:value-of select="adr:Level3/@Type" />&nbsp;<xsl:value-of
					select="adr:Level3/@Value"
				/>
				<xsl:if test="adr:Apartment or adr:Other">, </xsl:if>
			</xsl:if>
			<xsl:if test="string(adr:Apartment/@Value)">
				<xsl:value-of select="adr:Apartment/@Type" />&nbsp;<xsl:value-of
					select="adr:Apartment/@Value"
				/>
			...
			<tr>
				<td valign="top" align="center">
					<font style="font-size: 80%"
						>(полное наименование органа регистрации прав)</font
					>
				</td>
			</tr>
		</table>
		...
		<div class="floatL">
			...
			<xsl:if test="string($certificationDoc/cert:Date)">
				<u>
					<b>
						<xsl:apply-templates
							select="$certificationDoc/cert:Date"
							mode="word"
						/>
					</b>
				</u>
		...
		<table class="t" border="1" cellpadding="2" cellspacing="0" width="100%">-->
			<tr>
				<td width="25%">
					...
					<u
						><b>&nbsp;<xsl:value-of select="$curRazd" />&nbsp;</b></u
					>
				</td>
				<td width="30%">
					...
					<b>
						...
						<xsl:if test="$DeclarAttribute/@ExtractNumber!='0'">
							&nbsp;<xsl:text> № </xsl:text>&nbsp; &nbsp;<xsl:value-of
								select="$DeclarAttribute/@ExtractNumber"
							/>&nbsp;
						</xsl:if>
					</b>
				</td>
			...
			<tr>
				...
				<td colspan="2">
					<b>
						...
						<xsl:if test="/kp:KPZU/kp:Parcel/kp:CompositionEZ"
							>&nbsp;(единое землепользование)</xsl:if
						>
					</b>
				</td>
			</tr>
	...
	<xsl:template name="procherk">
		<table
			...
			width="50px"
			cellspacing="0"
			style="border-width: 0px; display: inline"
		>
			<tr valign="top">
				<td style="width: 5%" />
	...
	<xsl:template
		match="@DateCreated|@DateCreatedDoc|@DateRemoved|@DateExpiry|@Date|doc:Date|kp:RegDate|kp:Started|kp:Stopped"
	>
	...
	<xsl:template match="kp:EntitySpatial">
		...
		<xsl:variable
			name="itemEntity"
			select="$count - count(following::kp:EntitySpatial) - 1"
		/>
		...
		<xsl:apply-templates select="descendant::spa:SpatialElement">
			...
			<xsl:with-param
				name="count"
				select="count(descendant::spa:SpatialElement)"
			/>
		</xsl:apply-templates>
		<xsl:variable name="minX">
			<xsl:for-each select="descendant::spa:Ordinate/@X">
	...
	<xsl:template match="spa:SpatialElement">
		...
		<xsl:variable
			name="itemElement"
			select="$count - count(following-sibling::spa:SpatialElement) - 1"
		/>
		...
		<xsl:apply-templates select="descendant::spa:SpelementUnit">
			...
			<xsl:with-param
				name="count"
				select="count(descendant::spa:SpelementUnit)"
			/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="spa:SpelementUnit">
		...
		<xsl:variable
			name="itemUnit"
			select="$count - count(following-sibling::spa:SpelementUnit)"
		/>
		<xsl:text>Coords[</xsl:text>
		<xsl:value-of select="$itemEntity" />
		<xsl:text>][</xsl:text>
			...
			<xsl:when test="contains($Text, $Old)">
				...
				<xsl:call-template name="Replace">
					<xsl:with-param name="Text">
						<xsl:choose>
							<xsl:when
								test="string-length($Before) > 0 and $Before = substring-before($Text, $New)"
							>
