...
>
	...
	<xsl:variable name="requestEGRNDataAction" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction"/>
	<xsl:variable name="requestAInfo" select="//tns:EGRNRequest/tns:requestDetails/tns:requestAnalyticalInformation"/>
	<!-- Земельный участок -->
	...
	<!-- Заявители -->
	<xsl:variable name="operson" select="//tns:EGRNRequest/tns:declarant/subj:person"/>
	<xsl:variable name="opersonPrevileg" select="//tns:EGRNRequest/tns:declarant/subj:previligedPerson"/>
	<xsl:variable name="oorganization" select="//tns:EGRNRequest/tns:declarant/subj:organization"/>
	<xsl:variable name="oother" select="//tns:EGRNRequest/tns:declarant/subj:other"/>
	<!-- Представители 1го уровня -->
	<xsl:variable name="orperson" select="tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:person"/>
	<xsl:variable name="orpersonPrevileg" select="tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:previligedPerson"/>
	<xsl:variable name="ororganization" select="//tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:organization"/>
	<xsl:variable name="orother" select="//tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:other"/>
	<!-- Представитель 2го уровня. (Предс-ль правообладателя  - ЮЛ1 и представитель этого ЮЛ1 - ФЛ)-->
	<xsl:variable name="ororganizationPer" select="//tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:representative/subj:subject/subj:person"/>
	<xsl:variable name="ororganizationPerPrevileg" select="//tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:representative/subj:subject/subj:previligedPerson"/>
	<!-- ===== Новое формирование представителей ===== -->
	<xsl:variable name="tempRepresent" select="(//tns:EGRNRequest/descendant::subj:representative/subj:subject)"/>
	<xsl:variable name="tempRepFiltred" select="$tempRepresent/self::node()[not(@_id=preceding::subj:subject/@_id)]"/>
	<!-- Разное -->
	<xsl:variable name="appDocs" select="//tns:EGRNRequest/tns:header/stCom:appliedDocument"/>
	...
	<!-- Блок для подсчета страниц, документов и копий -->
		...
		<!--xsl:if test="$appDocs"-->
			...
			<xsl:variable name="nameOrganInner" select="tns:EGRNRequest/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:regRightAuthority"/>
...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
		...
		<div class="table">
			<div class="row">
				<div class="cell lh-middle center w260 wrap">
					<div class="ul center bottompad5">1. Запрос о предоставлении аналитической информации, полученной на основе сведений, содержащихся в Едином государственном реестре недвижимости</div>
				...
				<div class="cell title w380">
					<div class="sidepad wrap">
						...
						<div class="lh-small">
							...
							<xsl:if test="normalize-space($userFIO) and normalize-space(//tns:EGRNRequest/@_id)">
							...
							<xsl:if test="not(normalize-space($userFIO)) or not(normalize-space(//tns:EGRNRequest/@_id))">
								<span>__________________&nbsp;&nbsp;_________________</span>
							</xsl:if>
						</div>
						<div class="">
							<xsl:if test="$typeOrgan = 'MFC'">
								<xsl:if test="normalize-space($creationDate) and normalize-space(//tns:EGRNRequest/@_id)">
									<span>2.5 дата </span>
									<span class="inlineblock bottomborder sidepad notoppad lh-small bold">
										<xsl:call-template name="DateStr">
											<xsl:with-param name="dateStr" select="$creationDate"/>
								...
								<xsl:if test="not(normalize-space($creationDate)) or not(normalize-space(//tns:EGRNRequest/@_id))">
									<span>2.5 дата &laquo;_____&raquo; ___________________ _______ г., время ______ ч., ______ мин.</span>
								</xsl:if>
							</xsl:if>	
							<xsl:if test="$typeOrgan = 'PVD'">
								<xsl:if test="normalize-space($when) and normalize-space(//tns:EGRNRequest/@_id)">
									<span>2.5 дата </span>
									<span class="inlineblock bottomborder sidepad notoppad lh-small bold">
										<xsl:call-template name="DateStr">
								...
								<xsl:if test="not(normalize-space($when)) or not(normalize-space(//tns:EGRNRequest/@_id))">
									<span>2.5 дата &laquo;_____&raquo; ___________________ _______ г., время ______ ч., ______ мин.</span>
								</xsl:if>
							</xsl:if>	
						</div>
	...
	<xsl:template name="point1_1">
		<xsl:variable name="AInfo" select="$requestAInfo/analytic:analyticalInformation"/>
		<xsl:if test="$AInfo">
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad">Прошу предоставить аналитическую информацию: </div>
							</div>
							<xsl:choose>
								<xsl:when test="$AInfo/analytic:lease">
									<div class="row">
										<div class="table tablemargin">
											<div class="row">
												...
												<div class="cell leftpad ">
													<xsl:text>об аренде</xsl:text>
												</div>
											</div>
										</div>
									</div>
								</xsl:when>
								<xsl:when test="$AInfo/analytic:mortgage">
									<div class="row">
										<div class="table tablemargin">
											<div class="row">
												<div class="cell center w40 bold">
												...
												<div class="cell leftpad ">
													<xsl:text>об ипотеке</xsl:text>
												</div>
											</div>
										</div>
									</div>
								</xsl:when>
								<xsl:when test="$AInfo/analytic:registrationActionsObject">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell center w40 bold">V</div>
											<div class="table tablemargin">
												<div class="cell leftpad wrap">
													<xsl:text>об осуществлении регистрационных действий с:</xsl:text>
												</div>
												<div class="row">
													<div class="table tablemargin">
														<div class="cell center w40 bold">V</div>
														<div class="cell leftpad wrap ">
															<xsl:choose>
																<xsl:when test="$AInfo/analytic:registrationActionsObject/analytic:objectPurposeResidential">
																	<xsl:text>объектами недвижимости жилого назначения</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:registrationActionsObject/analytic:objectPurposeUninhabited">
																	<xsl:text>объектами недвижимости нежилого назначения</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:registrationActionsObject/analytic:underConstruction">
																	<xsl:text>объектами незавершенного строительства</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:registrationActionsObject/analytic:propertyComplex">
																	<xsl:text>предприятиями как имущественными комплексами</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:registrationActionsObject/analytic:unifiedRealEstateComplex">
																	<xsl:text>едиными недвижимыми комплексами</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:registrationActionsObject/analytic:land">
																	<xsl:text>земельными участками</xsl:text>
																</xsl:when>
															</xsl:choose>					
														</div>
													</div>
								...
								<xsl:when test="$AInfo/analytic:numberAreaFormedLand">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell center w40 bold">V</div>
											<div class="table tablemargin">
												...
												<div class="row">
													<div class="table tablemargin">
														...
														<div class="cell leftpad wrap ">
															<xsl:choose>
																<xsl:when test="$AInfo/analytic:numberAreaFormedLand/analytic:surveyProjectTerritory">
																	<xsl:text>в соответствии с утвержденным проектом межевания территории</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:numberAreaFormedLand/analytic:surveyProjectLand">
																	<xsl:text>в соответствии с проектом межевания земельного участка или земельных участков</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:numberAreaFormedLand/analytic:schemaParcelsInBlock">
																	<xsl:text>в соответствии со схемой расположения земельного участка или земельных участков на кадастровом плане территории</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:numberAreaFormedLand/analytic:forestParcel">
																	<xsl:text>в соответствии с проектной документацией лесных участков</xsl:text>
																</xsl:when>
																<xsl:when test="$AInfo/analytic:numberAreaFormedLand/analytic:schemaParcelsOnPKK">
																	<xsl:text>в соответствии со схемой размещения земельного участка на публичной кадастровой карте</xsl:text>
																</xsl:when>
															</xsl:choose>					
														</div>
													</div>
								...
								<xsl:when test="$AInfo/analytic:numberAreaLand">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell center w40 bold">V</div>
											<div class="table tablemargin">
												<div class="cell leftpad wrap">
													<xsl:text>об общем количестве и площади земельных участков, относящихся к:</xsl:text>
												</div>
												<div class="row">
													<div class="table tablemargin">
														<div class="cell center w40 bold">V</div>
														<div class="cell leftpad wrap ">
															<xsl:choose>
																<xsl:when test="$AInfo/analytic:numberAreaLand/analytic:categories">
																	<xsl:text>категории (категориям) земель: </xsl:text>
																	<xsl:for-each select="$AInfo/analytic:numberAreaLand/analytic:categories/analytic:category">
																		<span class="bold">
																...
																<xsl:when test="$AInfo/analytic:numberAreaLand/analytic:landPermittedUses">
																	<xsl:text>виду (видам) разрешенного использования: </xsl:text>
																	<xsl:for-each select="$AInfo/analytic:numberAreaLand/analytic:landPermittedUses/analytic:landPermittedUse">
																		<span class="bold">
																			<xsl:if test="analytic:landUse">
																				<xsl:call-template name="usageTypeTemplate">
																					<xsl:with-param name="usageType" select="analytic:landUse"/>
																				</xsl:call-template>
																			</xsl:if>
																			<xsl:if test="analytic:permittedUseText">
																				<xsl:value-of select="analytic:permittedUseText"/>
																			</xsl:if>
																			<xsl:if test="analytic:utilization">
																				<xsl:call-template name="utilizationTemplate">
																					<xsl:with-param name="utilization" select="analytic:utilization"/>
																				</xsl:call-template>
																			</xsl:if>
																			<xsl:if test="position() != last()">
																				<xsl:text>, </xsl:text>
																...
																<xsl:when test="$AInfo/analytic:numberAreaLand/analytic:categoryAndPermittedUse">

																	<xsl:text>категории земель </xsl:text>
																	<span class="bold">
																		<xsl:call-template name="landCategoryTemplate">
																			<xsl:with-param name="landCategory" select="$AInfo/analytic:numberAreaLand/analytic:categoryAndPermittedUse/analytic:category"/>
																		</xsl:call-template>
																	</span>

																	<xsl:text>&nbsp; и виду разрешенного использования </xsl:text>
																	<xsl:for-each select="$AInfo/analytic:numberAreaLand/analytic:categoryAndPermittedUse/analytic:landPermittedUse">
																		<span class="bold">
																			<xsl:if test="analytic:landUse">
																				<xsl:call-template name="usageTypeTemplate">
																					<xsl:with-param name="usageType" select="analytic:landUse"/>
																...
																<xsl:when test="$AInfo/analytic:numberAreaLand/analytic:transferCategory">
																	<xsl:text>переведенных из категории земель </xsl:text>
																	<span class="bold">
																		<xsl:call-template name="landCategoryTemplate">
																			<xsl:with-param name="landCategory" select="$AInfo/analytic:numberAreaLand/analytic:transferCategory/analytic:categoryOld"/>
																		</xsl:call-template>
																	</span>
																	<xsl:text>&nbsp;в категорию земель </xsl:text>
																	<span class="bold">
																		<xsl:call-template name="landCategoryTemplate">
																			<xsl:with-param name="landCategory" select="$AInfo/analytic:numberAreaLand/analytic:transferCategory/analytic:categoryNew"/>
																		</xsl:call-template>
																	</span>
																</xsl:when>
															</xsl:choose>					
								...
								<xsl:when test="$AInfo/analytic:objectCost">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell center w40 bold">V</div>
											<div class="table tablemargin">
												<div class="cell leftpad wrap">
													<xsl:text>об объектах недвижимости в разрезе видов объектов недвижимости и кадастровой стоимости (большей, меньшей или равной заданному значению): </xsl:text>
												</div>
												<xsl:if test="$AInfo/analytic:objectCost/analytic:objectTypeCode">
													<div class="row">
														<div class="table tablemargin">
															<div class="cell center w40 bold">V</div>
															<div class="cell leftpad wrap ">
																	...
																	<span class="bold">
																		<xsl:call-template name="objectTypeCodeTemplate">
																			<xsl:with-param name="code" select="$AInfo/analytic:objectCost/analytic:objectTypeCode"/>
																		</xsl:call-template>
																	</span>	
															</div>
														</div>
												...
												<xsl:if test="$AInfo/analytic:objectCost/analytic:cost">
													<div class="row">
														<div class="table tablemargin">
															<div class="cell center w40 bold">V</div>
															<div class="cell leftpad wrap ">
																	...
																	<span class="bold">
																		<xsl:choose>
																			<xsl:when test="$AInfo/analytic:objectCost/analytic:cost/analytic:valueDimensions">
																				<xsl:call-template name="ValuesBase">
																					<xsl:with-param name="pRoot" select="$AInfo/analytic:objectCost/analytic:cost/analytic:valueDimensions"/>
																				</xsl:call-template>
																			</xsl:when>
																			<xsl:when test="$AInfo/analytic:objectCost/analytic:cost/analytic:value">
																				<xsl:value-of select="$AInfo/analytic:objectCost/analytic:cost/analytic:value"/>
																			</xsl:when>
																		</xsl:choose>	
																	</span>	
															</div>
								...
								<xsl:when test="$AInfo/analytic:physicalProperties">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell center w40 bold">V</div>
											<div class="table tablemargin">
												<div class="cell leftpad wrap">
													<xsl:text>об общем количестве и площади (основной характеристике) земельных участков, зданий, сооружений, в отношении которых в Едином государственном реестре недвижимости отсутствуют сведения о категории земель, виде разрешенного использования, назначении:</xsl:text>
												</div>
												<xsl:if test="$AInfo/analytic:physicalProperties/analytic:objectTypeCode">
													<div class="row">
														<div class="table tablemargin">
															<div class="cell center w40 bold">V</div>
															<div class="cell leftpad wrap ">
																	...
																	<span class="bold">
																		<xsl:call-template name="objectTypeCodeTemplate">
																			<xsl:with-param name="code" select="$AInfo/analytic:physicalProperties/analytic:objectTypeCode"/>
																		</xsl:call-template>
																	</span>	
															</div>
														</div>
												...
												<div class="row">
													<div class="table tablemargin">
														<div class="cell center w40 bold">V</div>
														<div class="cell leftpad wrap ">
																<xsl:text>характеристика, отсутствующая в Едином государственном реестре недвижимости: </xsl:text>
																<span class="bold">
																	<xsl:choose>
																		<xsl:when test="$AInfo/analytic:physicalProperties/analytic:category">
																			<xsl:text>сведения о категории земель</xsl:text>
																		</xsl:when>
																		<xsl:when test="$AInfo/analytic:physicalProperties/analytic:permittedUses">
																			<xsl:text>сведения о виде разрешенного использования</xsl:text>
																		</xsl:when>
																		<xsl:when test="$AInfo/analytic:physicalProperties/analytic:purpose">
																			<xsl:text>сведения о назначении</xsl:text>
																		</xsl:when>
																	</xsl:choose>	
																</span>	
														</div>
								...
								<xsl:when test="$AInfo/analytic:cadWorks">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell center w40 bold">V</div>
											<div class="table tablemargin">
												<div class="cell leftpad wrap">
													<xsl:text>о кадастровых инженерах с указанием видов выполняемых работ и результатов профессиональной деятельности:</xsl:text>
												</div>
												<xsl:if test="$AInfo/analytic:cadWorks/analytic:workType">
													<div class="row">
														<div class="table tablemargin">
															<div class="cell center w40 bold">V</div>
															<div class="cell leftpad wrap ">
																	<xsl:text>вид выполняемых работ: </xsl:text>
																	<span class="bold">
																		<xsl:value-of select="$AInfo/analytic:cadWorks/analytic:workType"/>
																	</span>	
															</div>
														</div>
													</div>
												</xsl:if>
												<xsl:if test="$AInfo/analytic:cadWorks/analytic:resultCadWorks">
													<div class="row">
														<div class="table tablemargin">
															<div class="cell center w40 bold">V</div>
															<div class="cell leftpad wrap ">
																	<xsl:text>результаты профессиональной деятельности: </xsl:text>
																	<span class="bold">
																		<xsl:choose>
																			<xsl:when test="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionActions">
																				<xsl:text>Количество решений об осуществлении государственного кадастрового учета (в том числе кадастрового учета и государственной регистрации прав) </xsl:text>
																				<xsl:call-template name="ValuesBase">
																					<xsl:with-param name="pRoot" select="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionActions"/>
																				</xsl:call-template>
																			
																			</xsl:when>
																			<xsl:when test="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionToSuspend">
																				<xsl:text>Количество решений о приостановлении осуществления государственного кадастрового учета (в том числе кадастрового учета и государственной регистрации прав </xsl:text>
																				<xsl:call-template name="ValuesBase">
																					<xsl:with-param name="pRoot" select="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionToSuspend"/>
																				</xsl:call-template>
																			</xsl:when>
																			<xsl:when test="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionToRefuse">
																				<xsl:text>Количество решений об отказе в осуществлении государственного кадастрового учета (в том числе кадастрового учета и государственной регистрации прав) </xsl:text>
																				<xsl:call-template name="ValuesBase">
																					<xsl:with-param name="pRoot" select="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionToRefuse"/>
																				</xsl:call-template>
																			</xsl:when>
																			<xsl:when test="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionErrorRegistry">
																				<xsl:text>Количество решений о необходимости устранения реестровых ошибок в сведениях ЕГРН, связанных с ошибками, допущенными кадастровыми инженерами при подготовке межевых планов, технических планов, актов обследования, карты-плана территории </xsl:text>
																				<xsl:call-template name="ValuesBase">
																					<xsl:with-param name="pRoot" select="$AInfo/analytic:cadWorks/analytic:resultCadWorks/analytic:decisionErrorRegistry"/>
																				</xsl:call-template>
																			</xsl:when>
																		</xsl:choose>	
																	</span>	
		...
		<xsl:variable name="territoryType" select="$requestAInfo/analytic:territory"/>
		<xsl:if test="$territoryType">
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad">В отношении территории:</div>
							</div>
							<div class="row">
								<div class="table tablemargin">
									<div class="cell center w40">
									...
									<div class="cell leftpad ">
										<xsl:choose>
											<xsl:when test="$territoryType/analytic:territoryRussia">
												<xsl:text>Российской Федерации</xsl:text>
											</xsl:when>
											<xsl:when test="$territoryType/analytic:region">
												<xsl:text>субъекта Российской Федерации </xsl:text>
												<xsl:call-template name="regionCodeTemplate">
													<xsl:with-param name="regionCode" select="$territoryType/analytic:region"/>
												</xsl:call-template>
											</xsl:when>
											<xsl:when test="$territoryType/analytic:city">
												<xsl:text>муниципального образования </xsl:text>
												<xsl:call-template name="createAddressTemplate">
													<xsl:with-param name="pathToAddress" select="$territoryType/analytic:city"/>
												</xsl:call-template>
											</xsl:when>
											<xsl:when test="$territoryType/analytic:locality">
												<xsl:text>населенного пункта </xsl:text>
												<xsl:call-template name="createAddressTemplate">
													<xsl:with-param name="pathToAddress" select="$territoryType/analytic:locality"/>
												</xsl:call-template>
											</xsl:when>
											<xsl:when test="$territoryType/analytic:cadastralRegion">
												<xsl:text>кадастрового округа </xsl:text>
												<xsl:value-of select="$territoryType/analytic:cadastralRegion"/>
											</xsl:when>
											<xsl:when test="$territoryType/analytic:cadastralDistricts">
												<xsl:text>кадастрового района </xsl:text>
												<xsl:for-each select="$territoryType/analytic:cadastralDistricts/analytic:cadastralDistrict">
													<xsl:value-of select="."/>
													<xsl:if test="position() != last()">
														<xsl:text>, </xsl:text>
													</xsl:if>
											...
											<xsl:when test="$territoryType/analytic:cadastralBlocks">
												<xsl:text>кадастрового квартала </xsl:text>
												<xsl:for-each select="$territoryType/analytic:cadastralBlocks/analytic:cadastralBlock">
													<xsl:value-of select="."/>
													<xsl:if test="position() != last()">
														<xsl:text>, </xsl:text>
													</xsl:if>
		...
		<!-- За период  -->
		<xsl:variable name="periodType" select="$requestAInfo/analytic:period"/>
						...
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad">
									...
									<br/>
									<xsl:choose>
										<xsl:when test="$periodType/analytic:period = '012003004000' ">
											<xsl:value-of select="$periodType/analytic:Month"/>
											<xsl:text>&nbsp; месяц,</xsl:text>
											<br/>
										</xsl:when>
										<xsl:when test="$periodType/analytic:period = '012003007000' ">
											<xsl:value-of select="$periodType/analytic:Quarter"/>
											<xsl:text>&nbsp; квартал,</xsl:text>
											<br/>
										</xsl:when>
										<xsl:when test="$periodType/analytic:period = '012003008000' ">
											<xsl:value-of select="$periodType/analytic:Half"/>
											<xsl:text>&nbsp; полугодие,</xsl:text>
											<br/>
										</xsl:when>
										<!--xsl:when test="$periodType/analytic:period = '012003005000' ">
										</xsl:when-->
									</xsl:choose>					
									<xsl:value-of select="$periodType/analytic:Year"/>
	...
	<xsl:template name="point1_2">
		<xsl:variable name="dataReceiveForm" select="//tns:EGRNRequest/tns:deliveryDetails/stCom:resultDeliveryMethod/stCom:dataReceiveForm"/>
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad">Форма предоставления аналитической информации:</div>
							</div>
	...
	<xsl:template name="point1_3">
		<xsl:variable name="receiveMethod" select="//tns:EGRNRequest/tns:deliveryDetails/stCom:resultDeliveryMethod/stCom:recieveResultTypeCode"/>
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap">Способ получения аналитической информации:</div>
							</div>
								<div class="row">
									<div class="table tablemargin">
										<div class="cell center w40">
										...
										<xsl:if test="$receiveMethod = 'postMail'">
											<div class="cell leftpad wrap">
												...
												<xsl:if test="//tns:EGRNRequest/tns:deliveryDetails/stCom:resultDeliveryMethod/stCom:postAddress">
													<span class="bold">
														<xsl:call-template name="createAddressTemplate">
															<xsl:with-param name="pathToAddress" select="//tns:EGRNRequest/tns:deliveryDetails/stCom:resultDeliveryMethod/stCom:postAddress"/>
														</xsl:call-template>
													</span>
												</xsl:if>
											</div>
										...
										<xsl:if test="$receiveMethod = 'eMail'">
											<div class="cell leftpad wrap">
												...
												<xsl:if test="//tns:EGRNRequest/tns:deliveryDetails/stCom:resultDeliveryMethod/stCom:eMail">
													<span class="bold">
														<xsl:value-of select="//tns:EGRNRequest/tns:deliveryDetails/stCom:resultDeliveryMethod/stCom:eMail"/>
													</span>
												</xsl:if>
											</div>
										</xsl:if>
	...
	<xsl:template name="point31person">
		...
		<xsl:param name="mode1" select="'declarant'"/>
		<xsl:variable name="subjDoc" select="//tns:EGRNRequest/tns:header/stCom:appliedDocument/node()[@_id=$thisPerson/subj:idDocumentRef/@documentID]"/>
					...
					<xsl:if test="$thisPerson/subj:previousData/subj:idDocumentRef">
						...
						<!--span class="bold"-->
							<xsl:for-each select="$thisPerson/subj:previousData/subj:idDocumentRef">
								...
								<xsl:call-template name="documentTemplate">
									<xsl:with-param name="thisDocument" select="//tns:EGRNRequest/tns:header/stCom:appliedDocument/node()[@_id=$prevDocId]"/>
									<xsl:with-param name="modeIdDoc" select="'idDoc'"/>
								</xsl:call-template>
								<xsl:if test="position() != last()">
									<xsl:text>; </xsl:text>
	...
	<xsl:template name="point32organization">
		...
		<xsl:param name="mode1" select="'declarant'"/>
			...
			<xsl:if test="$thisOrganization/subj:name | $thisOrganization/tns:Name | $thisOrganization/subj:Name">
				<span class="bold">
					<xsl:value-of select="$thisOrganization/subj:name | $thisOrganization/tns:Name | $thisOrganization/subj:Name"/>
			...
			<xsl:if test="$thisOrganization/subj:nativeForeignParams/subj:foreignOrgParams/subj:countryCode">
				<br/>
				<xsl:text>страна регистрации (инкорпорации), код иностранной организации (КИО) </xsl:text>
						<span class="bold">
	...
	<!--Раздел 2-->
	<!-- выводятся сначала все Физ лица, потом ЮЛ, потом все представители-->
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
									<div class="cell leftpad  wrap">
										<xsl:text>Сведения о заявителе (физическом лице, арбитражном управляющем, нотариусе, судебном приставе-исполнителе):</xsl:text>
									</div>
								</div>
							</div>
							<div class="row">
								<div class="table tablemargin">
									<xsl:if test="$operson | $opersonPrevileg">
										<xsl:call-template name="point311">
											<xsl:with-param name="curPerson" select="$operson | $opersonPrevileg"/>
										</xsl:call-template>
									</xsl:if>
								</div>
							</div>
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
									<div class="cell leftpad  wrap">
										<xsl:text>Сведения о заявителе (юридическом лице, органе государственной власти, органе местного самоуправления, ином органе):</xsl:text>
									</div>
								</div>
							</div>
							<div class="row">
								<div class="table tablemargin">
									<xsl:if test="$oorganization |  $oother">
										<xsl:call-template name="point312">
											<xsl:with-param name="curPerson" select="$oorganization |  $oother"/>
										</xsl:call-template>
									</xsl:if>
								</div>
							</div>
	...
	<!--Раздел 3.5 представители. Если надо парами - см. расписку.-->
	...
	<xsl:template name="point5">
		<xsl:if test="//tns:EGRNRequest/tns:statementAgreements/stCom:qualityOfServiceTelephoneNumber">
			<div class="table">
				<div class="row">
								...
								<div class="table tablemargin">
									...
									<div class="cell left wrap">
										<xsl:call-template name="agreementData">
											<xsl:with-param name="agreementCode" select="//tns:EGRNRequest/tns:statementAgreements/stCom:qualityOfServiceAgreement"/>
											<xsl:with-param name="typeName" select="'DQualityOfServiceAgreements'"/>
										</xsl:call-template>
										<xsl:if test="//tns:EGRNRequest/tns:statementAgreements/stCom:qualityOfServiceTelephoneNumber">	
											<xsl:text>: </xsl:text>
											<span class="bold">
												<xsl:value-of select="//tns:EGRNRequest/tns:statementAgreements/stCom:qualityOfServiceTelephoneNumber"/>
											</span>
										</xsl:if>	
									</div>
								</div>
						...
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
									...
									<div class="cell left wrap">
										<xsl:call-template name="agreementData">
											<xsl:with-param name="agreementCode" select="//tns:EGRNRequest/tns:statementAgreements/stCom:persDataProcessingAgreement"/>
											<xsl:with-param name="typeName" select="'DPersDataAgreements'"/>
										</xsl:call-template>
									</div>
								</div>
							</div>
		...
		<!--xsl:if test="//tns:EGRNRequest/tns:statementAgreements"-->
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<!--xsl:if test="//tns:EGRNRequest/tns:statementAgreements/stCom:actualDataAgreement"-->
								<div class="row">
									<div class="table tablemargin">
										<div class="cell left wrap">
											<xsl:text>Подпись и иная информация: </xsl:text>
											<br/>
											<xsl:text>&nbsp;&nbsp;&nbsp;&nbsp; сведения, включенные в запрос, относящиеся к моей личности, а также внесенные мною ниже, достоверны;</xsl:text>
											<br/>
											<xsl:text>&nbsp;&nbsp;&nbsp;&nbsp; документы (копии документов), приложенные к запросу, соответствуют требованиям, установленным законодательством Российской Федерации, на момент представления запроса эти документы действительны и содержат достоверные сведения.</xsl:text>
											
											<!--xsl:call-template name="agreementData">
												<xsl:with-param name="agreementCode" select="//tns:EGRNRequest/tns:statementAgreements/stCom:actualDataAgreement"/>
												<xsl:with-param name="typeName" select="'DActualDataAgreements'"/>
											</xsl:call-template>
											<br/>
											<xsl:if test="normalize-space(//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:requestCause)">
												<span class="bold bottomborder">
													<xsl:value-of select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:requestCause"/>
													<xsl:if test="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:serviceName">, </xsl:if>
												</span>
											</xsl:if>
												<span class="bold bottomborder">
													<xsl:value-of select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:serviceName"/>
												</span>
											<br/>	
											<span class="sidepad">
												<xsl:text>(основание запроса сведений, в том числе наименование государственной или муниципальной услуги или базового государственного информационного ресурса)</xsl:text>
											</span-->
										</div>
									</div>
								</div>
				...
				<div class="cell clear">
					<div class="table tablemargin">
						<div class="row">
							<div class="table tablemargin">
								<div class="cell leftpad wrap">
									<xsl:text>Отметка принявшего запрос специалиста (или заполняется автоматически при представлении запроса) </xsl:text>
									<span class="bold">
										<xsl:value-of select="//tns:EGRNRequest/tns:deliveryDetails/stCom:requestDeliveryMethod/stCom:specialistNote"/>
									</span>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="table tablemargin">
								<div class="cell leftpad wrap">
									<br/>
									<xsl:text>____________________________________________&nbsp;&nbsp;&nbsp;&nbsp;_____________&nbsp;&nbsp;&nbsp;&nbsp;_____________________________</xsl:text>
									<br/>
									<xsl:text>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(должность) &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (подпись)  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  (инициалы, фамилия)</xsl:text>
								</div>
							</div>
						</div>
					</div>
	...
	<xsl:template name="point9">
		<xsl:variable name="note9" select="//tns:EGRNRequest/tns:requestDetails/tns:note/com:text"/>
		<xsl:if test="$note9">
			<div class="table">
				<div class="row">
	...
	<xsl:template name="regionCodeTemplate">
		...
		<xsl:if test="$regionCode = 98">Неопределено</xsl:if>
		<xsl:if test="$regionCode = 99">Иные территории, включая город и космодром Байконур</xsl:if>
...
<xsl:value-of select="($thisDocument/doc:series)"/-->
	...
	<xsl:template name="utilizationTemplate">
		<xsl:param name="utilization"/>
		<xsl:variable name="dictPath" select="'Dictionary/dUtilizations_v01.xsd'" />
		<xsl:variable name="utilTypeEnum" select="document($dictPath)"/>
		<xsl:value-of select="$utilTypeEnum//xs:enumeration[@value=$utilization]/xs:annotation/xs:documentation"/>
	</xsl:template>
	...
	<xsl:template name="ValuesBase">
        <xsl:param name="pRoot"/>
			<xsl:choose>
			<xsl:when test="$pRoot/analytic:moreThan">
			<xsl:text>более, чем </xsl:text>
			<xsl:value-of select="$pRoot/analytic:moreThan"/>
			</xsl:when>
			<xsl:when test="$pRoot/analytic:lessThan">
			<xsl:text>менее, чем </xsl:text>
			<xsl:value-of select="$pRoot/analytic:lessThan"/>
			</xsl:when>
			<xsl:when test="$pRoot/analytic:valueRange">
				<xsl:text>от </xsl:text>
				<xsl:value-of select="$pRoot/analytic:valueRange/analytic:valueFrom"/>
				<xsl:text>&nbsp;до </xsl:text>
				<xsl:value-of select="$pRoot/analytic:valueRange/analytic:valueTo"/>
			</xsl:when>
			</xsl:choose>	
	</xsl:template>
