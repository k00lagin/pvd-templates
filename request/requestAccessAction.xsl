...
xmlns:doc="http://rosreestr.ru/services/v0.26/commons/Documents">
	...
	<xsl:variable name="requestEGRNAccessAction" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNAccessAction"/>
	<!-- Земельный участок -->
	...
	<!-- Правообладатели -->
	<xsl:variable name="rightOwner" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNAccessAction/tns:requiredDataType/tns:incapacityPersonData"/>
	<!-- Заявители -->
...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
			...
			<!--body-->
				...
				<div id="wrapper">
					<section>
						...
						<xsl:call-template name="point4_1"/>
						<xsl:call-template name="point4_2"/>
	...
	<xsl:template name="point3">
		<xsl:if test="$requestEGRNAccessAction">
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap bold">Прошу предоставить сведения, содержащиеся в Едином государственном реестре недвижимости, посредством обеспечения доступа к федеральной государственой информационной системе ведения Единого государственного реестра недвижимости:</div>
							</div>
								<!--xsl:variable name="dataType" select="$requestEGRNAccessAction/tns:requiredDataType/@dataType"/-->
							<!--/xsl:if-->	
	...
	<xsl:template name="point3_1">
		<xsl:if test="$requestEGRNAccessAction">
			<!--xsl:variable name="notification" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNAccessAction/tns:notification"/-->
				...
				<div class="table">
					<div class="row">
						...
						<div class="cell clear">
							<div class="table tablemargin">
								<div class="row">
									...
									<div class="cell clear">
										<div class="table tablemargin">
											<div class="row">
												<div class="cell leftpad wrap bold">Правообладатель:</div>
											</div>
												<!--xsl:for-each select="$requestEGRNAccessAction/tns:requiredDataType/tns:subjectData">
													<div class="row">
														<div class="table tablemargin">
															<div class="row">
																<div class="cell center w40 bold">V</div>
																<div class="cell clear">
																	<div class="table tablemargin">
																		<div class="row">
																			<xsl:variable name="subjectData" select="."/>
																			<xsl:if test="$subjectData/tns:organization | $subjectData/tns:country | $subjectData/tns:rfSubject | $subjectData/tns:city">
																				<div class="cell leftpad wrap">юридическое лицо, в том числе орган государственной власти, орган местного самоуправления, публично правовое образование:</div>
																			</xsl:if>	
																			<xsl:if test="$subjectData/tns:person">
																				<div class="cell leftpad wrap">физическое лицо:</div>
																			</xsl:if>	
							...
							<xsl:for-each select="$requestEGRNAccessAction/tns:requiredDataType/tns:subjectData">
								<!-- TODO пока не изспольуем. Ждем разъяснений. xsl:variable name="incapacityData" select="tns:incapacityPersonData"/-->
								<xsl:variable name="subjectData" select="."/>
									<div class="table tablemargin">
											<div class="row">
												<div class="cell center w65 bold">V</div>
														<div class="table tablemargin">
														<div class="row">
									...
									<div class="table tablemargin">
											<xsl:if test="$subjectData/tns:organization | $subjectData/tns:country | $subjectData/tns:rfSubject | $subjectData/tns:city">
												...
												<xsl:call-template name="point312">
													<xsl:with-param name="curPerson" select="$subjectData/tns:organization | $subjectData/tns:country | $subjectData/tns:rfSubject | $subjectData/tns:city"/>
												</xsl:call-template>
											</xsl:if>	
											<!--xsl:if test="$subjectData/tns:person | $incapacityData"-->
											<xsl:if test="$subjectData/tns:person">
												<!--div class="cell leftpad wrap">физическое лицо:</div-->
												<xsl:call-template name="point311">
													<!--xsl:with-param name="curPerson" select="$subjectData/tns:person | $incapacityData"/-->
													<xsl:with-param name="curPerson" select="$subjectData/tns:person"/>
												</xsl:call-template>
											</xsl:if>	
									</div>	
							</xsl:for-each>
	...
	<xsl:template name="point31person">
		...
		<br/>
		<xsl:value-of select="$thisPerson/subj:idDocumentRef/@documentID"/>
		<br/-->
			...
			<xsl:if test="$mode1 = 'owner'">
				<xsl:if test="$thisPerson/subj:previousData">
					<xsl:if test="$thisPerson/subj:previousData/subj:FIO">
						<br/>
						<xsl:text>предыдущие фамилия и/или имя, и/или отчество </xsl:text>
						<span class="bold">
	...
	<xsl:template name="point32organization">
		...
		<xsl:param name="mode1" select="'declarant'"/>
			...
			<!-- subjName используется в запросах и ВСБЗ из tns:EGRNRequest/tns:requestDetails/tns:requestEGRNAccessAction/tns:requiredDataType/tns:subjectData/tns:city/subj:Name -->
			...
			<xsl:if test="$mode1 != 'owner'">
				...
				<!-- Документ, подтв. полномочия ЮЛ-->
				<xsl:if test="$thisOrganization/subj:regDocument">
					<br/>
					<xsl:text>документ, подтверждающий полномочия юридического лица: </xsl:text>
                    <span class="bold">
					<xsl:call-template name="showDocumentData">
						<xsl:with-param name="thisDocument" select="$thisOrganization/subj:regDocument"/>
					</xsl:call-template>
                    </span>
				</xsl:if>
			...
			<!-- Для правообладателей выводим после страны регистрации все даты вместе, какие есть -->
			<xsl:if test="$mode1 = 'owner'">
				<xsl:if test="$thisOrganization/subj:nativeForeignParams/node()/subj:regDate or $thisOrganization/subj:regDate">
					<br/>
					<xsl:text>дата государственной регистрации </xsl:text>
	...
	<xsl:template name="point4_1">
		<xsl:if test="$operson | $opersonPrevileg">
			<div class="table">
				<div class="row">
					<div class="cell center w40 ">4.1</div>
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
	...
	<!--Раздел 4.2-->
	<xsl:template name="point4_2">
		<xsl:if test="$oorganization |  $oother">
			<div class="table">
				<div class="row">
					<div class="cell center w40 ">4.2</div>
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							...
							<div class="row">
								<div class="table tablemargin">
									<xsl:call-template name="point311">
										<!--xsl:with-param name="curPerson" select="$orperson | $orpersonPrevileg"/-->
										<xsl:with-param name="curPerson" select="$tempRepFiltred/subj:person"/>
										<xsl:with-param name="mode1" select="'declarant'"/>
									</xsl:call-template>
								</div>
							</div>
							<!-- Выводим только представителей ФЛ 
								div class="row">
								<div class="table tablemargin">
						...
						<div class="cell clear">
							<div class="table tablemargin">
								...
								<xsl:for-each select="$appDocs/node()">
									<!-- Не отображаем документы с типом "Документ удостоверяющий личность" на который есть ссылка (он уже указан ранее) -->
							...
							<xsl:if test="//tns:EGRNRequest/tns:statementAgreements/stCom:actualDataAgreement">
                                ...
                                <xsl:if test="$signPerson">
                                    <xsl:call-template name="signTemplate">
                                        ...
                                        <xsl:with-param name="notary" select="$signPerson"/>
                                    </xsl:call-template>
                                </xsl:if>
                                <xsl:if test="not($signPerson)">
                                    <xsl:call-template name="signTemplate">
                                        <xsl:with-param name="doStamp" select="'true'"/>
                                        <xsl:with-param name="notary" select="//tns:EGRNRequest/tns:declarant/subj:person"/>
                                    </xsl:call-template>
                                </xsl:if>
                            </xsl:if>
						</div>
	...
	<xsl:template name="objectTypeCode">
		...
		<xsl:choose>
			...
			<xsl:when test="$objType = $realEstateComplex">
				<xsl:text>имущественный комплекс</xsl:text>
			</xsl:when>
