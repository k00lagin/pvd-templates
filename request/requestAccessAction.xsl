...
>
	...
	<xsl:variable
		name="requestEGRNAccessAction"
		select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNAccessAction"
	/>
	<!-- Земельный участок -->
	...
	<xsl:variable
		name="rightOwner"
		select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNAccessAction/tns:requiredDataType/tns:incapacityPersonData"
	/>
	<!-- Заявители -->
	<xsl:variable
		name="operson"
...
input[type="reset"],input[type="submit"] {
						...
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad wrap bold">
									Прошу предоставить сведения, содержащиеся в Едином
									государственном реестре недвижимости, посредством обеспечения
									доступа к федеральной государственой информационной системе
			...
			<div class="table">
				<div class="row">
					...
					<div class="cell clear">
						...
						<xsl:for-each
							select="$requestEGRNAccessAction/tns:requiredDataType/tns:subjectData"
						>
									...
									<div class="table tablemargin">
										<div class="row">
											<div class="cell leftpad wrap">
												<xsl:if
													test="$subjectData/tns:organization | $subjectData/tns:country | $subjectData/tns:rfSubject | $subjectData/tns:city"
												>
													<xsl:text
														>юридическое лицо, в том числе орган государственной
							...
							<div class="table tablemargin">
								...
								>
									...
									<xsl:call-template name="point312">
										<xsl:with-param
											name="curPerson"
											select="$subjectData/tns:organization | $subjectData/tns:country | $subjectData/tns:rfSubject | $subjectData/tns:city"
										/>
									</xsl:call-template>
								</xsl:if>
	...
	<xsl:template name="point32organization">
		...
		<xsl:if test="$mode1 != 'owner'">
			...
			<xsl:if test="$thisOrganization/subj:regDocument">
				<br />
				<xsl:text
					>документ, подтверждающий полномочия юридического лица:
				</xsl:text>
				<span class="bold">
					<xsl:call-template name="showDocumentData">
						<xsl:with-param
							name="thisDocument"
							select="$thisOrganization/subj:regDocument"
						/>
					</xsl:call-template>
				</span>
			</xsl:if>
		...
		<xsl:if test="$mode1 = 'owner'">
			<xsl:if
				test="$thisOrganization/subj:nativeForeignParams/node()/subj:regDate or $thisOrganization/subj:regDate"
			>
				<br />
				<xsl:text>дата государственной регистрации </xsl:text>
				<span class="bold">
	...
	<xsl:template name="point7">
		<xsl:if
			test="//tns:EGRNRequest/tns:statementAgreements/stCom:qualityOfServiceAgreement"
		>
			<div class="table">
				<div class="row">
											...
											>
												<span class="nopadding wrap">
													<xsl:text
														...
														Российской Федерации, которым уполномочено
														должностное лицо такого органа:
													</xsl:text>
													<span class="bold">
														<xsl:call-template name="showDocumentData">
															<xsl:with-param
												...
												<span class="sidepad center">
													<div style="font-size: 80%">
														<xsl:text
															...
															реквизиты документа, подтверждающего наделение
															нотариуса полномочиями)</xsl:text
														>
													</div>
												</span>
											</xsl:if>
