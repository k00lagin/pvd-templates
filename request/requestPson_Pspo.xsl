...
xmlns:doc="http://rosreestr.ru/services/v0.26/commons/Documents">
	...
	<!-- Блок для подсчета страниц, документов и копий -->
		...
		<!--xsl:if test="$appDocs"-->
			...
			<xsl:variable name="isDepositary"   select="tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDataAction/tns:isDepositary"/>
				...
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							...
							<xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/node()/tns:object">
								<div class="row">
									<div class="table tablemargin">
										<div class="row">
								...
								<div class="row">
									<div class="table tablemargin">
										...
										<div class="table tablemargin">
											...
											<xsl:for-each select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/node()/tns:object">
												...
												<xsl:if test="obj:cadastralNumber or obj:address or obj:location or obj:notes or obj:physicalProperties">
													<div class="row">
														<div class="cell leftpad wrap">
                                                            ...
                                                            <xsl:if test="obj:location">
                                                                ...
                                                                <span class="bold">
                                                                    <xsl:call-template name="createAddressTemplate">
                                                                        <xsl:with-param name="pathToAddress" select="obj:location"/>
                                                                    </xsl:call-template>
                                                                </span>
                                                                <br/>
                                                            </xsl:if>
                                                            <xsl:variable name="areaProp" select="obj:physicalProperties/obj:property[@type='area']"/>
															<xsl:if test="$areaProp">
																...
																<span class="bold">
																	...
																	<xsl:call-template name="unitTypeTemplate">
																		<xsl:with-param name="unitType" select="$areaProp/obj:unitType"/>
																	</xsl:call-template>
																</span>
                                                                <br/>
															</xsl:if>
															<!--TODO - остальные св-ва ОН если они есть выводить в особые отметки. Пока подразумевается что есть только площадь -->
															<xsl:variable name="otherProps" select="obj:physicalProperties/obj:property[@type!='area']"/>
															<xsl:variable name="objNotes" select="obj:notes"/>
															<xsl:if test="$objNotes">
																<xsl:text>дополнительная информация: </xsl:text>
																<span class="bold">
																	<xsl:for-each select="$objNotes/obj:noteGroup/obj:objectNote">
															...
															<xsl:if test="$otherProps">
																...
																<span class="bold">
																	<xsl:for-each select="$otherProps">
																		<xsl:apply-templates select="."/>
																		<xsl:if test="position() != count($otherProps)">
																			<xsl:text>; </xsl:text>
																		</xsl:if>
																	</xsl:for-each>
																</span>
							...
							<!--Правообладатели -->	
							<xsl:variable name="extractIncapacity" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractIncapacity"/>
								...
								<div class="row">
									<div class="table tablemargin">
										...
										<div class="table tablemargin">
											<!--div class="row">
												<div class="cell leftpad wrap"-->
												<xsl:if test="$extractSubject/tns:owner/tns:organization | $extractSubject/tns:owner/tns:country | $extractSubject/tns:owner/tns:rfSubject | $extractSubject/tns:owner/tns:city">
													<xsl:call-template name="point312">
														<xsl:with-param name="curPerson" select="$extractSubject/tns:owner/tns:organization | $extractSubject/tns:owner/tns:country | $extractSubject/tns:owner/tns:rfSubject | $extractSubject/tns:owner/tns:city"/>
														<xsl:with-param name="mode1" select="'owner'"/>
													</xsl:call-template>
												</xsl:if>
												
												<xsl:if test="$extractSubject/tns:owner/tns:person | $extractIncapacity/tns:incapacityOwner">
													<xsl:call-template name="point311">
														<xsl:with-param name="curPerson" select="$extractSubject/tns:owner/tns:person | $extractIncapacity/tns:incapacityOwner"/>
														<xsl:with-param name="mode1" select="'owner'"/>
													</xsl:call-template>
												</xsl:if>
												<!--/div>
                                    ...
                                    <div class="row">
                                        <div class="table tablemargin">
                                            ...
                                            <div class="cell leftpad wrap">
                                                <span class="bold">
                                                    ...
                                                    <xsl:choose>
                                                        <xsl:when test="$requestType = 'extractRealty'">
                                                            <xsl:text>выписки из Единого государственного реестра недвижимости об объекте недвижимости</xsl:text>
                                                        </xsl:when>
                                                        <xsl:when test="$requestType = 'extractRealtyList'">
                                                            <xsl:text>выписки из Единого государственного реестра недвижимости о переходе прав на объект недвижимости</xsl:text>
                                                        </xsl:when>
                                                        <xsl:when
                                                                test="$requestType = 'extractEquityConstructionContract'">
                                                            <xsl:text>выписки из Единого государственного реестра недвижимости о зарегистрированных договорах участия в долевом строительстве</xsl:text>
                                                        </xsl:when>
                                                        <xsl:when
                                                                test="$requestType = 'extractObjectMainFeaturesRights'">
                                                            <xsl:text>выписки из Единого государственного реестра недвижимости об основных характеристиках и зарегистрированных правах на объект недвижимости</xsl:text>
                                                        </xsl:when>
                                                        <xsl:when test="$requestType = 'extractStatementReceiptDate'">
                                                            <xsl:text>выписки о дате получения органом регистрации прав заявления о государственном кадастровом учете и (или) государственной регистрации прав и прилагаемых к нему документов</xsl:text>
                                                        </xsl:when>
                                                ...
                                                <!-- строка 3 -->
                                                <xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractIncapacity">
                                                    <span class="bold">
                                                        <xsl:text>выписки из Единого государственного реестра недвижимости о признании правообладателя недееспособным или ограниченно дееспособным</xsl:text>
                                                    </span>
                                                </xsl:if>
                                                <!-- строка 4 -->
                                                <!--xsl:variable name="extractSubject" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractSubject"/-->
                                                <xsl:if test="$extractSubject">
                                                    <span class="bold">
                                                        <xsl:text>выписки из Единого государственного реестра недвижимости о правах отдельного лица на имевшиеся (имеющиеся) у него объекты недвижимости</xsl:text>
                                                    </span>
                                                    <xsl:if test="$extractSubject/tns:realtyTypes/tns:realtyTypeAll">
                                                        <br/>
                                                    ...
                                                    <!-- Виды объектов -->
                                                    <xsl:if test="$extractSubject/tns:realtyTypes/tns:realtyType">
                                                        <br/>
                                                        <xsl:if test="count($extractSubject/tns:realtyTypes/tns:realtyType)=1">
                                                    ...
                                                    <xsl:if test="$territory/tns:regions">
                                                        ...
                                                        <xsl:text>на территории </xsl:text>
                                                        <span class="bold">
                                                            <xsl:for-each select="$territory/tns:regions/tns:region">
                                                                <xsl:call-template name="regionCodeTemplate">
                                                                    <xsl:with-param name="regionCode" select="."/>
                                                                </xsl:call-template>
                                                                <xsl:if test="position() != last()">
                                                                    <xsl:text>, </xsl:text>
                                                                </xsl:if>
                                                    ...
                                                    <!-- Период -->
                                                    <xsl:variable name="period" select="$extractSubject/tns:period"/>
                                                    <xsl:if test="$period">
                                                        <br/>
                                                ...
                                                <!-- строка 8 -->
                                                <xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDocument">
                                                    <!--span class="bold"-->
                                                    <xsl:text>выписки о содержании правоустанавливающих документов </xsl:text>
                                                    ...
                                                    <xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDocument/tns:describeContract">
                                                        <xsl:text>, </xsl:text>
                                                        <xsl:value-of select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDocument/tns:describeContract"/>
                                                    </xsl:if>
                                                    <xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDocument/tns:informationTitle/tns:basisTransferRight">
                                                        <xsl:text>, Сведения о правоустанавливающем документе, на основании которого был зарегистрирован переход права на указанный в запросе объект недвижимости от одного лица к другому</xsl:text>
                                                    </xsl:if>
                                                    <xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDocument/tns:informationTitle/tns:basisPurchased">
                                                        <xsl:text>, Сведения о правоустанавливающем документе, на основании которого конкретное лицо, указанное в запросе, приобрело объект недвижимости</xsl:text>
                                                    </xsl:if>
                                                    <xsl:if test="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractDocument/tns:person">
                                                        <xsl:text>, </xsl:text>
                                                        <br/>
                                                        <xsl:call-template name="point31person">
                                                            <xsl:with-param name="thisPerson"
                                                ...
                                                <!-- строка 9 -->
                                                <xsl:variable name="aboutPersons"
                                                              select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:aboutPersons"/>
                                                <xsl:if test="$aboutPersons">
                                                    <span class="bold">
                                                        <xsl:text>справки о лицах, получивших сведения об объекте недвижимого имущества </xsl:text>
                                                    </span>
                                                    <xsl:variable name="periodAbout" select="$aboutPersons/tns:period"/>
                                                    <xsl:if test="$periodAbout">
                                                        <span class="bold">
                                                            <xsl:call-template name="periodTemplate">
                                                                <xsl:with-param name="period" select="$periodAbout"/>
                                                            </xsl:call-template>
                                                        </span>
                                                    </xsl:if>
                                                </xsl:if>
                                                ...
                                                <!-- строка 10 КС -->
                                                <xsl:variable name="cadCost"
                                                              select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:extractKSObject"/>
                                                <xsl:if test="$cadCost">
                                                    <span class="bold">
                                                        <xsl:text>выписки о кадастровой стоимости объекта недвижимости</xsl:text>
                                                        <xsl:if test="$cadCost/tns:data">
                                                            <xsl:text>, применяемой по состоянию на  </xsl:text>
                                                        </xsl:if>
                                                    </span>
                                                    <xsl:variable name="costDate" select="$cadCost/tns:data"/>
                                                    <xsl:if test="$costDate">
                                                        <span class="bold">
                                                            <xsl:call-template name="DateStr">
                                                                <xsl:with-param name="dateStr" select="$costDate"/>
                                                            </xsl:call-template>
                                                        </span>
                                                    </xsl:if>
                                                </xsl:if>
                                            ...
                                            <xsl:if test="$isDepositary = 'true'">
                                                <div class="row">
                                                    ...
                                                    <div class="row">
                                                        <div class="cell center w40 bold">V</div>
                                                        <div class="cell leftpad wrap bold">
                                                            <xsl:text>с указанием сведений о депозитарии, который осуществляет хранение обездвиженной документарной закладной или электронной закладной</xsl:text>
                                                        </div>
                                                    </div>
                                                </div>
                                            </xsl:if>
                                    ...
                                    <xsl:if test="$extractSubject/tns:uptodateData = 'true'">
                                        <div class="row">
                                            <div class="table tablemargin">
                                                ...
                                                <div class="cell center w40 bold">V</div>
                                                <div class="cell leftpad wrap">
                                                    <xsl:text>только актуальные на дату предоставления сведения о зарегистрированных правах</xsl:text>
                                                </div>
                                            </div>
                                        </div>
                                    </xsl:if>
...
<xsl:value-of select="($thisDocument/doc:series)"/-->
	...
	<xsl:template name="agreementData">
        ...
        <xsl:value-of select="$agreement//xs:simpleType[@name=$typeName]/xs:restriction/xs:enumeration[@value=normalize-space($agreementCode)]/xs:annotation/xs:documentation"/>
	</xsl:template>
