...
input[type="reset"],input[type="submit"] {
				...
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							...
							<xsl:variable
								name="cadPlan"
								select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:cadastralPlanTerritory"
							/>
							<xsl:variable
								name="terrZone"
								select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:area"
							/>
									...
									<div class="table tablemargin">
										<div class="row">
											...
											<div class="cell leftpad wrap">
												<xsl:text
													>о зоне, публичном сервитуте, территории, о береговой
													линии (границе водного объекта), лесничестве,
													охотничьем угодье, Байкальской природной территории и
													ее экологических зонах, проекте межевания территории:
												</xsl:text>
											</div>
										</div>
									</div>
												...
												<div class="table tablemargin">
													...
													<div class="cell left wrap">
														...
														<xsl:if test="$cadPlan">
															<xsl:variable
																name="cadNumber"
																select="$cadPlan/tns:cadastralBlock/obj:number/obj:cadastralNumber"
															/>
															<xsl:variable
																name="orient"
																select="$cadPlan/tns:cadastralBlock/obj:Orient"
															/>
															<xsl:if test="$orient">
																<span class="bold">
																	<xsl:text
																		>территории в пределах кадастрового квартала
																		с №
																	</xsl:text>
																...
																<div style="font-size: 80%">
																	<xsl:text
																		>(указываются ориентиры территории в
																		пределах кадастрового квартала)</xsl:text
																	>
																</div>
															</xsl:if>
														...
														<xsl:if test="$terrZone">
															<xsl:variable
																name="zoneNumber"
																select="$terrZone/tns:area/tns:number"
															/>
															<xsl:variable
																name="legalAct"
																select="$terrZone/tns:area/tns:legalAct"
															/>
															<div class="left">
																<xsl:text>
																	индивидуальное обозначение территориальной
																	зоны, публичного сервитута, зоны с особыми
																	условиями использования территории, территории
																	объекта культурного наследия, территории
																	опережающего социально-экономического
																	развития, зоны территориального развития в
																	Российской Федерации, игорной зоны,
																	лесничества, особо охраняемой природной
																	территории, особой экономической зоны,
																	охотничьего угодья, экологической зоны
																	Байкальской природной территории</xsl:text
																>
																<br />
																<span class="bottomborder bold">
																	<xsl:if
																		test="$terrZone/tns:area/tns:borderType = 17"
																	>
																		<xsl:text>учетный номер </xsl:text>
																	</xsl:if>
																	<xsl:if
																		test="not ($terrZone/tns:area/tns:borderType = 17)"
																	>
																	...
																	<xsl:if
																		test="not ($terrZone/tns:area/tns:borderType = 6 or $terrZone/tns:area/tns:borderType = 7)"
																	>
																		<xsl:text>, </xsl:text>
																		<xsl:call-template
																			name="borderTypeTemplate"
																		>
																			<xsl:with-param
																				name="borderType"
																	...
																	<xsl:if
																		test="$terrZone/tns:area/tns:borderType = 6 or $terrZone/tns:area/tns:borderType = 7"
																	>
																		<!--<вид зоны> (если не заполнен вид зоны по документу) или <вид зоны по документу>,-->
																		<xsl:if
																			test="not($terrZone/tns:area/tns:individualDesignation/tns:codeZoneDoc) and $terrZone/tns:area/tns:individualDesignation/tns:codeZone"
																		>
																			<xsl:text>, </xsl:text>
																			<xsl:call-template
																				name="codeZoneTemplate"
																			>
																				<xsl:with-param
																					name="codeZone"
																		...
																		<xsl:if
																			test="$terrZone/tns:area/tns:individualDesignation//tns:codeZoneDoc"
																		>
																			<xsl:text>, </xsl:text>
																			<xsl:value-of
																	...
																	>
																		<xsl:if
																			test="$terrZone/tns:area/tns:individualDesignation/tns:numberZone"
																		>
																		...
																		<xsl:if
																			test="$terrZone/tns:area/tns:individualDesignation/tns:index"
																		>
																		...
																		<xsl:if
																			test="$terrZone/tns:area/tns:individualDesignation/tns:other"
																		>
																			<xsl:text
															...
															<div class="left">
																<xsl:text>
																	наименование органа государственной власти или
																	органа местного самоуправления, принявшего
																	решение об установлении, изменении зоны,
																	установлении публичного сервитута, границы
																	территории, границы лесничества, охотничьего
																	угодья, Байкальской природной территории и ее
																	экологических зон, береговой линии (границы
																	водного объекта), утверждении проекта
																	межевания территории
																</xsl:text>
																<br />
																<span class="bottomborder bold">
																	<xsl:value-of
																		select="$legalAct/doc:issuer/doc:name"
																	/>
																	<xsl:text>;</xsl:text>
																</span>
															</div>
															...
															<div class="left">
																<xsl:text>
																	реквизиты решения об установлении, изменении
																	зоны, установлении публичного сервитута,
																	границы территории, границы лесничества,
																	охотничьего угодья, береговой линии (границы
																	водного объекта), утверждении проекта
																	межевания территории
																</xsl:text>
																<br />
							...
							<xsl:variable
								name="border"
								select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:border/tns:border"
							/>
							<xsl:if test="$border">
								<div class="row">
									<div class="table tablemargin">
												...
												<div class="table tablemargin">
													...
													<div class="cell left wrap">
														...
														<xsl:if test="$border">
															<xsl:variable
																name="region"
																select="$border/tns:region"
															/>
															<xsl:variable
																name="city"
																select="$border/tns:city"
															/>
															<xsl:variable
																name="locality"
																select="$border/tns:locality"
															/>
															...
															<xsl:if test="$region">
																...
																<span class="bottomborder bold">
																	<xsl:for-each select="$region">
																		<xsl:value-of
																			select="concat(adr:region/adr:type, ' ', adr:region/adr:name)"
																		/>
																...
																<div style="font-size: 80%">
																	<xsl:text>
																		названия субъектов (субъекта) Российской
																		Федерации</xsl:text
																	>
																</div>
															</xsl:if>
															...
															<xsl:if test="$city">
																...
																<span class="bottomborder bold">
																	<xsl:value-of
																		select="concat($city/adr:region/adr:type, ' ', $city/adr:region/adr:name)"
																	/>
																	<xsl:if test="$city/adr:autonomy">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($city/adr:autonomy/adr:type, ' ', $city/adr:autonomy/adr:name)"
																		/>
																	</xsl:if>
																	<xsl:if test="$city/adr:district">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($city/adr:district/adr:type, ' ', $city/adr:district/adr:name)"
																		/>
																	</xsl:if>
																	<xsl:if test="$city/adr:city">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($city/adr:city/adr:type, ' ', $city/adr:city/adr:name)"
																		/>
																	</xsl:if>
																</span>
																<br />
																<div style="font-size: 80%">
																	<xsl:text
																		...
																		субъекта Российской Федерации</xsl:text
																	>
																</div>
															</xsl:if>
															...
															<xsl:if test="$locality">
																...
																<span class="bottomborder bold">
																	<xsl:value-of
																		select="concat($locality/adr:region/adr:type, ' ', $locality/adr:region/adr:name)"
																	/>
																	<xsl:if test="$locality/adr:autonomy">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($locality/adr:autonomy/adr:type, ' ', $locality/adr:autonomy/adr:name)"
																		/>
																	</xsl:if>
																	<xsl:if test="$locality/adr:district">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($locality/adr:district/adr:type, ' ', $locality/adr:district/adr:name)"
																		/>
																	</xsl:if>
																	<xsl:if test="$locality/adr:city">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($locality/adr:city/adr:type, ' ', $locality/adr:city/adr:name)"
																		/>
																	</xsl:if>
																	<xsl:if test="$locality/adr:urbanDistrict">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($locality/adr:urbanDistrict/adr:type, ' ', $locality/adr:urbanDistrict/adr:name)"
																		/>
																	</xsl:if>
																	<xsl:if test="$locality/adr:locality">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of
																			select="concat($locality/adr:locality/adr:type, ' ', $locality/adr:locality/adr:name)"
																		/>
																	</xsl:if>
																</span>
																<br />
																<div style="font-size: 80%">
																	<xsl:text
																		>название населенного пункта</xsl:text
																	>
																</div>
															</xsl:if>
														</xsl:if>
							...
							<xsl:variable
								name="bordernum"
								select="$border/tns:boundaryObjectRecordNumber"
							/>
									...
									<div class="table tablemargin">
										<div class="row">
											<div class="cell left wrap">
												...
												<xsl:text>
													реестровый номер границы между субъектами Российской
													Федерации, границы муниципального образования субъекта
													Российской Федерации, границы населенного пункта
									...
									<div class="table tablemargin">
										<div class="row">
											<div class="cell left wrap">
												...
												<br />
												<xsl:text>
													реквизиты правового акта об утверждении, изменении
													границы между субъектами Российской Федерации, границы
													муниципального образования субъекта Российской
													Федерации, границы населенного пункта субъекта
								...
								<div class="table tablemargin">
									<div class="row">
										...
										<div class="cell leftpad wrap">
											...
											<xsl:if test="$terrZone">
												<xsl:text
													>выписки о зоне с особыми условиями использования
													территорий, территориальной зоне, публичном сервитуте,
													территории объекта культурного наследия, территории
													опережающего социально-экономического развития, зоне
													территориального развития в Российской Федерации,
													игорной зоне, лесничестве, особо охраняемой природной
													территории, особой экономической зоне, охотничьем
													угодье, береговой линии (границе водного объекта),
													проекте межевания территории</xsl:text
												>
											</xsl:if>
											<xsl:if test="$border">
												<xsl:text
													>выписки о границе между субъектами Российской
													Федерации, границе муниципального образования и
													границе населенного пункта</xsl:text
												>
											</xsl:if>
										</div>
									</div>
	...
	<xsl:template name="point31person">
		...
		<xsl:if test="$thisPerson/subj:contactInfo">
			...
			<xsl:if test="$thisPerson/subj:contactInfo/subj:postalAddress">
				...
				<xsl:if
					test="$thisPerson/subj:contactInfo/subj:email or $thisPerson/subj:contactInfo/subj:phoneNumber"
				>
					<br />
				</xsl:if>
			</xsl:if>
	...
	<xsl:template name="point32organization">
			...
			<xsl:if test="$thisOrganization/subj:regDate">
				...
				<b>
					<xsl:call-template name="DateStrFull">
						<xsl:with-param
							name="dateStr"
							select="$thisOrganization/subj:regDate"
						/>
					</xsl:call-template>
				</b>
			</xsl:if>
		...
		<xsl:if test="$mode1 != 'owner'">
			<xsl:if
				test="$thisOrganization/subj:nativeForeignParams/node()/subj:regDate"
			>
				<br />
				...
				<span class="bold">
					<xsl:call-template name="DateStrFull">
						<xsl:with-param
							name="dateStr"
							select="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:regDate"
						/>
					</xsl:call-template>
				</span>
				<xsl:if
		...
		<xsl:if test="$thisOrganization/subj:contactInfo">
			...
			<xsl:if test="$thisOrganization/subj:contactInfo/subj:postalAddress">
				...
				<xsl:if
					test="$thisOrganization/subj:contactInfo/subj:email or $thisOrganization/subj:contactInfo/subj:phoneNumber"
				>
					<br />
				</xsl:if>
			</xsl:if>
									...
									<div class="table tablemargin">
										<div class="cell left wrap">
											...
											<span class="sidepad center">
												<div style="font-size: 80%">
													<xsl:text
														>(основание запроса сведений, в том числе
														наименование государственной или муниципальной
														услуги или базового государственного информационного
														ресурса)</xsl:text
													>
												</div>
											</span>
			...
			<div class="row">
				...
				<div class="cell clear">
					<div class="table tablemargin">
						...
						<xsl:if test="$typeOrgan = 'MFC'">
							<xsl:call-template name="signTemplateMfc"> </xsl:call-template>
						</xsl:if>
					</div>
				</div>
			</div>
		...
		<xsl:if test="obj:mark">
			<xsl:choose>
				...
				<xsl:when test="@noteType='mortgageOwner'">
					<xsl:text
						>Государственная регистрация смены владельца закладной, в том числе
	...
	<xsl:template name="borderTypeTemplate">
		...
		<!--xsl:variable name="usageTypeEnum" select="document('schema/TStatement/Dictionary/DUsageType.xsd')"/-->
		<xsl:value-of
			select="$borderTypeEnum//xs:enumeration[@value=$borderType]/xs:annotation/xs:documentation"
		/>
	</xsl:template>
	<xsl:template name="codeZoneTemplate">
		...
		<xsl:value-of
			select="$codeZoneEnum//xs:enumeration[@value=$codeZone]/xs:annotation/xs:documentation"
		/>
	</xsl:template>
