...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
		...
		<div class="table">
			<div class="row">
				<div class="cell lh-middle center w260 wrap">
					<div class="ul center bottompad5">1. Запрос о предоставлении сведений, содержащихся в Едином государственном реестре недвижимости</div>
				...
				<div class="row">
					...
					<div class="cell clear">
						<div class="table tablemargin">
							...
							<!-- Территории или зоны -->	
							<xsl:variable name="cadPlan" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:cadastralPlanTerritory"/>
							<xsl:variable name="terrZone" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:area"/>
							<xsl:if test="$cadPlan or $terrZone">
								<div class="row">
									<div class="table tablemargin">
										<div class="row">
											...
											<div class="cell leftpad wrap">
												<xsl:text>о зоне, публичном сервитуте, территории, о береговой линии (границе водного объекта),
                                                    лесничестве, охотничьем угодье, Байкальской природной территории и ее экологических зонах,
                                                    проекте межевания территории: </xsl:text>
											</div>
										</div>
									</div>
								</div>
											...
											<div class="row">
												<div class="table tablemargin">
													...
													<div class="cell left wrap ">
														<!-- КК -->
														<xsl:if test="$cadPlan">
															<xsl:variable name="cadNumber" select="$cadPlan/tns:cadastralBlock/obj:number/obj:cadastralNumber"/>
															<xsl:variable name="orient" select="$cadPlan/tns:cadastralBlock/obj:Orient"/>
															<xsl:if test="$orient">
																<span class="bold">
																	<xsl:text>территории в пределах кадастрового квартала с № </xsl:text>
																	<xsl:if test="$cadNumber">
																		<xsl:value-of select="$cadNumber"/>
																	</xsl:if>
																	<xsl:if test="not($cadNumber)">
																		<xsl:text>_____________ </xsl:text>
																	</xsl:if>
																	<xsl:text> ограниченной </xsl:text>
                                                                    <br/>
																	<span class="bottomborder wrap">
																		<xsl:value-of select="$orient"/>
																	</span>
																</span>	
																<br/>
                                                                <div style=" font-size: 80%;">
                                                                    <xsl:text>(указываются ориентиры территории в пределах кадастрового квартала)</xsl:text>
                                                                </div>
															</xsl:if>
															<xsl:if test="not($orient) and $cadNumber">
																<span class="bold">
																	<xsl:text>территории кадастрового квартала с № </xsl:text>
																	<xsl:value-of select="$cadNumber"/>
																</span>	
															</xsl:if>
														</xsl:if>
														<!-- Тер-зона -->
														<xsl:if test="$terrZone">
															<xsl:variable name="zoneNumber" select="$terrZone/tns:area/tns:number"/>
															<xsl:variable name="legalAct" select="$terrZone/tns:area/tns:legalAct"/>
                                                            <div class="left">
                                                                <xsl:text>
                                                                    индивидуальное обозначение территориальной зоны, публичного сервитута, зоны с особыми условиями использования территории,
                                                                    территории объекта культурного наследия, территории опережающего социально-экономического развития,
                                                                    зоны территориального развития в Российской Федерации, игорной зоны, лесничества, особо охраняемой природной территории,
                                                                    особой экономической зоны, охотничьего угодья, экологической зоны Байкальской природной территории</xsl:text>
                                                                <br/>
                                                                <span class="bottomborder bold">
                                                                    <xsl:if test="$terrZone/tns:area/tns:borderType = 17">
                                                                        <xsl:text>учетный номер </xsl:text>
                                                                    </xsl:if>
                                                                    <xsl:if test="not ($terrZone/tns:area/tns:borderType = 17)">
                                                                        <xsl:text>реестровый номер </xsl:text>
                                                                    </xsl:if>
                                                                    <xsl:value-of select="$zoneNumber"/>

                                                                    <!-- <xsl:text>, вид объекта из реестра границ </xsl:text>-->
                                                                    <xsl:if test="not ($terrZone/tns:area/tns:borderType = 6 or $terrZone/tns:area/tns:borderType = 7)">
                                                                        <xsl:text>, </xsl:text>
                                                                        <xsl:call-template name="borderTypeTemplate">
                                                                            <xsl:with-param name="borderType"
                                                                                            select="$terrZone/tns:area/tns:borderType"/>
                                                                        </xsl:call-template>
                                                                    </xsl:if>

                                                                    <xsl:if test="$terrZone/tns:area/tns:borderType = 6 or $terrZone/tns:area/tns:borderType = 7">
                                                                        <!--<вид зоны> (если не заполнен вид зоны по документу) или <вид зоны по документу>,-->
                                                                        <xsl:if test="not($terrZone/tns:area/tns:individualDesignation/tns:codeZoneDoc) and $terrZone/tns:area/tns:individualDesignation/tns:codeZone">
                                                                            <xsl:text>, </xsl:text>
                                                                            <xsl:call-template name="codeZoneTemplate">
                                                                                <xsl:with-param name="codeZone"
                                                                                                select="$terrZone/tns:area/tns:individualDesignation/tns:codeZone"/>
                                                                            </xsl:call-template>
                                                                        </xsl:if>
                                                                        <xsl:if test="$terrZone/tns:area/tns:individualDesignation//tns:codeZoneDoc">
                                                                            <xsl:text>, </xsl:text>
                                                                            <xsl:value-of
                                                                                    select="$terrZone/tns:area/tns:individualDesignation//tns:codeZoneDoc"/>
                                                                        </xsl:if>
                                                                    </xsl:if>
                                                                    <xsl:if test="not ($terrZone/tns:area/tns:borderType = 17)">
                                                                        <xsl:if test="$terrZone/tns:area/tns:individualDesignation/tns:numberZone">
                                                                            <xsl:text>, номер </xsl:text>
                                                                            <xsl:value-of
                                                                                    select="$terrZone/tns:area/tns:individualDesignation/tns:numberZone"/>
                                                                        </xsl:if>
                                                                        <xsl:if test="$terrZone/tns:area/tns:individualDesignation/tns:index">
                                                                            <xsl:text>, индекс </xsl:text>
                                                                            <xsl:value-of
                                                                                    select="$terrZone/tns:area/tns:individualDesignation/tns:index"/>
                                                                        </xsl:if>
                                                                        <xsl:if test="$terrZone/tns:area/tns:individualDesignation/tns:other">
                                                                            <xsl:text>, иные идентифицирующие сведения: </xsl:text>
                                                                            <xsl:value-of
                                                                                    select="$terrZone/tns:area/tns:individualDesignation/tns:other"/>
                                                                        </xsl:if>
                                                                    </xsl:if>
                                                                    <xsl:text>;</xsl:text>
                                                                </span>
                                                            ...
                                                            <div class="left">
                                                                <xsl:text>
                                                                    наименование органа государственной власти или органа местного самоуправления, принявшего решение об установлении,
                                                                    изменении зоны, установлении публичного сервитута, границы территории, границы лесничества, охотничьего угодья, Байкальской природной
                                                                    территории и ее экологических зон, береговой линии (границы водного объекта), утверждении проекта межевания территории
                                                                </xsl:text>
                                                                <br/>
                                                                <span class="bottomborder bold">
                                                                    <xsl:value-of select="$legalAct/doc:issuer/doc:name"/>
                                                                    <xsl:text>;</xsl:text>
                                                                </span>
                                                            </div>
                                                            <br/>
                                                            <div class="left">
                                                                <xsl:text>
                                                                    реквизиты решения об установлении, изменении зоны, установлении публичного сервитута, границы территории, границы лесничества,
                                                                    охотничьего угодья, береговой линии (границы водного объекта), утверждении проекта межевания территории
                                                                </xsl:text>
                                                                <br/>
                                                                <span class="bottomborder bold">
                                                                    <xsl:call-template name="showDocumentData">
                                                                        <xsl:with-param name="thisDocument" select="$legalAct"/>
                                                                    </xsl:call-template>
                                                                    <xsl:text>;</xsl:text>
                                                                </span>
                                                            </div>
							...
							<!-- Границы -->	
							<xsl:variable name="border" select="//tns:EGRNRequest/tns:requestDetails/tns:requestEGRNDataAction/tns:border/tns:border"/>
							<xsl:if test="$border">
								<div class="row">
									<div class="table tablemargin">
										<div class="row">
											...
											<div class="row">
												<div class="table tablemargin">
													...
													<div class="cell left wrap">
														<!-- Тер-зона -->
														<xsl:if test="$border">
															<xsl:variable name="region" select="$border/tns:region"/>
															<xsl:variable name="city" select="$border/tns:city"/>
															<xsl:variable name="locality" select="$border/tns:locality"/>

                                                            <xsl:if test="$region">
                                                                <xsl:text>между субъектами Российской Федерации </xsl:text>
																<span class="bottomborder bold">
																	<xsl:for-each select="$region">
																		<xsl:value-of select="concat(adr:region/adr:type, ' ', adr:region/adr:name)" />
																		<xsl:if test="position() != count($region)">
																			<xsl:text>; </xsl:text>
																		</xsl:if>
																	</xsl:for-each>
																</span>
																<br/>
                                                                <div style=" font-size: 80%;">
                                                                    <xsl:text> названия субъектов (субъекта) Российской Федерации</xsl:text>
                                                                </div>
                                                            </xsl:if>
															...
															<xsl:if test="$city">
																...
																<span class="bottomborder bold">
																	<xsl:value-of select="concat($city/adr:region/adr:type, ' ', $city/adr:region/adr:name)" />
																	<xsl:if test="$city/adr:autonomy">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($city/adr:autonomy/adr:type, ' ', $city/adr:autonomy/adr:name)" />
																	</xsl:if>
																	<xsl:if test="$city/adr:district">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($city/adr:district/adr:type, ' ', $city/adr:district/adr:name)" />
																	</xsl:if>
																	<xsl:if test="$city/adr:city">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($city/adr:city/adr:type, ' ', $city/adr:city/adr:name)" />
																	</xsl:if>
																</span>
																<br/>
                                                                <div style=" font-size: 80%;">
                                                                    <xsl:text>название муниципального образования субъекта Российской Федерации</xsl:text>
                                                                </div>
															</xsl:if>
															...
															<xsl:if test="$locality">
																...
																<span class="bottomborder bold">
																	<xsl:value-of select="concat($locality/adr:region/adr:type, ' ', $locality/adr:region/adr:name)" />
																	<xsl:if test="$locality/adr:autonomy">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($locality/adr:autonomy/adr:type, ' ', $locality/adr:autonomy/adr:name)" />
																	</xsl:if>
																	<xsl:if test="$locality/adr:district">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($locality/adr:district/adr:type, ' ', $locality/adr:district/adr:name)" />
																	</xsl:if>
																	<xsl:if test="$locality/adr:city">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($locality/adr:city/adr:type, ' ', $locality/adr:city/adr:name)" />
																	</xsl:if>
																	<xsl:if test="$locality/adr:urbanDistrict">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($locality/adr:urbanDistrict/adr:type, ' ', $locality/adr:urbanDistrict/adr:name)" />
																	</xsl:if>
																	<xsl:if test="$locality/adr:locality">
																		<xsl:text>, </xsl:text>
																		<xsl:value-of select="concat($locality/adr:locality/adr:type, ' ', $locality/adr:locality/adr:name)" />
																	</xsl:if>
																</span>
																<br/>
                                                                <div style=" font-size: 80%;">
                                                                    <xsl:text>название населенного пункта</xsl:text>
                                                                </div>
															</xsl:if>
														</xsl:if>
													</div>
                            ...
                            <xsl:variable name="bordernum" select="$border/tns:boundaryObjectRecordNumber"/>
                            <xsl:if test="$bordernum">
                                <div class="row">
                                    <div class="table tablemargin">
                                        <div class="row">
                                            <div class="cell left wrap">
                                                <span class="bottomborder bold ">
                                                    <xsl:value-of select="$bordernum"/>
                                                    <br/>
                                                </span>
                                                <xsl:text>
                                                    реестровый номер границы между субъектами Российской Федерации, границы муниципального образования субъекта
                                                    Российской Федерации, границы населенного пункта субъекта Российской Федерации
                                            </xsl:text>
                            ...
                            <xsl:variable name="document" select="$border/tns:document"/>
                                    ...
                                    <div class="table tablemargin">
                                        <div class="row">
                                            <div class="cell left wrap">
                                                <span class="bottomborder bold ">
                                                    <xsl:call-template name="showDocumentData">
                                                        <xsl:with-param name="thisDocument" select="$document"/>
                                                    </xsl:call-template>
                                                </span>
                                                <br/>
                                                <xsl:text>
                                                    реквизиты правового акта об утверждении, изменении границы между субъектами Российской Федерации,
                                                    границы муниципального образования субъекта Российской Федерации,
                                                    границы населенного пункта субъекта Российской Федерации
                                                </xsl:text>
                                            </div>
                                        </div>
                                    </div>
							...
							<div class="row">
								<div class="table tablemargin">
									<div class="row">
										...
										<div class="cell leftpad wrap">
											<xsl:if test="$cadPlan">
												<xsl:text>кадастрового плана территории</xsl:text>
											</xsl:if>	
											<xsl:if test="$terrZone">
												<xsl:text>выписки о зоне с особыми условиями использования территорий, территориальной зоне, публичном сервитуте, территории объекта культурного наследия, территории опережающего социально-экономического развития, зоне территориального развития в Российской Федерации, игорной зоне, лесничестве, особо охраняемой природной территории, особой экономической зоне, охотничьем угодье, береговой линии (границе водного объекта), проекте межевания территории</xsl:text>
											</xsl:if>	
											<xsl:if test="$border">
												<xsl:text>выписки о границе между субъектами Российской Федерации, границе муниципального образования и границе населенного пункта</xsl:text>
											</xsl:if>	
										</div>
									</div>
								</div>
	...
	<xsl:template name="point311">
		...
		<xsl:param name="mode1" select="'owner'"/>
	...
	<xsl:template name="point31person">
		...
		<xsl:variable name="subjDoc" select="//tns:EGRNRequest/tns:header/stCom:appliedDocument/node()[@_id=$thisPerson/subj:idDocumentRef/@documentID]"/>
			...
			<xsl:if test="$thisPerson/subj:address">
				...
				<!-- Согласно шаблону одной строкой ^^^^ 
				<xsl:choose>
			...
			<xsl:if test="$thisPerson/subj:contactInfo">
				...
				<xsl:if test="$thisPerson/subj:contactInfo/subj:postalAddress">
					...
					<xsl:if test="$thisPerson/subj:contactInfo/subj:email or $thisPerson/subj:contactInfo/subj:phoneNumber">
						<br/>
					</xsl:if>
				</xsl:if>
	...
	<xsl:template name="point32organization">
		...
		<xsl:param name="mode1" select="'declarant'"/>
			...
			<!-- Для заявителей дата ГОС регистрации (после КПП)-->
			<xsl:if test="$mode1 = 'declarant'">
				<!--xsl:if test="$thisOrganization/subj:nativeForeignParams/node()/subj:regDate or $thisOrganization/subj:regDate"-->
				<xsl:if test="$thisOrganization/subj:regDate">
					<br/>
					<xsl:text>дата государственной регистрации </xsl:text>
					<b>
						<xsl:call-template name="DateStrFull">
							<xsl:with-param name="dateStr" select="$thisOrganization/subj:regDate"/>
						</xsl:call-template>
					</b>
				</xsl:if>
			</xsl:if>	
			...
			<!-- Для Заявителей выводим после страны регистрации ТОЛЬКО дату и номер для организаций -->
			<xsl:if test="$mode1 != 'owner'">
				<xsl:if test="$thisOrganization/subj:nativeForeignParams/node()/subj:regDate">
					<br/>
					<xsl:text>дата и номер регистрации </xsl:text>
					<span class="bold">
						<xsl:call-template name="DateStrFull">
							<xsl:with-param name="dateStr" select="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:regDate"/>
						</xsl:call-template>
					</span>
			...
			<xsl:if test="$thisOrganization/subj:contactInfo">
				<br/>
				<!--xsl:text>____________________________________________________________________________________________</xsl:text>
				<br/-->
				<xsl:if test="$thisOrganization/subj:contactInfo/subj:postalAddress">
					...
					<xsl:if test="$thisOrganization/subj:contactInfo/subj:email or $thisOrganization/subj:contactInfo/subj:phoneNumber">
						<br/>
					</xsl:if>
				</xsl:if>
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
									<div class="cell leftpad  wrap">
										<xsl:text>Сведения о заявителе (физическом лице, кадастровом инженере, арбитражном управляющем, нотариусе, судебном приставе-исполнителе):</xsl:text>
									</div>
								</div>
							</div>
							<div class="row">
								<div class="table tablemargin">
									<xsl:if test="$operson | $opersonPrevileg">
										<xsl:call-template name="point311">
											...
											<xsl:with-param name="mode1" select="'declarant'"/>
										</xsl:call-template>
									</xsl:if>
								</div>
							</div>
					...
					<div class="cell clear">
						<div class="table tablemargin">
							<xsl:if test="//tns:EGRNRequest/tns:statementAgreements/stCom:actualDataAgreement">
								...
								<xsl:if test="normalize-space(//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:serviceCode)">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell left wrap">
												<xsl:text>Номер (идентификатор) услуги в реестре государственных услуг или в реестре муниципальных услуг:</xsl:text>
                                                <br/>
												<span class="bold">
													<xsl:value-of select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:serviceCode"/>
												</span>
											</div>
										</div>
									</div>
								</xsl:if>
								<xsl:if test="normalize-space(//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:legalActSection)">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell left wrap">
												<xsl:text>Положение соответствующего нормативного правового акта: </xsl:text>
												<span class="bold">
													<xsl:value-of select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:legalActSection"/>
												</span>
											</div>
										</div>
									</div>
								</xsl:if>
								<xsl:if test="normalize-space(//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:answerDate)">
									<div class="row">
										<div class="table tablemargin">
											<div class="cell left wrap">
												<xsl:text>Срок ожидаемого ответа на межведомственный запрос: </xsl:text>
												<span class="bold">
													<xsl:call-template name="DateStrFull">
														<xsl:with-param name="dateStr" select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:answerDate"/>
													</xsl:call-template>
												</span>
											</div>
										</div>
                                ...
                                <xsl:if test="normalize-space(//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:legalAct)">
                                    <span class="nopadding wrap">
                                        <xsl:text>Реквизиты решения руководителя федерального государственного органа, определенного Президентом Российской Федерации, которым уполномочено должностное лицо такого органа: </xsl:text>
                                        <span class="bold">
                                            <xsl:call-template name="showDocumentData">
                                                <xsl:with-param name="thisDocument"
                                                                select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:legalAct"/>
                                            </xsl:call-template>
                                        </span>
                                    </span>
                                </xsl:if>
                                <xsl:if test="normalize-space(//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:post)">
                                    <br/>
                                    <span class="bold bottomborder wrap">
                                        <xsl:value-of select="//tns:EGRNRequest/tns:header/stCom:serviceInfo/stCom:post"/>
                                    </span>
                                    <span class="sidepad center">
                                        <div style=" font-size: 80%;">
                                            <xsl:text>(должность, включая полное наименование органа, реквизиты документа, подтверждающего наделение нотариуса полномочиями)</xsl:text>
                                        </div>
                                    </span>
                                </xsl:if>
                                <xsl:variable name="signPerson"  select="//tns:EGRNRequest/tns:declarant/subj:representative/subj:subject/subj:person"/>
                                <xsl:if test="$signPerson">
                                    <xsl:call-template name="signTemplateSimple">
                                        <xsl:with-param name="person" select="$signPerson"/>
                                    </xsl:call-template>
                                </xsl:if>
                                <xsl:if test="not($signPerson)">
                                    <xsl:call-template name="signTemplateSimple">
                                        <xsl:with-param name="person" select="//tns:EGRNRequest/tns:declarant/subj:person"/>
                                    </xsl:call-template>
                                </xsl:if>
                            </xsl:if>
						</div>
					...
					<div class="table tablemargin">
						<div class="row">
							<div class="table tablemargin">
								<div class="cell leftpad w543 wrap">
									<xsl:text>Подлинность подписи заявителя (представителя заявителя) свидетельствую:</xsl:text>
								</div>
						...
						<xsl:if test="$typeOrgan = 'PVD'">
							<xsl:call-template name="signTemplate">
								...
								<xsl:with-param name="notary" select="//tns:EGRNRequest/tns:declarant/subj:notary"/>
							</xsl:call-template>
						</xsl:if>	
						<xsl:if test="$typeOrgan = 'MFC'">
	...
	<xsl:template name="point12">
		<xsl:variable name="note12" select="//tns:EGRNRequest/tns:requestDetails/tns:note/com:text"/>
		<xsl:if test="$note12">
			<div class="table">
				<div class="row">
		...
		<xsl:param name="modeIdDoc" select="'default'"/>
			...
			<xsl:if test="contains(name($thisDocument), ':idDocument')">
				<xsl:if test="$thisDocument/doc:name">
                    <b><xsl:value-of select="$thisDocument/doc:name"/></b>
				...
				<xsl:if test="$thisDocument/doc:issuer/doc:name">
                    ...
                    <b><xsl:value-of select="$thisDocument/doc:issuer/doc:name"/></b>
	...
	<xsl:template match="obj:objectNote">
		<xsl:if test="obj:propertyNote">
			...
			<xsl:if test="obj:mark or obj:note">
				<xsl:text>, </xsl:text>
			</xsl:if>
        </xsl:if>
	...
	<xsl:template name="borderTypeTemplate">
		<xsl:param name="borderType"/>
		<xsl:variable name="dictPath" select="'Dictionary/DBorderObjectType.xsd'" />
		<xsl:variable name="borderTypeEnum" select="document($dictPath)"/>
		<!--xsl:variable name="usageTypeEnum" select="document('schema/TStatement/Dictionary/DUsageType.xsd')"/-->
		<xsl:value-of select="$borderTypeEnum//xs:enumeration[@value=$borderType]/xs:annotation/xs:documentation"/>
	</xsl:template>
	<xsl:template name="codeZoneTemplate">
		<xsl:param name="codeZone"/>
		<xsl:variable name="dictPath" select="'Dictionary/DTerzone.xsd'" />
		<xsl:variable name="codeZoneEnum" select="document($dictPath)"/>
		<xsl:value-of select="$codeZoneEnum//xs:enumeration[@value=$codeZone]/xs:annotation/xs:documentation"/>
	</xsl:template>

	<xsl:template name="signTemplateSimple">
        <xsl:param name="person"  select="null"/>
		<div class="pagebreak">
			<div class="row">
				<div class="table tablemargin">
                    ...
                    <div class="cell leftpad w251">
                        <div class="title">
                            <xsl:if test="$person/subj:surname">
                            ...
                            <xsl:if test="not($person/subj:surname)">
                                <div class="left wrap sidepad ul">
                                    <xsl:text>&nbsp;</xsl:text>
                                </div>
                            </xsl:if>
