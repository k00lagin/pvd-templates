...
>
	...
	<xsl:variable
		name="requestCopyAction"
		select="//tns:EGRNRequest/tns:requestDetails/tns:requestCopyAction"
	/>
...
input[type="reset"],input[type="submit"] {
			...
			<div class="row">
				<div class="cell lh-middle center w260 wrap">
					<div class="ul center bottompad5">
						1. Запрос о предоставлении копии документа, на основании которого
						сведения об объекте недвижимости внесены в Единый государственный
						реестр недвижимости, или копий документов, помещенных в реестровое
						дело
					</div>
					<span
						...
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad">
									Прошу предоставить копию помещенного в реестровое дело:
								</div>
							</div>
							<xsl:if
								test="$requestCopyAction/tns:object/tns:boundaryObjectRecordNumber"
							>
								<div class="row">
									<div class="table tablemargin">
										...
										<div class="cell leftpad wrap">
											<xsl:text
												>о зоне, публичном сервитуте, территории или границе с
												реестровым номером:
											</xsl:text>
											<span class="bold">
												<xsl:value-of
													select="$requestCopyAction/tns:object/tns:boundaryObjectRecordNumber"
												/>
											</span>
										</div>
									</div>
								...
								<xsl:if test="$requestCopyAction/tns:allDocument">
									<div class="row">
										<div class="table tablemargin">
											...
											<div class="cell leftpad wrap">
												<xsl:text
													>документ, на основании которого сведения о зоне,
													публичном сервитуте, территории или границах внесены в
									...
									<div class="row">
										<div class="table tablemargin">
											...
											<div class="cell leftpad wrap">
												...
												<span class="bold">
													<xsl:for-each
														select="$requestCopyAction/tns:document"
													>
							...
							<xsl:if
								test="$requestCopyAction/tns:object/tns:objectCadastralNumber"
							>
								<div class="row">
									<div class="table tablemargin">
										<div class="cell center w40 bold">
										...
										<div class="cell leftpad">
											<xsl:text
												>об объекте недвижимости с кадастровым номером:
											</xsl:text>
											<span class="bold">
												<xsl:value-of
													select="$requestCopyAction/tns:object/tns:objectCadastralNumber"
												/>
											</span>
										</div>
									</div>
								...
								<xsl:if test="$requestCopyAction/tns:allDocument">
									<div class="row">
										<div class="table tablemargin">
											...
											<div class="cell leftpad wrap">
												<xsl:text
													>иного документа, на основании которого сведения об
													объекте недвижимости внесены в Единый государственный
													реестр недвижимости, иного документа, помещенного в
													реестровое дело
												</xsl:text>
												<span class="bold">
													<xsl:text>все</xsl:text>
										...
										<div class="row">
											<div class="table tablemargin">
												...
												<div class="cell leftpad wrap">
													<xsl:text
														>иного документа, на основании которого сведения об
														объекте недвижимости внесены в Единый
														государственный реестр недвижимости, иного
														документа, помещенного в реестровое дело
													</xsl:text>
													<span class="bold">
														<xsl:for-each
															select="$requestCopyAction/tns:document"
												...
												<div class="table tablemargin">
													...
													<div class="cell leftpad wrap">
														<xsl:call-template name="documentMatchTable">
															<xsl:with-param
																name="docCode"
																select="doc:requestDocumentType"
															/>
														</xsl:call-template>
														<xsl:text>&nbsp;</xsl:text>
														<span class="bold">
								...
								<div class="table tablemargin">
									<div class="cell leftpad wrap">
										<xsl:text
											...
											управляющем, судебном приставе-исполнителе,
											нотариусе):</xsl:text
										>
									</div>
								</div>
	...
	<xsl:template name="point31person">
		...
		<xsl:if
			test="$thisPerson/subj:contactInfo or (($mode1 != 'representative') and $thisPerson/subj:snils)"
		>
			<div class="row">
				<div class="table tablemargin">
					<div class="cell wrap">
						<xsl:if test="$thisPerson/subj:contactInfo/subj:phoneNumber">
							...
							<span class="bold">
								...
								/>
								<xsl:if
									test="$thisPerson/subj:contactInfo/subj:postalAddress or $thisPerson/subj:contactInfo/subj:email"
								>
									<xsl:text>, </xsl:text>
								</xsl:if>
							</span>
						...
						<xsl:if
							test="($mode1 != 'representative') and $thisPerson/subj:snils"
						>
							<xsl:text>СНИЛС </xsl:text>
							<span class="bold">
								<xsl:value-of select="$thisPerson/subj:snils" />
		...
		<div class="row">
			<div class="table tablemargin">
				<div class="cell wrap">
					...
					<xsl:if
						test="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:ogrn"
					>
						<xsl:text>, ОГРН </xsl:text>
						<span class="bold">
							<xsl:value-of
								select="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:ogrn"
							/>
						</span>
					</xsl:if>
					...
					<xsl:if
						test="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:regDate"
					>
					...
					<xsl:if
						test="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:inn"
					>
						<xsl:text> ИНН </xsl:text>
						<span class="bold">
							<xsl:value-of
								select="$thisOrganization/subj:nativeForeignParams/subj:nativeOrgParams/subj:inn"
							/>
						</span>
					</xsl:if>
					...
					>
						<br />
						<xsl:text
							>Страна регистрации (инкорпорации), код иностранной организации
							(КИО)
						</xsl:text>
						<span class="bold">
...
<xsl:value-of select="($thisDocument/doc:series)"/-->
	...
	<xsl:template name="documentMatchTable">
		...
		<xsl:if test="$docCode = 561010016000"
			>документа, на основании которого сведения о зоне, публичном сервитуте,
			территории или границах внесены в Единый государственный реестр
			недвижимости</xsl:if
		>
	</xsl:template>
