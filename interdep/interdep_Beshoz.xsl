...
input[type="reset"],input[type="submit"]{-webkit-appearance:button;cursor:pointer}button[disabled],html input[disabled]{cursor:default}button::-moz-focus-inner,input::-moz-focus-inner{border:0;padding:0}input{line-height:normal}input[type="checkbox"],input[type="radio"]{box-sizing:border-box;padding:0}input[type="number"]::-webkit-inner-spin-button,input[type="number"]::-webkit-outer-spin-button{height:auto}input[type="search"]{-webkit-appearance:textfield;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;box-sizing:content-box}input[type="search"]::-webkit-search-cancel-button,input[type="search"]::-webkit-search-decoration{-webkit-appearance:none}fieldset{border:1px solid silver;margin:0 2px;padding:.35em .625em .75em}legend{border:0;padding:0}textarea{overflow:auto}optgroup{font-weight:700}table{border-collapse:collapse;border-spacing:0}td,th{padding:0}
				...
				<div class="cell title w380">
					<div class="sidepad wrap">
						...
						<xsl:if test="not(string($numPPOZ))">
							<span>2.1. № книги учета входящих документов _______</span>
							<div class="">
	...
	<xsl:template name="point3">
		<xsl:if test="$objects/stCom:object">
			<div class="table">
				<div class="row">
					<div class="cell leftpad w40">3</div>
		...
		<div class="table">
			<div class="row">
				...
				<div class="cell clear">
					<div class="table tablemargin">
						<div class="row">
							<div class="cell leftpad bold">Заявитель:</div>
						</div>
						<div class="row">
							<div class="table tablemargin">
								<div class="row">
	...
	<xsl:template name="point5">
		<xsl:if test="$owner">
			<div class="table">
				<div class="row">
					<div class="cell leftpad w40">5</div>
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="cell leftpad bold">Собственник:</div>
							</div>
							<div class="row">
								<div class="table tablemargin">
									<div class="row">
	...
	<xsl:template name="point6">
		...
		<xsl:if test="not(($needAppDocs = 'regRightAuthority') or ($needAppDocs = 'mfc') or ($needAppDocs = 'extReceipt')) ">
			...
			<xsl:if test="$appDocs">
				...
				<!-- debug - - >
				<xsl:text>*******: </xsl:text>
				<xsl:value-of select="count($appDocs)"/>
				<xsl:text> check1: </xsl:text>
				<xsl:value-of select="count($check1)"/>
				<xsl:text> check2: </xsl:text>
				<xsl:value-of select="count($check2)"/>
				<xsl:text> check31: </xsl:text>
				<xsl:value-of select="count($check31)"/>
				<xsl:text> check32: </xsl:text>
				<xsl:value-of select="count($check32)"/>
				<xsl:text> check4: </xsl:text>
				<xsl:value-of select="count($check4)"/>
				<xsl:text> plat: </xsl:text>
				<xsl:value-of select="count($check5)"/>
				<xsl:text> check6: </xsl:text>
				<xsl:value-of select="count($check6)"/>

				<xsl:text> totalCheck: </xsl:text>
				<xsl:value-of select="$totalCheck"/>
				<xsl:text> appDocCount: </xsl:text>
				...
				<xsl:if test="$totalCheck &lt; $appDocCount">
					<div class="table">
						<div class="row">
							<div class="cell leftpad w40">6</div>
							<div class="cell clear">
								<div class="table tablemargin">
									<div class="row">
										<div class="table tablemargin">
	...
	<xsl:template name="point7">
		...
		<xsl:if test="$note7">
			<div class="table">
				<div class="row">
					<div class="cell leftpad w40">7</div>
					<div class="cell clear">
						<div class="table tablemargin">
							<div class="row">
								<div class="table tablemargin">
	...
	<xsl:template name="point8">
		<div class="table">
			<div class="row">
				<div class="cell leftpad w40">8</div>
				<div class="cell clear">
					<div class="table tablemargin">
						<div class="row">
							<div class="table tablemargin">
			...
			<div class="row">
				...
				<div class="cell clear">
					<div class="table tablemargin">
						<div class="row">
							<div class="table tablemargin">
								<div class="cell leftpad w542 wrap bold">
